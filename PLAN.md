...
> **Status update 2026-06-03:** Phase 6 add-ons have landed, and a wave of Plan-2 data-layer + ecosystem features now ship and are test-covered: relations (`packages/do/src/relations.ts`), triggers/lifecycle hooks (`packages/do/src/triggers.ts`), Row-Level Security (`packages/server/src/rls/middleware.ts`), aggregate `count()` (O(1) counter tables in `@cirrus/do` + `@cirrus/d1`), ranked index `rank()`/`rankPage()` (`packages/do/src/rank.ts`), full-text search (DO backend; D1/`.global()` FTS still pending), vector search (`@cirrus/vectors`, now merged — see "Open questions" below), and rate limiting (`@cirrus/ratelimit`: token-bucket / sliding-window / fixed-window stores + middleware + `useRateLimit`). See PLAN2.md's "Remaining gaps (verified 2026-06-03)" for what's still outstanding.
...
- Vector search (Cloudflare Vectorize). **Built 2026-05-28; merged to `alpha` and now in the main tree** as `packages/vectors/` (`@cirrus/vectors`: `createVectors` runtime adapter, bring-your-own-embedder — user supplies `embed(input) => number[] | Promise<number[]>`, framework never couples to a provider). Both DSL shapes shipped:
