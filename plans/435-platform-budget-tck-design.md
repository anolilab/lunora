...
validation into `@lunora/platform` as a zero-dep helper that *reports* a
...
| 5 | **Advisory** | Same shape as row 4: a verbatim-forwarded option of a CF service with no in-repo second implementation today. Becomes a real drift hazard only if a Node `KVNamespaceLike` emulation lands; the build plan for *that* host should add the check to the facade then. |
...
helper therefore *finds* the violation; each adapter throws in its own error
 ...
 */
export const findSocketTagBudgetViolation = (
    tags?: ReadonlyArray<string>,
): string | undefined => { /* count check, then length check, as
