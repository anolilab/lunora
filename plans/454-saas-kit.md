...
**D4a. The shell ships with the items already composed into it, by running the real `registry add`.** _Added during E, which found the fork D4 did not anticipate._ A template whose routes import `./lunora/saas-ui/react` cannot build until those files exist, so "shell" cannot mean "empty": either `lunora init` grows a way for a template to declare the items it needs, or the composed result is committed. The second is Laravel's answer too (§2.5 — maestro builds the kits out from one source rather than having them assemble themselves), and it keeps the registry items as the single source: the payload in `templates/saas/lunora/` is produced by `lunora registry add saas-ui-react`, not hand-written. What D4 was actually protecting against — a backend hand-maintained in thirteen places — still holds, because no one edits the composed copy.
...
- **E — Template shells (S each).** **`templates/saas` done** (TanStack Start + React): the hand-authored frame is 12 files — routes, router, worker entry, configs, README — and the `lunora/` payload beneath it is composed by the real `registry add`, which pulled `saas-ui-react` → `saas` → `auth` + `ratelimit` in one command. 47 files total, the richest template in the repo. `lunora/server.ts` carries the two callbacks that make tenancy real: `resolveIdentity` and an `authorizeShard` that admits a caller to their own organisation's shard and the root shard, nothing else. Registered in the CLI's `FRAMEWORK_CHOICES`, so `lunora init -t saas` picks it up and `TEMPLATE_IDS` derives the rest. **Passes `test:templates`** — install, build, typecheck and a credential-free deploy dry run — which took four rounds and found the three defects in §9.5.
- **F — Billing UI + entitlement gating (M).** **Core and both views done.** Plan catalog in code (the provider owns money; the app owns what a plan _means_, which is what lets one UI sit on six adapters), `pricingRows`, `seatUsage`, `isEntitled`, plus `PricingTable`, `BillingPanel` and `Gated` in React and Svelte. 79 tests. Two decisions the tests pin: `past_due` still entitles, because cutting a customer off mid-dunning turns a card retry into a churn event; and seats count the organisation's members, not `subscription.quantity`, which lags an invite by however long a webhook takes. **Wired into the template too**, which settled the question A was re-scoped around: composing `payment` by hand costs three documented steps (inline the tables, wire the adapter, mount the webhook), and the exception is livable — so A stays "accept and document" rather than "bring payment into the schema-extension mechanism". Two decisions the wiring forced: the payment tables stay **root-scoped** while everything else is sharded, because a provider webhook arrives with no session and no tenant to resolve a shard from; and `referenceId` is the **organisation**, not the buyer, so a subscription cannot follow an admin out of the tenant and leave the rest of the team unable to see their own plan. `AuthorizeReference` receives only the reference with no identity, so that check lives in the functions, where `ctx.auth` does.
...
- **H — The realtime wedge (S).** **Done.** `presenceRoster` collapses per-tab presence rows into a per-person roster (two tabs are one person, as present as their freshest heartbeat), `PresenceBar` renders it in both ports, and the template's dashboard wires `usePresence` with the organisation as the room — presence scoped to the tenant exactly like its data. This is the one screen in the kit a request/response backend could not render at all, which is the point: §2.4.1 found the column empty across all 24 kits surveyed. 93 tests; the template still passes the smoke matrix.
...
## 9.5 Defects found by building this, worth fixing outside the kit

Each surfaced from a gate rather than from review, and each is other people's problem too:

1. **`lunora registry add` writes an unreadable schema stub.** On a project with no `lunora/schema.ts` it creates one exporting `export const schema = …`, but codegen's generated `app.ts` and `shard.ts` import that module's **default**. Every such project fails `tsc` with `TS2613` until someone changes the export by hand. Caught by `test:templates`; worked around in `templates/saas` by authoring the schema properly.
2. **`createAuth` erases better-auth's plugin inference.** `createAuth(options: LunoraAuthOptions): LunoraAuth` is annotated with the non-generic instance type, so a consumer who enables `organization()` or `admin()` gets the fields at runtime and nothing at the type level — `session.activeOrganizationId` and `user.role` simply do not exist to TypeScript, however the instance is annotated downstream. This hits anyone using any better-auth plugin through `@lunora/auth`, not just this kit. Contained for now behind one documented cast at the kit's single trust boundary (`templates/saas/lunora/server.ts`), where the identity contract validates the result and rejects a mismatch with a 401. The real fix is to make `createAuth` generic over its options.
3. **`payment`'s `mySubscriptions` returns too little to bill with.** It projects `providerSubscriptionId`, `referenceId` and `state` — no `priceId`, so nothing can match a subscription to a plan, and no `cancelAtPeriodEnd`, so no screen can say "cancels at period end". Any real billing page has to widen it first, as `templates/saas` does.
4. **A wrangler `vars` entry is not on `env`'s type.** `CloudflareBindings` is generated from bindings, so a plain var declared in `wrangler.jsonc` reaches the running Worker and not the type-checker — `env["APP_BASE_URL"]` is `TS7053`. The `payment` item reads exactly that key, so every project wiring payments hits it.
5. **`usePresence` is React-only.** `@lunora/react` ships it; `@lunora/svelte` (and Vue, Solid, Angular) do not, so a non-React app has to hand-roll the heartbeat interval, the visibility-change hook and the subscription. The kit survives this because its components take rows as props — both ports share `PresenceBar` and only the route differs — but the four remaining view ports will each hit it. Same shape as the adapter-parity work in plan 047.
6. **`useMutation` and `useAction` read as callable and are not.** They return `{ mutate, … }` and `{ call, … }`, and its own docs say to destructure — but the shape invites `createProject({ … })`, which type-errors with `TS2349: This expression is not callable`. Worth a line in the React adapter's docs, since the failure mode is one every first-time caller will hit.
