...
3. **`useMutation` reads as callable and is not.** It returns `{ mutate, pending, … }`, and its own docs say to destructure — but the shape invites `createProject({ … })`, which type-errors with `TS2349: This expression is not callable`. Worth a line in the React adapter's docs, since the failure mode is one every first-time caller will hit.
