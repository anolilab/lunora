...
- **Priority**: P3 (Niche. Workers for Platforms is for SaaS platforms running _untrusted customer-authored Workers_ in a dispatch namespace. That is a fundamentally different product shape from Lunora's "own your backend" positioning. No zeroback/Convex parity pressure here — neither offers it. This is a "say yes if a platform customer asks", not a roadmap item.)
- **Effort**: S for the honest minimum (validator awareness + a typed `env.DISPATCHER` accessor); L+ if Lunora ever tried to _own_ the multi-tenant dispatch story (sandboxing, per-tenant limits, script upload API, outbound bindings) — which it should not.
...
use case that is orthogonal to Lunora's value proposition (a type-safe backend
_you_ author). The honest deliverable is the **config-only minimum**: teach the
wrangler validator about `dispatch_namespaces` so a Lunora project that _does_
...
outbound bindings, tags/limits) is a separate product Lunora should not absorb.
...
- `packages/config/src/wrangler-validator.ts` — `WranglerConfig` (65-83) has no
  ...
  (the validator only checks known keys), but it gets no Lunora-side validation.
...
- [x] Item 1: Validator passthrough for `dispatch_namespaces`
    - `packages/config/src/wrangler-validator.ts` — add to `WranglerConfig` (65-83): `dispatch_namespaces?: ReadonlyArray<{ binding?: string; namespace?: string; outbound?: unknown } | null | undefined>`. Add a minimal `validateDispatchNamespaces(wrangler, errors)` (model on `validateTailConsumers`, 300-322): each entry needs a non-empty `binding` and a non-empty `namespace`. Wire it into `validateWranglerConfig` (346-401). Keep validation _thin_ — do not validate `outbound` binding shapes (that's deep WfP territory Lunora shouldn't police).
...
- [x] Item 3: Documentation note (scope honesty)
    - In the plan-status update (and any future docs PR — do **not** create docs files here unless the user asks), record explicitly: Lunora supports `dispatch_namespaces` as **wrangler config passthrough** (validated, optionally typed) for users who already operate a Workers-for-Platforms account; Lunora does **not** provide script-upload/management, tenant isolation, or outbound-binding helpers. This sets the boundary so a future contributor doesn't mistake passthrough support for an endorsement to build the multi-tenant runtime.
...
pnpm --filter "@lunora/config" run test                  # validateDispatchNamespaces
...
- The task is reinterpreted as "build multi-tenant Worker execution / script upload / tenant isolation" — **stop immediately and report**. That is a separate product with a security boundary Lunora must not casually own.
