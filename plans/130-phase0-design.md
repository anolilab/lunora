...
1. **Cursor format stability** — the base keyset cursor (`encodeCursor`/
   ...
   equivalent) is *not* unified — any refactor touching the rank-page
...
5. **Cross-shard/global routing indirection** — DO's `globalWriterFor`/
   fallback plumbing has no sql-store analog (sql-store tables *are* the
...
- The two "hidden-P" clusters (rank-page, companion-sync) look like the
  ...
  sql-store's ~254 inline lines) but are hidden behind the *same* sync/async
...
instruction: *"When editing one of [`aggregate`, `count`, `delete`,
...
different storage."* This directly targets the actual failure mode surfaced
