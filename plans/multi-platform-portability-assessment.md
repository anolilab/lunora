...
4. **In progress** — extract the reactive engine from `@lunora/do` into `@lunora/shard-engine`; re-bind `shard-do.ts` to `ShardHost`/`SocketHost` (move-only first; `_generated/` goldens stay byte-identical). `dependency-tracker.ts` and `transaction.ts` already moved; remaining engine files (`ctx-db.ts`, `reactive-cache.ts`, `relations.ts`, `socket-pool.ts`, relay tier) still in `@lunora/do`.
5. Create `@lunora/platform-cloudflare` as the default composition root; gradually fold Cloudflare-specific packages into it.
6. Build `@lunora/platform-aws` (plan 115) as the first non-Cloudflare target.
7. Update `lunorash` exports, Vite plugin, and CLI to support host selection (`target` field, default `cloudflare`).
