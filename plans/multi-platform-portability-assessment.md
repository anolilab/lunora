...
4. **In progress** — extract the reactive engine from `@lunora/do` into `@lunora/shard-engine`; re-bind `shard-do.ts` to `ShardHost`/`SocketHost` (move-only first; `_generated/` goldens stay byte-identical). Moved so far: `dependency-tracker.ts`, `transaction.ts`, `geo.ts`, `not-found-error.ts`, `search-text.ts`, `where-types.ts`, `reactive-cache.ts`, `socket-pool.ts`. Corresponding unit tests (and the reactive-cache bench) moved with them; the integration test that wires the cache into a DO shard stayed in `@lunora/do` and imports from `@lunora/shard-engine`. The `@lunora/do` barrel re-exports moved symbols so downstream packages keep compiling. Remaining engine files (`ctx-db.ts`, `relations.ts`, relay tier, `subscription-delivery.ts`) still in `@lunora/do`.
...
6. Create `@lunora/platform-cloudflare` as the default composition root; gradually fold Cloudflare-specific packages into it.
7. Build `@lunora/platform-aws` (plan 115) as the first non-Cloudflare target.
8. Update `lunorash` exports, Vite plugin, and CLI to support host selection (`target` field, default `cloudflare`).
