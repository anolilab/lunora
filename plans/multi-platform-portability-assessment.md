...
7. **Next** — move the remaining engine files (`relations.ts`, `relation-predicates.ts`, `query-args.ts`, `rank.ts`, `aggregates.ts`, `aggregate-sql.ts`, `aggregate-tally.ts`, and the `ctx-db-*.ts` family) into `@lunora/shard-engine` using the new `schema-types.ts` contracts.
