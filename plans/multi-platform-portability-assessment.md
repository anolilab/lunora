...
11. **Config inference/emission is split behind `DeployDriver` (partially adopted).** `@lunora/config` ships the provider-neutral `ResourceGraph` (shard namespaces, queues, workflows, containers, crons, global database, object storage, key-value store) and the `DeployDriver` contract, with `CLOUDFLARE_DRIVER` as the reference implementation delegating to the same `inferLunoraBindings` / `reconcileWrangler*` functions the CLI called directly before — so it is behavior-preserving by construction. `lunora prepare` now provisions through the driver and logs the neutral requirement summary.
    ...
    **Still open in §5.3:** `deploy` / `dev` / `tail` / `secret` are not modelled on the driver (those handlers still call wrangler directly, ~2,100 lines across the two), there is no `--target` selection yet, and the wrangler validator + `.dev.vars` machinery has not moved to a `cloudflare` subpath. The other four `inferLunoraBindings` callers (`deploy`, `doctor`, `env`) also still call it directly.

12. **Create `@lunora/platform-cloudflare`** as the default composition root: re-export the adapters from `@lunora/do`, add the `@lunora/scheduler` host, and run the full four-contract TCK from one place (today's Cloudflare run reports `SchedulerHost` as a gap because `@lunora/do` has no scheduler to offer).

13. **Build `@lunora/platform-aws`** (plan 115) as the first non-Cloudflare target.

14. **Update `lunorash` exports, the Vite plugin, and the CLI** to support host selection (`target` field, default `cloudflare`).
