...
### Known gaps
- **Workflows**: pause/restart not implemented (visulima has no equivalent); `ctx.run` fails (no Node HTTP server); `ctx.parallel` join can't interleave in one trigger; `MemoryStore` not durable across restarts
