...
`LunoraServices` interface + a `discover-service-bindings.ts`, codegen emits one
...
services-only `LunoraServices` would not serve them, a general `Env` does; and
(2) the plan's own Item 3 notes Lunora can't type a third-party worker's RPC
...
Per this plan's STOP condition ("if a Lunora `Env`/`CloudflareBindings` type
already exists or is mid-introduction, coordinate rather than emit a competing
`LunoraServices`"), the single `Env` seam is the coordinated outcome.

Item 4 remains **deferred, not built** (the `@lunora/service` typed RPC client),
as the plan directs.

- **Priority**: P2 (Service bindings are the composition primitive for multi-worker Lunora apps — they let a Lunora worker call a sibling Cloudflare Worker over RPC/`fetch`. This is the seam for the `@lunora/astro` single-worker-composition story and for splitting a large app. zeroback/Convex don't have a direct analog — this is a Cloudflare-native lead, but it's plumbing, not a headline feature.)
...
- **Risk**: LOW–MEDIUM (LOW for config/validation; MEDIUM if we generate typed RPC stubs, since `WorkerEntrypoint` types cross worker boundaries and Lunora can't fully type a third-party service's RPC surface.)
- **Depends on**: none
...
onto `env`. There is no per-request facade to wrap the way `@lunora/storage`
...
auto-provision** (the `service` name points at an external worker Lunora can't
discover — like R2's bucket name, it's user-supplied), surface a hint instead,
and (3) optionally emit a typed `env` augmentation. A thin `@lunora/service`
package with a typed RPC-client builder is a **defer-to-later** stretch (Item 4),
worth it only once multi-worker Lunora composition has real users.
...
undefined>`). `lunora dev --remote` already proxies a service binding to its
...
- `packages/config/src/wrangler-validator.ts` — `WranglerConfig` (65-83) has no
  ...
  Lunora-side check (each entry needs a non-empty `binding` + `service`) is cheap
  polish (Item 1).
- `packages/config/src/infer-bindings.ts` — no service capability. A service
  binding's `service` target is an external worker name; Lunora has no signal in
...
- `packages/codegen/src/emit.ts` — service bindings don't map to a `ctx.*`
  ...
  _where_ a Lunora `Env` type would live (see Item 3 — this is the real design
...
- [x] Item 2: Inference hint for service bindings (no auto-write) — shipped as option (a) (config-only, no scan)
    - **Decide the source signal.** Unlike `@lunora/auth`/`storage`, there is no import that says "this app uses a service binding" — service usage is `env.<NAME>.fetch(...)` / `env.<NAME>.<rpc>()`, and the binding name is arbitrary. Two honest options: (a) **skip inference entirely** (service bindings are 100% user-authored config; Lunora has nothing to infer), or (b) scan `lunora/` for `env.<UPPER_SNAKE>.fetch(` patterns and, for any such name not present in `services`, emit a hint. **Recommend (a) for this PR** — option (b) is heuristic and noisy. Document the decision in the plan-status update.
...
- [x] Item 3: Codegen-typed `env` access (the real design item) — shipped as a broad `CloudflareBindings`/`Env` seam, not a services-only `LunoraServices` (see Status banner)
    - **Problem**: codegen passes `env` around as `Record<string, unknown>` (`emit.ts:1265`, `1957-1959`, etc.), so `env.MY_SERVICE` is untyped. Typing service-binding access end-to-end needs a generated `Env`/`CloudflareBindings` interface — which Lunora does not currently emit.
    - **Scope decision**: emitting a full `Env` type is a larger cross-cutting change (it would also want to cover KV/R2/D1/AI bindings). For _this_ plan, do the **minimal** version: emit (in `_generated/server.ts`, near the `ActionCtx` block at `emit.ts:852-868`) a `LunoraServices` interface — one optional `Fetcher`-typed property per `services[].binding` discovered from `wrangler.jsonc` — and a helper `services(env)` (or a typed `ctx.env`/`ctx.services` accessor on `ActionCtx`) that returns `env` narrowed to `LunoraServices`. Read the binding names from the parsed wrangler config via a new `discover-service-bindings.ts` in `packages/codegen/src/` (mirror `discover-containers.ts`/`discover-workflows.ts`, which already read structured config).
    - `entrypoint`-typed RPC stubs (typing the _methods_ of a `WorkerEntrypoint`) are **out of scope** — Lunora can't see a third-party worker's RPC surface; type these as `Service<unknown>` / `Fetcher` and let the user cast. Document this limit in the generated comment.
    ...
    - **Test**: a codegen fixture with a `services: [{ binding: "PRICING", service: "pricing-worker" }]` wrangler config emits a `LunoraServices` with a `PRICING` property; an empty/absent `services` emits nothing. Plain-Node golden assertion — no workerd.

- [ ] Item 4 (DEFERRED — stretch): `@lunora/service` typed RPC-client package
    - Only pursue once multi-worker Lunora composition (the `@lunora/astro` single-worker story, or a deliberately split app) has a concrete user. Shape: a thin `createServiceClient<TEntrypoint>(env.<BINDING>)` that wraps a `WorkerEntrypoint` stub with typed method passthrough + a `fetch` helper, packaged like `@lunora/storage` (src/index.ts named-only exports, src/create-service-client.ts, src/types.ts, `__tests__/` over a structural `Fetcher`/`Service` fake, package.json conditional exports, project.json `type:package`+`category:add-on`, packem/vitest/.releaserc/FSL license).
    - **Test (when built)**: plain-Node fake `Service`/`Fetcher` object asserts `fetch` passthrough + RPC-method forwarding. No workerd in the unit suite; any real-binding workerd test is CI-only behind `LUNORA_WORKERD_TESTS=1`.
...
pnpm --filter "@lunora/config" run test                  # validateServices + (optional) inference hint
pnpm --filter "@lunora/config" run lint:types
pnpm --filter "@lunora/codegen..." run build
pnpm --filter "@lunora/codegen" run test                 # LunoraServices emit golden
pnpm --filter "@lunora/codegen" run lint:types
```

- All tests are plain-Node (config parsing, codegen string emit). No workerd in scope for Items 1-3. Item 4's real worker-to-worker RPC can only be exercised in a workerd pool — CI-only, gated behind `LUNORA_WORKERD_TESTS=1`; it will time out in this sandbox.
...
- Item 3 reveals that a Lunora `Env`/`CloudflareBindings` type already exists or is mid-introduction in another plan — coordinate rather than emit a competing `LunoraServices` type; report the overlap.
- Emitting `LunoraServices` requires reading wrangler config from a codegen path that doesn't currently parse it — confirm `discover-containers.ts`/`discover-workflows.ts` already have a wrangler-read seam to reuse; if not, stop and scope the config-read plumbing separately.
