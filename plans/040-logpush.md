...
- [x] **Item 2 (docs only): tie the flag to the Studio logs story.**
    - Short docs note (where observability/logs are documented): `"logpush": true` enables Cloudflare Logpush; the actual sink (R2 / HTTP / Splunk / Datadog / S3) is a **Logpush job** created in the CF dashboard or via API — Cirrus does not manage jobs. Point at the Studio Logs → Log Drains panel (`log-drains-panel.tsx`) which already shows the snippet and deep-links to the dashboard.
    - State the boundary explicitly: Cirrus validates the flag; job lifecycle stays in Cloudflare.
...
- If you start building Logpush-_job_ CRUD (create/list/delete jobs via the CF API) inside Cirrus, STOP — job lifecycle is dashboard/API territory and explicitly out of scope.
