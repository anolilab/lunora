...
- **Risk**: LOW (a leaf outbound binding; no DO/runtime/state surface; cert material lives in Cloudflare, never in Cirrus code).
- **Depends on**: none
...
via `wrangler mtls-certificate upload` and referenced by `certificate_id`; Cirrus
...
pnpm --filter "@cirrus/config" run test                  # validateMtlsCertificates
...
- The task is reinterpreted as "manage/upload/rotate certificates" or "store cert material in Cirrus" — **stop and report**. Cirrus must never touch cert/key bytes; that stays in Cloudflare's mTLS cert store.
