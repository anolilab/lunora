...
- **Priority**: P2 (KV closes a real Convex/zeroback parity edge — ephemeral caches, feature flags, session blobs — but Cirrus already has DO-SQLite for durable state, so KV is a convenience layer, not a load-bearing gap.)
...
Build it as a **thin `@cirrus/kv` package** mirroring `@cirrus/storage`: a typed
...
ReadonlyArray<BindingEntry | null | undefined>`. So `cirrus dev --remote`
...
- `packages/config/src/wrangler-validator.ts` — `WranglerConfig` (lines 65-83)
  ...
  required — wrangler validates the binding shape itself — but a Cirrus-side
...
- `packages/config/src/infer-bindings.ts` — no KV capability. `Capabilities`
  ...
  `@cirrus/storage` (lines 499-501) — it can only ever be a **hint**, never an
  auto-write.
...
- `packages/codegen/src/discover-feature-usage.ts:48-56` — `PROBES` is where a
  `kv: { contextProperty: "kv", moduleSpecifier: "@cirrus/kv" }` probe is added
  so the studio nav + codegen gating sees KV usage.
- No `packages/kv/` exists. `@cirrus/storage` is the package template (`src/index.ts`,
...
- [x] Item 1: Scaffold the `@cirrus/kv` package
    - Run `vis generate cirrus-package --name=kv --description='Typed Workers KV namespaces for Cirrus: JSON helpers and ctx.kv'` to create `packages/kv/`, then conform it to the `@cirrus/storage` shape.
    - `packages/kv/src/types.ts` — define `KVNamespaceLike` (the structural subset of Cloudflare's `KVNamespace`: `get`/`getWithMetadata`/`put`/`delete`/`list`, with the `type`/`cacheTtl`/`expiration`/`expirationTtl`/`metadata` options). Do NOT import `@cloudflare/workers-types` at runtime — declare a structural `*Like` interface so the package stays runtime-agnostic and unit-testable in plain Node (mirrors `R2BucketLike` in `packages/storage/src/types.ts`). Also export `CirrusKvOptions`, `KvPutOptions`, `KvListOptions`, `KvListResult<T>`, and the public `Kv` facade interface.
    - `packages/kv/src/create-kv.ts` — `createKv(namespace: KVNamespaceLike, options?: CirrusKvOptions): Kv`. Methods: `get<T>(key)` / `getRaw(key)` / `getWithMetadata<T, M>(key)` (JSON-parse by default, with a typed `text`/`arrayBuffer`/`stream` escape hatch), `put<T>(key, value, opts?)` (JSON-stringify by default; pass `expirationTtl`/`expiration`/`metadata` through), `delete(key)`, `list(opts?)`. Add an optional `keyPrefix` in `CirrusKvOptions` and a `scopeKey(prefix, key)` helper mirroring `scopeKey` exported from `@cirrus/storage` (`packages/storage/src/index.ts:3`) for multi-tenant key namespacing.
...
- [x] Item 2: Codegen-typed `ctx.kv` on `ActionCtx`
    ...
    - **Test**: extend the codegen golden/emit tests under `packages/codegen/__tests__/` — assert that with the `kv` flag set, the generated `server.ts` contains the `ctx.kv` field on `ActionCtx` and the `@cirrus/kv` import, and with it unset, neither appears. Follow the existing `ai`/`payments` emit assertions.

- [x] Item 3: Wire KV into feature discovery
    - `packages/codegen/src/discover-feature-usage.ts` — add `kv: boolean` to `FeatureUsage` (lines 23-38) and `kv: { contextProperty: "kv", moduleSpecifier: "@cirrus/kv" }` to `PROBES` (lines 48-56). This flips when a `cirrus/` source imports `@cirrus/kv` or reads `ctx.kv`. Plumb the flag through to `emitKvFragments` in `run-codegen.ts`/`emit.ts` exactly as `ai` is plumbed.
    - If the studio nav gating consumes `FeatureUsage` (`buildStudioFeatures`), decide whether KV gets a nav page — likely **not** in this PR (no studio KV browser yet); just ensure the new flag doesn't break the existing `StudioFeaturesResult` shape. If `StudioFeaturesResult` (`@cirrus/do`) is a closed type, either extend it or keep `kv` codegen-only and out of the studio features object.
    - **Test**: a discovery test fixture with a `cirrus/` file importing `@cirrus/kv` (and one reading `ctx.kv`) asserts `usage.kv === true`; a fixture with neither asserts `false`.

- [x] Item 4: Config inference hint (no auto-write)
    - `packages/config/src/infer-bindings.ts` — add `usesKv: boolean` to `Capabilities` (lines 141-148), `NO_CAPABILITIES` (150), `mergeCapabilities` (152-161), `capabilityForImportSource` (`@cirrus/kv` → `{ ...NO_CAPABILITIES, usesKv: true }`, lines 164-186), `regexCapabilities`/`IMPORT_KV_PATTERN` (211-221), and `InferredBindings` (117-138). Surface a hint in `describeSignals` (456-508) like the storage hint (499-501): "@cirrus/kv is imported; add a kv_namespaces binding (binding names are user-defined) and pass env.<BINDING> to createKv()." <!-- gitleaks:allow -->
    ...
    - **Test**: extend `packages/config/__tests__/` (the infer-bindings / reconcile-bindings suites) — a project importing `@cirrus/kv` with no `kv_namespaces` produces the hint; one with a `kv_namespaces` entry already present is silent; reconcile never writes a `kv_namespaces` block.
...
pnpm --filter "@cirrus/kv..." run build                  # build pkg + its deps
pnpm --filter "@cirrus/kv" run test                      # plain-Node fakes — must pass locally
pnpm --filter "@cirrus/kv" run lint:types
pnpm --filter "@cirrus/kv" run lint:eslint
pnpm --filter "@cirrus/codegen..." run build
pnpm --filter "@cirrus/codegen" run test                 # ctx.kv emit + discovery
pnpm --filter "@cirrus/config..." run build
pnpm --filter "@cirrus/config" run test                  # infer/reconcile KV hint
```

- All `@cirrus/kv` unit tests are plain-Node (Map-backed `KVNamespaceLike` fake); no workerd. Any workerd-pool smoke test (real KV binding via Miniflare) MUST be gated behind `CIRRUS_WORKERD_TESTS=1` (see `packages/storage/vitest.config.ts:37`) and is **CI-only** — it will time out in this sandbox.
...
- `@cirrus/storage`'s package shape (exports/scripts/devDeps) differs from what's documented here — match the live storage package, not this plan's snapshot.
- `StudioFeaturesResult` (`@cirrus/do`) cannot accept a new `kv` field without a breaking change to the studio — keep KV codegen-only (skip studio nav) and report.
