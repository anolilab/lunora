...
`withSearchIndex` to mask the *output* rows but never checks whether the range /
...
The `.filter` path masks the row *before* the predicate
(`middleware.ts:261-265`) — that pattern (mask-then-observe) is not available for
an index range because the range constrains *which rows are fetched*, before any
...
`indexName` → its columns directly. The range/search is a *builder callback*
...
any is masked, and only then pass the *real* builder through to
...
table's *non-masked* columns. Only choose B if A proves infeasible, and record
the tradeoff.
