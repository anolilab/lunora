...
W9–W11 change how `vectorStore` maps:
...
| `RagVectorStore` — pgvector (W10)   | emulated     | emulated    | Built over Hyperdrive / `@lunora/sql-store`, not a first-class provider product — `emulated` on both, per the `capabilities.ts:118-127` rule.                |
| `RagVectorStore` — DO SQLite (W11)  | emulated     | emulated    | Cloudflare: DO SQLite. Node: `@lunora/platform-node`'s better-sqlite3 shard state. **This is what flips `vectorStore` off `unsupported` for Node** (`:272`). |
| `RagLexicalStore` — SQLite FTS (W8) | emulated     | emulated    | On `@lunora/search-core`; no provider product on either host.                                                                                                |
