...
| 110  | [Spike] `@lunora/next` composition adapter + `templates/next`    | feature/spike | P2  | L      | MED      | **SPIKE DONE** — PR [#96](https://github.com/anolilab/lunora/pull/96), design doc + prototype (both removed on ship; record in git history). Recommendation: Next is class-B; reuse the shipped `withFrameworkWorker` at the OpenNext **custom-worker boundary** (WebSocket MUST mount there — a Next Route Handler strips the `101` upgrade). Open Q: OpenNext version pinning + a validator; remove the `init` no-op only when `templates/next` lands. No STOP.                                                                                                                                                                                                                                                                                                                                                                                                              |
| 111  | [Spike] RAG helper coupling `ctx.ai.embed` + `ctx.vectors`       | feature/spike | P2  | M      | LOW–MED  | **SPIKE DONE** — PR [#96](https://github.com/anolilab/lunora/pull/96), design doc + prototype (ran over the real `createVectors`; both removed on ship). Recommendation: thin `defineRag(config)(ctx)` as a `@lunora/ai/rag` subpath (not a new binding/codegen `ctx.rag`), no public-surface change, respects topK≤20-with-metadata + namespace isolation. Open Q: where chunk text lives (metadata vs DO SQLite table); `LunoraVectors` lacks delete-by-source. No STOP.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 113  | [Spike] Durable AI-agent primitive (`defineAgent`)               | feature/spike | P2  | XL     | MED–HIGH | **SPIKE DONE** — PR [#96](https://github.com/anolilab/lunora/pull/96), design doc (removed on ship) + PoC (asserts thread ordering AND resume-doesn't-re-run-a-completed-tool; step-name is the idempotency key → no `@lunora/workflow` change). **Recommendation: document-first + an opt-in `@lunora/agent` add-on**, not a core primitive — the build-vs-document tension with "scale invisibly" is surfaced as the top open question for the maintainer to ratify. No STOP.                                                                                                                                                                                                                                                                                                                                                                                                |
...
collab CRDT (demand-gated) · **306** pluggable queue drivers (TODO, P2) ·
**312** the two gaps that leave a `v.bigint()` column unaggregatable (TODO, P2 —
its §4 decisions are deliberately unmade pending a measurement) · **314** an
...
rung) · **334** mutation determinism (global-swap implementation proven unsound
...
**312** (bigint aggregate gaps) DONE & REMOVED: verified against the live code —
`isLiveForCompanion` is in `query-args.ts` and wired through
`ctx-db-companions.ts` and `ctx-db-backfill.ts`. Its two spun-out follow-ups
(**314**, **315**) keep their own files. Record in git history.
...
which shipped inside #365 rather than trailing it; and **265** / **302**, both
now DONE & REMOVED above. Their record is those rows plus git history.
...
| Plan | Title                                                              | Findings            | Pkg                    | Pri | Effort | Risk    | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---- | ------------------------------------------------------------------ | ------------------- | ---------------------- | --- | ------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 265  | Make `v.bigint()`/`v.bytes()` round-trip on the DO row store       | DO-01, SQL-01       | shard-engine/sql-store | P1  | M      | MED     | ✅ **DONE & REMOVED — shipped in #365** (`bb398f461`, `59b09aed8`, `1ee5fab82`). Took four attempts, and the record is the point: `ab0afaf00` made `v.bigint()` storable and thereby **unqueryable**; a `Number()` projection then returned rows that were **not equal** above 2^53 and made a `unique` index reject two distinct bigints; a reserved key silently destroyed a user field of the same name. What shipped stores a fixed-width sign-aware order-preserving decimal key, proven by a property test over ~44.5k pairs in JS and again under real SQLite `ORDER BY`. **The exact-int64 alternative is a trap worth remembering**: SQLite really does hold int64 through `json_extract`, but reading such a row back throws `RangeError` out of the driver, so the row becomes unreadable. The backfill (plan 311) shipped in the same PR, so no data was left behind. Cost, tracked as **312**: padded text cannot be summed, so `SUM` on the scan path refuses a bigint field rather than return the ~1.5e40 that falls out of coercing it |
...
| Plan | Title                                                         | Category     | Pkg      | Pri | Effort | Risk | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | ------------------------------------------------------------- | ------------ | -------- | --- | ------ | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 302  | Repo lint gate reports success for ten projects it never runs | ci/tech-debt | repo/vis | P1  | M–L    | LOW  | ✅ **DONE & REMOVED — all ten projects now lint for real.** Phase 1 with plan 290 (`596dd3c3d`), phases 2–5 as `a4c6522eb` (#343), phase 6 as #363, phases 7–10 as #364. The guard (`scripts/check-project-json-targets.js`) runs as the first step of the Lint workflow's eslint job — **not** the root `postinstall`, where one failure turns all ~20 CI jobs red in their setup step — and `.github/workflows/lint.yml` plus `.github/file-filters.yml` are themselves in `frontend_lintable`, so the gate cannot be switched off by editing the gate. **The merge-order dependency fired**: #363 made `svelte` clean while #364's allowlist still named it, and the check fails on a stale entry by design, so `alpha` went red until #368 emptied the map. Real counts were 38/117/303/24, not the plan's — it had measured on a cold tree, where missing dependency `dist/` inflates the figures ~4–12x                                                                                                                                       |
| 303  | Wire-codec display leftovers from plan 300                    | bug          | studio   | P3  | S      | LOW  | ✅ **DONE (2026-08-09) — one half was already shipped, the other was decided against.** **(b) the dropped `__proto__` field is fixed** — `f8b05d09c` (#342) gave `encodeWire`'s plain-object branch the same `defineProperty` treatment the decode side always had, with a round-trip test in `packages/client/__tests__/wire-codec.test.ts`; this row simply predated it. **(a) the SQL console will NOT decode.** It shows the DATABASE; the studio data browser shows the MODEL and decodes there (plan 300). Three reasons the split stays, now recorded on `runReadonlySql`'s docstring so it is not reopened: the console is the only place an operator can see how a value is physically stored, which is exactly what is needed when a `filter` or `ORDER BY` does not match; decoding would disagree with the `json_extract(__doc__, …)` the same operator types next, since that reads the stored form; and the studio's saved-query template selects `__doc__` explicitly, so decoding would change what an existing saved query returns |
...
| 311  | Rewrite the rows `v.bigint()`/`v.bytes()` left unqueryable                | data/migration  | shard-engine/cli    | P1  | M      | MED  | ✅ **DONE & REMOVED — shipped in #365** (`bb398f461`) — it ships with the fix it completes rather than trailing it, so the two land or revert together. Rows written between `ab0afaf00` (2026-08-03) and #365 hold the bigint tagged in place: they read back correctly but `json_extract` never matches, so `filter`/`withIndex`/`ORDER BY`/`SUM` miss them. #365 heals a row on any `patch`/`replace` and migrates nothing on its own, so the exposure is **the rows nobody rewrites** — for `paymentSessions` that is exactly the settled sessions never touched again. No new machinery was written: `runDataMigration` already owns batching, resumption, the cursor, claim/heartbeat and `dryRun`, so `reprojection-backfill.ts` (~175 lines) contributes only the affected-table derivation, a stored-text detector and an identity transform. The detector is `json_extract(__doc__, '$.f[0]')` — it resolves for a legacy array, `NULL` for a current row whose field is a JSON string — with **bound** paths and a sentinel derived from `encodeWire(0n)[0]` rather than hand-copied. A test pins the STOP condition: **a row whose only tagged value is nested is not matched**, because nested values stay wire-tagged under the current projection and a blob-scanning detector would rewrite every table forever while looking like it worked. Decided during execution: **not automatic on deploy** (every rewrite pokes every subscriber on the row, so an all-legacy shard is a deploy-time thundering herd on a path nobody asked for), and `countLegacyRows` is a cheap completeness check exported for `lunora doctor` but **not wired** — that needs a per-shard fan-out decision. **Follow-up CLOSED (2026-08-09) — it was already done.** `lunora doctor` is entirely local (filesystem + `wrangler.jsonc`, no deployment call), so it cannot know a shard's row counts; and nothing new was needed anyway. `buildReprojectionMigration`'s transform returns `undefined` for a row already in the current projection, and the runner counts `changed` only when a transform returns a document — so `lunora migrate up __lunora_reproject__<table> --dry-run` **already reports the exact legacy-row count per shard**. A `legacyRows` field was added to the `migrationStatus` admin RPC before that was noticed, and reverted in full: both thermo reviews independently found it omitted `reprojectableFields`' `.global()` exclusion, so `migrate status __lunora_reproject__<globalTable>` would hit `no such table` where the old behaviour returned cleanly — plus a live subscription re-running the unindexed COUNT per write flush, and `__lunora_reproject__constructor` throwing off a prototype member. Five defects in code that did not need to exist. The real gap was documentation: `concepts/migrations.mdx` had never mentioned the reserved migration, and now documents it with `--dry-run`. Plan file removed per the convention above; the record is this row plus `bb398f461`                                                                                                                                                                                                                       |
| 312  | Close the two gaps that leave a `v.bigint()` column unaggregatable        | bug/design      | shard-engine        | P2  | M      | MED  | ✅ **DONE (2026-08-09)** — gap 1 fixed (the aggregate companion tallies LIVE rows only, so a `.softDelete()` table can reach it instead of falling to a scan that refuses a projected field); gap 2 kept loud and unchanged. Two adversarial-review rounds found three further defects, two of them silent: `recomputeExtreme` reduced the padded sort KEY in SQL and stored ~1e39 as a group's extreme; the same guard was keyed off the declared type, missing `v.any()` columns; and the liveness gate let a dead-on-arrival row bypass the 2^53 companion bound, so `restore()` then threw forever. All three fixed and pinned. **Found and deliberately NOT fixed:** `assertReducibleBySql` is narrow the same way, so an aggregate over a `v.any()` column holding bigints is neither refused nor answered — needs its own plan. Execution record and both review rounds in §10–§12 of the plan file; read that, not this cell. Spun out: [315](315-durable-companion-backfill-marker.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 313  | Put the long-term backup tier on object storage instead of someone's disk | data/durability | cli/storage         | P2  | M      | MED  | ✅ **DONE & REMOVED — shipped in #375 and #389.** `lunora backup` had two tiers and its own docblock named them: `pitr` drives the platform's change log for the last 30 days **in place**, and `create`/`restore` is the portable tier for everything older — but that second one wrote NDJSON through `node:fs` to a local directory, so the durability of the tier that exists for exactly the case the 30-day window cannot cover was whatever disk the CLI ran on. #375 made object storage a destination (a `BackupDestination` seam, `restore --verify`, an admin read route so a bucket restore needs no URL signing) and #389 closed WS4: **the scheduled backup no longer deletes at all**, `lunora backup prune` is the only thing that removes a backup, and `lunora backup retention` previews what it would take. One `selectStaleBackups` feeds all three consumers. **Six review rounds**, and the pattern is the record worth keeping: every defect after the first round was in the _remediation_, and the recurring shape was a claim about a second party's behaviour never checked against the second party — a guard documented as preventing an OOM it could not prevent, an upload documented as checksum-verified above the size where verification stops, remediation text whose suggested call does not compile, and a prune whose confirmation was not binding because the POST carried no selection and the server re-selected. Nine such comments in all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 314  | Stop shipping an English NLP library in every Worker                      | perf/deps       | observability       | P2  | S–M    | MED  | TODO — every Lunora Worker ships **`compromise`, an English NLP library**, because `@visulima/redact` hard-depends on it (`"compromise": "^14.15.1"`, verified in the installed package) and `observability/src/request-log.ts` imports redact for `redactArgs`, which `database-telemetry.ts` and `context-telemetry.ts` both call — so it is reachable from any app that logs. Wave 20 measured it at ~606 KiB raw, **larger than any first-party package**, against a `templates/standalone` worker of 1684.9 KiB raw. Nobody asked for NLP; they asked for their logs not to leak secrets. The measurement tooling already exists (`bundle-meta.json` carries per-module `bytesInOutput`), and the STOP condition is that redaction must not get weaker — this is a security path, so characterisation tests come first. See [314-compromise-in-every-worker.md](314-compromise-in-every-worker.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 315  | Make the aggregate-companion backfill marker durable                      | perf            | shard-engine        | P2  | M      | MED  | TODO — spun out of 312. `ensureBackfilledIndex` is an unconditional full-table rescan + companion rewrite memoised only in a `Set` on the ctx-db instance, and codegen builds one **per dispatch** — so the mechanism whose purpose is to answer without scanning pays a full scan before it answers, once per request, for every table with an `aggregateIndex`. Pre-existing on `alpha`; found while sizing 312's reader gate. Landing it deletes that gate and its note. The risk is the trade it makes: today's design is wasteful but self-correcting, and a durable marker must invalidate on an index-definition change or the companion answers a stale grouping forever. See [315-durable-companion-backfill-marker.md](315-durable-companion-backfill-marker.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 363  | Close nine gaps in the Studio's data/SQL surface                          | dx/studio       | studio/codegen      | P2  | M–L    | LOW  | ✅ **DONE — W1–W4, W6–W10 all shipped** (W5 spun out to 364). Two of the nine had a wrong premise the plan itself records: multi-statement rejection is the console's ENFORCEMENT point so W4 splits above the gate rather than relaxing it, and the enum dropdown needed a codegen change because `emit.ts` dropped a union's members. Four plan corrections found by building it — there are four declarations of the ColumnMeta shape not three; `useRunSql` is the dashboard's hook so a list there would have forced `results[0]` on every tile; flipping the history-persistence toggle ON would have DISCARDED the history it was asked to keep; and a text dashboard tile needs its own component because a conditional hook is not available. Two defects found on the way and **since fixed**: `classifyStatement`'s batch check scanned raw text, so a `;` inside a literal, a `--`/`/* */` comment, or a quoted identifier read as a statement boundary and FOUR classes of legal read-only query were refused outright (fixed with a same-length masked scan that fails closed on anything unclosed, and deliberately NOT extended to the keyword check, whose literal-rejection is documented as an accepted trade); and the studio's request-loop warning turned out to be the detector's bug — a probe showed each admin read fires exactly once per mount, while the detector's module-level bucket cannot tell one component looping from 25 mounting, and its own comment justified the threshold with an in-flight-dedupe claim that never applies to sequential mounts. Originally — headline: **a Studio dashboard cannot draw anything but a bar chart.** `dashboards-panel.tsx:126` renders `<SqlResultChart result={result} />` with no `axes`, and `result-chart.tsx:120` takes the `"bar"` constant arm whenever `axes` is absent — so the component's line/area paths and the assistant's `aiChartConfig` inference are both unreachable from the one surface that exists to chart things. Eight further gaps in the same audit: single-block dashboards, a row editor that infers widgets from the runtime value rather than the schema it already has (`row-form.tsx:26`), one result set per run, a stateless three-RPC assistant, no paste-into-grid, plaintext `localStorage` SQL history, no media preview for `v.storage()` cells, and hardcoded shortcuts with no preferences surface. **Six of the nine are wiring, not building** — the capability exists elsewhere in the repo and the consumer does not reach for it. Two premises need correcting before anyone starts: multi-statement rejection is _deliberate_ (`shared/sql-readonly.ts:33,148`, the enforcement point for the whole console) so W4 must split above the gate, never relax it; and the enum dropdown needs a codegen change because `emit.ts:2759` drops a union's members. STOP conditions on both. W5 (a conversational assistant) must not await a model on the ShardDO admin dispatch — `sql-assistant.ts:70-78` documents the 15 s deadline that exists there precisely because it is single-threaded — and should produce its own plan before any code. See [363-studio-data-surface-parity.md](363-studio-data-surface-parity.md) |
| 364  | A conversational assistant for the Studio, off the DO admin dispatch      | dx/studio       | runtime/studio      | P3  | M      | MED  | TODO — spun out of 363 W5, which required a plan before code. Headline: **the transport question 363 left open is already answered in the tree, and the answer is neither option it named.** `runtime/src/auth-audit-rpc.ts` shows an `__lunora_admin__:*` op INTERCEPTED AND SERVED AT THE WORKER rather than forwarded to a shard — admin-gated first, capability injected as a closure dep, `*_NOT_CONFIGURED` degrade — and it has shipped twice (`getAuthAuditLog`, `listPushSubscriptions`). That satisfies 363's STOP condition by choosing where the op is intercepted, with no Workflows binding, no add-on install, and no generated agent class. Re-sizes the work from L to ~M: the inference primitive, the untrusted fence, the retry policy, the degrade contract and the admin gate all already exist and are all reusable at the Worker. What is genuinely new is multi-turn state and tool dispatch — and both start smaller than "an agent": history is client-held and re-sent (a console scratchpad nobody resumes, and persisting it would be one more store of raw statements outliving the browser), every prior turn is fenced as untrusted because a re-sent transcript is caller-supplied by definition, and tools are read-only ops that are ALREADY gated so the security review stays "does the existing gate hold". Two STOPs: the op must never end up in `aiAdminHandlers()`, and a write tool is a different plan — that is the point where `@lunora/agent`'s approvals stop being over-engineering. See [364-studio-conversational-assistant.md](364-studio-conversational-assistant.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

Worth reading before picking **306** up: it only pays off if a non-Cloudflare
target becomes real, and `@lunora/platform-node` is still experimental with no
`lunora dev --target node`. **311 and 312** both fall out of the plan-265 work
