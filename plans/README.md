...
Lunora differs — and, after a second pass, don't copy its preview architecture
either (see the status cell). §9 of the plan records every open question as an
answered decision with the alternative it beat; what remains open there is four
measurements, each with its decision already attached to the outcome.

| Plan | Title                                                                 | Category | Pkg                         | P   | Effort | Conf | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---- | --------------------------------------------------------------------- | -------- | --------------------------- | --- | ------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 335  | `apps/builder` ("Lander") — AI app builder on Lunora + TanStack Start | product  | apps/builder, new `sandbox` | P2  | L      | MED  | TODO — **decisions settled (D1–D21), ready to execute.** The agent layer is mostly composition of shipped packages (`@lunora/agent`, `@lunora/mcp`, `lunora verify/eval`, `packages/cli/skills`, `registry/`, `templates/tanstack-start-react`); the genuinely new engineering is `@lunora/sandbox`, per-user deploy/ownership, and the workbench UI. **D2 is the load-bearing decision: one execution tier — the preview is `lunora dev` in a Cloudflare Sandbox, not a Worker-Loader/DO-facet bundle.** An earlier draft chose the VibeSDK-style two-tier model and left it to a spike; it was reversed on **fidelity** (a facet preview and a namespace-backed `ShardDO` deploy differ in lifecycle, hibernation and alarms, so preview and deploy would disagree) and on **duplication** (a bundle path re-implements the worker composition `@lunora/vite` already owns). The facet WebSocket-hibernation unknown — workerd [#6702](https://github.com/cloudflare/workerd/issues/6702) treats facets as separate DOs for I/O — is corroborating, not deciding. **§5.0 audits extend-vs-app-local:** the framework needs exactly three package changes — `@lunora/container` gains a `/sandbox` subpath **and a first-class `exec`** (closing the POST-to-`/exec` hack `containerTool` documents), and `lunora verify --format json` gains structured diagnostics the fix loop can anchor edits on; everything else is app code (D22–D24). The audit also found three capabilities already shipped (`skillFromMarkdown`, `tokenBudget`, container egress control), shrinking W4/W7/W10. Because E2 extends `ctx.containers`, §6 parity **is** applicable — one matrix row, landing in the same phase. Phase 0 now **measures** the one accepted cost (cold session start ≤ ~15 s) rather than choosing an architecture. See [335-ai-app-builder.md](335-ai-app-builder.md) |
