...
| Plan | Title                                                              | Findings            | Pkg                    | Pri | Effort | Risk    | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ------------------------------------------------------------------ | ------------------- | ---------------------- | --- | ------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 265  | Make `v.bigint()`/`v.bytes()` round-trip on the DO row store       | DO-01, SQL-01       | shard-engine/sql-store | P1  | M      | MED     | ⚠️ **SHIPPED WITH THE BLOCKERS UNFIXED — remediated in #365, ready for review. The first two rounds each shipped a fresh silent-wrong-answer bug — round two's `Number()` projection returned rows that are **not equal** above 2^53 and made a `unique` index reject two distinct bigints, trading false negatives for false positives. Round three stores a fixed-width order-preserving decimal key instead: exact `=`, numeric `ORDER BY`, writes past the 39-digit width refused rather than silently mis-sorted. The cost is `SUM` on the scan path, which now refuses a `v.bigint()` field rather than return the 1.5e40 that falls out of coercing padded text; `aggregateIndex` is the supported way to sum one. A BACKFILL IS REQUIRED, see the end of this cell.** The row previously read "BLOCKED — DO NOT MERGE"; that was wrong. The write side **did land on `alpha`** as `ab0afaf00` (#338) — `encodeDocJson`/`decodeDocJson` in `shard-engine/src/do-sql.ts`, called from six `ctx-db.ts` sites — but `serialize-sql.ts` and `estimate-bytes.ts` were **not in that commit**, so the thermos §11 findings are live defects on `alpha` today, not hypotheticals about a branch. Still live: **(a) `v.bigint()` is silently unqueryable** — storage holds `["$lunora.wire$","bigint","10"]` while `serializeSqlValue` (`serialize-sql.ts:21-23`) still renders the comparison as `"10"`, so `json_extract` never matches and `filter`/`withIndex`/`aggregate` return empty/0 on the `@lunora/payment` **money path**; **(b) CDC egress does not re-encode** (`ctx-db-cdc.ts:105` decodes, `shard-do.ts:2335` returns it raw), so bytes reach consumers as `{}`; **(c)** byte-identity is false for array-position `undefined`, `NaN`, `Infinity` and `Date`, and `patch` rewrites whole documents so the first patch silently migrates a legacy row. **Corrected since the review:** `estimateBytes` does _not_ throw or charge the 32 MiB cap — it returns `undefined` and `transaction-headroom.ts:100-110` raises `BAD_REQUEST` "not JSON-serializable", so with 270 landed (`d5721334a`) a bigint insert **fails loudly at the door**. The M1 introspect item is **resolved by decision**, not outstanding (`introspect.ts:842-862` documents why the admin path deliberately does not decode). The review missed all of this because the added test declares `indexes: []` and never filters or aggregates. See §11. **The remediation** projects top-level bigint/bytes to a SQL-comparable scalar (a JSON number / base64 string) inside `encodeDocJson` and parks the exact wire-tagged originals under a reserved `__sql__` key, with `serializeSqlValue` binding the same projection — teaching the query side the _tagged_ form was a 6-line diff but would have fixed equality only, leaving ranges, `ORDER BY`, `SUM` and `MIN`/`MAX` silently wrong, because a tagged array is still text to SQLite. Egress now re-encodes (`cdcSync` through `encodeWire`, `applyCdc` decoding it back). Two §11 items proved **not live**: `sql-store`'s `sqliteDecode` already has its bytes branch, and the introspect item is a documented decision from plan 300. A defect **introduced by the first remediation round and caught in review** is fixed in `250d0589e`: the reserved `__sql__` key silently destroyed a user field of that name (overwritten when a bigint was present, spread-and-dropped when not), defended by a docblock citing a "codegen column guard" that **does not exist** — `RESERVED_TABLE_NAMES` covers table names, `SYSTEM_INDEX_FIELDS` is just `["_creationTime","_id"]`. It now throws a typed `BAD_REQUEST` at the single write chokepoint. ⚠️ **Backfill**: rows written between `ab0afaf00` and this branch hold the bigint tagged in place — they decode fine, but `$.field` is still array text, so `filter`/`withIndex`/`ORDER BY`/`SUM` miss them. `patch`/`replace` re-project on write (pinned by test), so any rewrite migrates a row — but **a row nobody rewrites stays invisible to amount queries**, which for `paymentSessions` means exactly the completed sessions that are never touched again. A rewrite pass over affected tables is required before this reaches real payment data; the fix migrates nothing on its own |
