...
| 162  | crossTabSync — leader demotion/health, offline-queue FIFO, subscribe-relay design | bug/direction | P3 | M (CLIENT-02/03) / L design (CLIENT-01) | MED | **CLIENT-02/03 DONE, CLIENT-01 DESIGN DONE** on `advisor/162-crosstabsync`. CLIENT-02: `cross-tab.ts` `TabCoordinator` now stamps `lastHeartbeat` at claim-tie-break adoption (was left at its 0/stale default, causing immediate un-adopt/re-claim churn) and a leader stepping down on a heartbeat from a lexicographically-smaller tabId (`handleHeartbeat`/`resolveLeaderVsLeaderTieBreak`), fixing the permanent two-leader split-brain from backgrounded-tab timer throttling — kept the existing heartbeat/`BroadcastChannel` mechanism (a full Web-Locks-based election rewrite was considered but deferred per the plan's own STOP condition: Web Locks landed later than `BroadcastChannel` in some evergreen browsers already supported here). CLIENT-03: `offline-queue.ts` `hydrate()` now `unshift`s restored prior-session writes ahead of whatever was enqueued during the async hydration window instead of `push`-ing them to the end, so a boot-time write issued while offline no longer replays before an older restored write on the same document (last-writer-wins clobber). CLIENT-01: no relay protocol built (design-first per the plan) — [162-phase0-crosstabsync-design.md](162-phase0-crosstabsync-design.md) specifies a `subscribe-request`/`unsubscribe-request` follower→leader relay + leader-side refcounting + late-joiner replay, flags an **identity-scoping gap already present in the shipped broadcast path** (the relay key has no identity dimension, so two same-origin tabs signed in as different identities could cross-leak via the same registry key), and recommends gating `crossTabSync` experimental + documenting the "follower mirrors only what the leader holds" limitation rather than building the relay blind. 3 new `cross-tab.ts` tests + 1 new `offline-queue.ts` test; `@lunora/client` `lint:types` exit 0, 391/391 tests pass, ESLint 0 errors. |
