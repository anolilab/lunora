...
| Plan | Title                                                            | Category      | Pri | Effort | Risk     | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---- | ---------------------------------------------------------------- | ------------- | --- | ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 108  | `lunora add payment` registry item (+ pattern for heavy add-ons) | feature/dx    | P2  | M      | LOW      | **BLOCKED** (engine limitation — verified) — [108](108-registry-payment-item.md). The `add` engine cannot express what payment needs: (1) codegen emits a `paymentStub` that throws `"no payment configured"` unless a `config.payment(env)` thunk is hand-written in the worker entry (`emit.ts:2698`) — but the registry `merge` enum is only `create-or-skip`/`schema-extension` and `apply.ts:219,281` **deliberately excludes** `main`/entrypoint keys, so it can't inject `createShardDO({ payment })`; (2) same for mounting the webhook `httpRouter`; (3) `createDatabasePaymentStore` hardcodes bare table names but `schema-extension` auto-prefixes them. This affects **all** non-binding-backed add-ons (payment/ai/workflow/queue/scheduler/container/flags), not just payment. **Unblock = a new registry capability** (a `merge` mode / manifest field that injects a `createShardDO`/`createWorker` config block + mounts an `httpRouter` + an unprefixed inline-schema merge). Rewrite 108 as that engine-capability plan first. billing.ts uses only the public API (that STOP was clear); no partial item shipped. |
...
tests per closing-the-loop). **17 of 18 plans DONE; 108 BLOCKED.**
