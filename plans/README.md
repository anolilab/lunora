...
its §4 decisions are deliberately unmade pending a measurement) · **313** the
long-term backup tier on object storage (phases 0–1 + WS5 shipped in #375; WS4,
an explicit `prune` verb, still open) · **314** an English NLP library in every
Worker (TODO, P2).
...
| 311  | Rewrite the rows `v.bigint()`/`v.bytes()` left unqueryable                | data/migration  | shard-engine/cli    | P1  | M      | MED  | ✅ **DONE & REMOVED — shipped in #365** (`bb398f461`) — it ships with the fix it completes rather than trailing it, so the two land or revert together. Rows written between `ab0afaf00` (2026-08-03) and #365 hold the bigint tagged in place: they read back correctly but `json_extract` never matches, so `filter`/`withIndex`/`ORDER BY`/`SUM` miss them. #365 heals a row on any `patch`/`replace` and migrates nothing on its own, so the exposure is **the rows nobody rewrites** — for `paymentSessions` that is exactly the settled sessions never touched again. No new machinery was written: `runDataMigration` already owns batching, resumption, the cursor, claim/heartbeat and `dryRun`, so `reprojection-backfill.ts` (~175 lines) contributes only the affected-table derivation, a stored-text detector and an identity transform. The detector is `json_extract(__doc__, '$.f[0]')` — it resolves for a legacy array, `NULL` for a current row whose field is a JSON string — with **bound** paths and a sentinel derived from `encodeWire(0n)[0]` rather than hand-copied. A test pins the STOP condition: **a row whose only tagged value is nested is not matched**, because nested values stay wire-tagged under the current projection and a blob-scanning detector would rewrite every table forever while looking like it worked. Decided during execution: **not automatic on deploy** (every rewrite pokes every subscriber on the row, so an all-legacy shard is a deploy-time thundering herd on a path nobody asked for), and `countLegacyRows` is a cheap completeness check exported for `lunora doctor` but **not wired** — that needs a per-shard fan-out decision. **Still open from this work**: `lunora doctor` should surface `countLegacyRows` so an operator learns they have legacy rows without being told to look — the exports are there, the per-shard fan-out decision is not made. Plan file removed per the convention above; the record is this row plus `bb398f461` |
| 312  | Close the two gaps that leave a `v.bigint()` column unaggregatable        | bug/design      | shard-engine        | P2  | M      | MED  | TODO — the cost side of #365's design, both **loud** (they throw; they do not return wrong answers). **(a)** A soft-delete table can never use the aggregate companion — `aggScope` is always truthy at `ctx-db.ts:2451`, so `:2467`'s fast-path predicate always fails and every aggregate falls to the scan, which now refuses a bigint field rather than coerce padded text into ~1.5e40. Net: no aggregate over a bigint column on such a table at any magnitude. **(b)** Declaring `aggregateIndex` on a bigint column caps that column's **writes** at 2^53 (`aggregate-tally.ts:62`), because the companion's `__value__` is a REAL column — one index declaration makes the column unusable for large values, the opposite of why people reach for `v.bigint()`. Closing either by rounding is a STOP condition. See [312-bigint-aggregate-gaps.md](312-bigint-aggregate-gaps.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 313  | Put the long-term backup tier on object storage instead of someone's disk | data/durability | cli/storage         | P2  | M      | MED  | ✅ **PHASES 0–1 + WS5 SHIPPED in #375**; **WS4 deliberately deviated from** — retention still deletes inside the cron run, narrowed by a cron-expression marker read off the object listing rather than made explicit, and §10 of the plan records why rather than reinterpreting the plan to match the code. Three thermo rounds, each finding something the previous declared clean, and **every defect in the remediation rather than the original work**; the recurring shape was a claim about a second party's behaviour never checked against the second party — a guard documented as preventing an OOM it could not prevent (the export fan-out resolves every shard's rows before the first byte is encoded), an upload documented as checksum-verified above the size where the verified route stops being used, and remediation text whose suggested call does not compile. Original finding: `lunora backup` already shipped two tiers and names them in its own docblock: `pitr` drives the platform's DO change log for the last 30 days **in place**, and `create`/`restore` is the off-platform, portable, **>30-day** tier. But that second tier writes NDJSON through `node:fs/promises` to `--dir` (default `.lunora-backups`), so the durability of the tier that exists for exactly the case the 30-day window cannot cover is whatever disk the CLI ran on — and the documented automation is "schedule `create` (CI cron…)", i.e. an external machine in the durability path. The fix is a **destination, not a format**: the snapshot, manifest, export/import endpoints and R2 bindings all exist, and plan 304's checksum-verified admin upload route is the precedent for getting bytes into R2 without minting new credentials. Acceptance is a `v.bigint()` + `v.bytes()` round-trip through the new destination, because plan 265 showed `exportShard` could 500 on the first and flatten the second to `{}` in a consumer's backup. See [313-r2-backup-tier.md](313-r2-backup-tier.md)   |
