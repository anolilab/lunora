...
Deferred/blocked plans: **033** Stream (P3) · **037** Realtime/Calls (P3) ·
**078** custom scalar types (`v.custom` — verified NOT shipped) · **089**
promise-pipelining batch (DRAFT, design-only) · **133** live CDC + DO-consumes-DO
(P3, demand-gated) · **135** stable-1.0 umbrella (TODO) · **160** voice/agent
adapter consolidation (deferred; subsumes 149/150/158) · **166** enterprise auth
(Phase 1b SAML still gated) · **168** cross-shard txn spike (decision-first) ·
**169** collab CRDT (demand-gated).
...
**235** progressive sharding (prototype only, no src routing) · **237** admin-auth
hooks (one-adapter prototype; remaining adapters open) · **238** vector reader
(codegen wiring + `define-rag` refactor open) · **240** agent run queue · **241**
in-app inbox (D1 backend open) · **242** agent reply · **245** eval runner (CLI
shipped; **Studio Evals panel verified NOT built**) · **247** event store (not
ratified) · **248** runtime lints (productizing open) ·
...
| Plan | Title                                                              | Findings            | Pkg                    | Pri | Effort | Risk    | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ------------------------------------------------------------------ | ------------------- | ---------------------- | --- | ------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 265  | Make `v.bigint()`/`v.bytes()` round-trip on the DO row store       | DO-01, SQL-01       | shard-engine/sql-store | P1  | M      | MED     | ⛔ **BLOCKED — DO NOT MERGE** (was DONE; reversed by thermos). The fix makes `v.bigint()` **silently unqueryable**: the write side stores `["$lunora.wire$","bigint","10"]` while `serializeSqlValue` (`serialize-sql.ts:21`) still renders the comparison value as `"10"`, so `json_extract` never matches — `filter`/`withIndex`/`aggregate` return empty/0 on a **money path**, converting a loud throw into a silent wrong answer. Also: `estimateBytes` throws on bigint and charges the full 32 MiB cap (one bigint write per mutation is the ceiling), and **with 270 also landed `ctx.db.insert` of a bigint throws `BAD_REQUEST` outright** — the two branches must land together or not at all. Byte-identity is additionally false for array-position `undefined`, `NaN`, `Infinity` and `Date`, and `patch` rewrites whole documents so the first patch **silently migrates a legacy row**. Egress (`export`, CDC) doesn't re-encode, so bytes reach consumers as `{}`. My earlier approval missed all of this because the added test declares `indexes: []` and never filters or aggregates. See §11 |
...
| Plan | Title                                                               | Category  | Pkg                 | Pri | Effort | Risk | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---- | ------------------------------------------------------------------- | --------- | ------------------- | --- | ------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 300  | Decode wire-tagged doc columns on the three display read paths      | bug       | studio/shard-engine | P3  | S      | LOW  | ✅ **DONE** — on `advisor/300-decode-doc-read-paths` (stacked on 265). S1 `introspect.ts` routes `safeParseObject` through `decodeDocJson`, which covers BOTH its call sites at once; the non-throwing contract is kept (a malformed tag degrades to an unexpanded page, pinned by a test). **S2 was not a one-liner.** Decoding alone _breaks_ the JSON editor: the rows now carry real `bigint`s and `JSON.stringify` throws on one — proven, the pre-fix code fails the new test with `TypeError: Do not know how to serialize a BigInt`, i.e. editing any `paymentSessions` row was already broken. Fixed symmetrically: the prefill seeds from `encodeWire(doc)` and `writeRow` decodes on the way out, so an untouched save re-encodes byte-identically to what the shard stored (asserted directly against `encodeDocJson`'s output). **§1 is wrong about S3**: `payments-panel` renders only `subscriptions`/`events`, and every `v.bigint()` in the payment schema is on `paymentSessions`, which that panel never reads — so the money columns surface in the DATA BROWSER (S1+S2), not here. The decode landed anyway for codec agreement, with a decode-sensitive test (undecoded, the value renders as `""` and vanishes). **Scope +1 file**: `lib/internal.ts` — a decoded `ArrayBuffer` formatted to `{}`, strictly less informative than the tag it replaced; §4's open question answered as `<bytes: N>` (byte length). §9 Q2 answered: the SQL console shares `expandDocumentRows`, and expansion already presents the model rather than the raw row, so it decodes too. STOP condition on the studio→shard-engine dependency did NOT fire — studio reaches the codec via bundler-inlined `shared/wire-codec` (it already inlines `shared/log-fields`), so no package dep was added. Gates: shard-engine 981, studio 1007, do 519, both `lint:types` clean; every new behavioural test demonstrated to fail against base |
...
| 290  | Stop `eslint-plugin-n` crashing the linter on Markdown              | LINT-01   | 47 eslint configs | P2  | S      | LOW  | TODO                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
