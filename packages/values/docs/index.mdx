...
- `v.optional(inner)` — the explicit spelling.
- `v.any()` — it returns its input unchanged, so `undefined` parses. A bare
  `v.any()` field is therefore `data?: unknown`, not `data: unknown`.
- `v.union(a, b, ...)` with an `any` or `v.optional(...)` member — the union
  tries its members, and one of them accepts `undefined`.
...
generated reference for a procedure declaring the _same_ validator always agree.
...
absent field (see [Which keys are optional](#which-keys-are-optional)).
