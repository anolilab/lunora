...
describe("cloudflare transport (default provider)", () => {
    ...
    it("throws a clear error (not a generic 'send failed') when cc is supplied", async () => {
        expect.assertions(2);
    ...
    it("throws a clear error when bcc is supplied", async () => {
        expect.assertions(2);

        const mailer = createMailer({ cloudflareSend: async () => undefined, from: "noreply@x.test" });

        await expect(mailer.send({ bcc: ["bcc@x.test"], subject: "Hi", text: "x", to: "user@x.test" })).rejects.toThrow(/does not support cc\/bcc/);
        expect(sendEmail).not.toHaveBeenCalled();
    });
    ...
    it("does not trip the cc/bcc guard on empty cc/bcc arrays", async () => {
        ...
        // An empty `cc`/`bcc` array means "no recipients" — it must NOT be treated
        // as a cc/bcc send and rejected.
