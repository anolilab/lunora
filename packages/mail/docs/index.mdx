...
shard 502, a missing `LUNORA_ADMIN_TOKEN`) is transient, so the handler rethrows
and lets the platform redeliver — rejecting it would hand the sender an
unrecoverable NDR for a message that had nowhere to be replayed from.
