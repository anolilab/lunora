...
failed; the key is what covers the remaining window, where the provider accepted
the send but the worker died before acking. Skip the store and that window sends a
