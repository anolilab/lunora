...
<Callout type="warn">
    ...
    `email.authentication` via the `verify` hook before dispatch, and treat the function input as fully attacker-controlled. A bare `spf === "pass"` or `dkim
    === "pass"` is not enough: SPF vouches for the envelope `MAIL FROM` domain and DKIM for the signing `d=`, both of which the sender picks. Each verdict
    carries the domain it is about (`spfDomain`, `dkimDomain`, `dmarcDomain`) — accept an SPF or DKIM pass only when that domain equals the `From` address's
    domain, and treat a pass with a `null` domain as unauthenticated. A DMARC pass already checked alignment.
</Callout>
