...
/** The locale the dashboard ships with and falls back to. */
 ...
 */
type DashboardCatalogs = Readonly<Record<string, Messages>>;
...
const BUILTIN_CATALOGS: DashboardCatalogs = { en: enMessages };

/**
 * Build a dashboard-scoped Lingui instance. We deliberately use `@lingui/core`'s
 * `setupI18n` (a fresh instance) rather than the global singleton so the
 * dashboard can never clobber — or be clobbered by — a host app that also uses
 * Lingui.
 *
 * The message compiler is installed explicitly. `@lingui/core` only auto-installs
 * it when `NODE_ENV !== "production"`, but the dashboard ships as a library and
 ...
 * to `DEFAULT_LOCALE`.
 */
const createDashboardI18n = (locale: string = DEFAULT_LOCALE, catalogs: DashboardCatalogs = BUILTIN_CATALOGS): I18n => {
 ...
 * composable dashboard and individual exported panels resolve their strings even
 * when rendered without `DashboardI18nProvider` (e.g. in tests, or a host's own
 * admin UI). English-only, so it renders source strings as-is.
 */
const dashboardI18n: I18n = createDashboardI18n();
...
// only Lingui runtime the dashboard pulls in.
const DashboardI18nContext: Context<I18n> = createContext<I18n>(dashboardI18n);

/**
 * Hook returning a `t` bound to the nearest `DashboardI18nProvider` (or the
 * shared instance when there's none). This is the runtime stand-in for Lingui's
 * `t` macro — the dashboard builds with esbuild and tests under Vite 8/Rolldown,
...
const useT = (): TFunction => {
    const i18n = use(DashboardI18nContext);

    return useCallback<TFunction>((id, values) => i18n._(id, values), [i18n]);
};

export { createDashboardI18n, dashboardI18n, DashboardI18nContext, DEFAULT_LOCALE, useT };
export type { DashboardCatalogs, TFunction };
