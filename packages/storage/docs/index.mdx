...
handlers get a read-only surface (`download`, `head`, `getMetadata`,
`getSignedUrl`, `getUrl`), and only `action` handlers get the full read/write
surface (`store`, `delete`, `generateUploadUrl`, multipart, presigned). `list` is
NOT on the read-only surface — it is dropped along with the other ungated methods
so it cannot sidestep `storageRules`; call it on the `Storage` instance itself.
`download(key)` resolves to the R2 object (its metadata plus a `body` stream), not
to a bare `ReadableStream`.
