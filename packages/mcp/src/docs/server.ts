...
import type { McpServerInfo, McpTool } from "../compose";
...
interface DocsMcpServerOptions {
    /** Extra tools to advertise alongside the documentation surface. */
...
const createDocsMcpServer = (options: DocsMcpServerOptions): Server =>
    createToolServer(serverInfo(options.version), [...docsTools(options.index), ...(options.extraTools ?? [])]);
...
const createDocsMcpFetchHandler =
    ...
    (request: Request): Promise<Response> =>
        serveStateless(createDocsMcpServer(options), request);
