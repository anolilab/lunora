...
introspecting a deployment (`lunora_list_functions`, `lunora_list_tables`,
`lunora_get_function_schema`) and invoking its functions (`lunora_run_query`,
`lunora_run_mutation`, `lunora_run_action`), each backed by
[`@lunora/client`](/docs/packages/client) over HTTP RPC.
