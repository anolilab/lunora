...
<Callout type="warn">
    ...
    programmatically). That gate is the whole read-only guarantee: the `token` must be the deployment's **admin** bearer (every tool reads admin-gated routes),
    so it cannot be scoped down to enforce read-only from the credential side. Guard who can reach the server process instead.
</Callout>
