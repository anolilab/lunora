...
import { planBackfillPass, searchCoverageSurvives, searchIndexField, searchIndexProfile } from "../src/backfill";
...
describe(planBackfillPass, () => {
    ...
    it("resumes from the recorded cursor when the profile still matches", () => {
        ...
        expect(planBackfillPass({ cursor: "row-40", done: false, profile }, profile)).toStrictEqual({ cursor: "row-40", finished: false, wipe: false });
        expect(planBackfillPass({ cursor: "row-99", done: true, profile }, profile)).toStrictEqual({ cursor: "row-99", finished: true, wipe: false });
    });

    it("restarts without wiping when a never-started index changes profile — there is nothing to discard", () => {
        expect.assertions(1);

        expect(planBackfillPass({ cursor: undefined, done: false, profile: "en-v1:body" }, profile)).toStrictEqual({
            cursor: undefined,
            finished: false,
            wipe: false,
        });
    ...
    it("wipes and restarts when rows were analyzed under another profile, whether mid-walk or finished", () => {
        ...
        expect(planBackfillPass({ cursor: "row-10", done: false, profile: "en-v1:body" }, profile)).toStrictEqual({
            cursor: undefined,
            finished: false,
            wipe: true,
        });
        expect(planBackfillPass({ cursor: "row-99", done: true, profile: "en-v1:body" }, profile)).toStrictEqual({
            cursor: undefined,
            finished: false,
            wipe: true,
        });
    ...
    it("never resumes a state that predates profile tracking — nothing says what analyzed those rows", () => {
        ...
        expect(planBackfillPass({ cursor: "row-10", done: false, profile: undefined }, profile)).toStrictEqual({
            cursor: undefined,
            finished: false,
            wipe: true,
        });
