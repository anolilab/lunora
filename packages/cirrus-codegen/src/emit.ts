...
export const emitApi = (functions: ReadonlyArray<FunctionIR>): string => {
    ...
    const renderNamespace = ([file, list]: [string, FunctionIR[]]): string => {
        ...
        return `    ${file.replaceAll(/[^\dA-Za-z]/gu, "_")}: {\n${members}\n    };`;
    };
 ...
 */
const moduleAlias = (filePath: string, index: number): string => `cirrus_${filePath.replaceAll(/[^\dA-Za-z]/gu, "_")}_${index}`;

/** Identifier the `emitApi()` namespace uses for `${namespace}:${fnName}` keys. */
const apiNamespace = (filePath: string): string => filePath.replaceAll(/[^\dA-Za-z]/gu, "_");
...
export const emitServer = (functions: ReadonlyArray<FunctionIR>): string => {
    ...
    const dispatchEntries = [...moduleEntries.entries()]
        .flatMap(([filePath, entry]) =>
            entry.functions.map(
                (function_) =>
                    `    "${apiNamespace(filePath)}:${function_.exportName}": ${entry.alias}.${function_.exportName} as unknown as RegisteredCirrusFunction,`,
            ),
        )
        .join("\n");
