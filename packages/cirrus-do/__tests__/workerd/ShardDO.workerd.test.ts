...
describe("ShardDO (workerd)", () => {
    ...
    test("broadcastDelta only reaches matching-table subscribers", async () => {
        ...
        const subs = await Promise.all([
            openSocket(stub, "sub-msg", { table: "messages" }),
            openSocket(stub, "sub-doc", { table: "documents" }),
        ]);
...
const waitFor = async (predicate: () => boolean, { timeoutMs = 2_000, intervalMs = 10 }: { timeoutMs?: number; intervalMs?: number } = {}): Promise<void> => {
