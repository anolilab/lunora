...
describe("createWorker — admin export endpoint", () => {
    ...
    test("rejects without an authorization header (403)", async () => {
        ...
        const response = await worker.fetch(
            new Request("https://app.example/_cirrus/admin/export", { method: "POST" }),
            {},
            fakeCtx,
        );
    ...
    test("streams NDJSON from orchestrateExport", async () => {
        const orchestrateExport = vi.fn(
            async (_namespace: unknown, _request: { tables: ReadonlyArray<string> }) => ({
                failed: 0,
                ok: 1,
                shards: [
                    {
...
describe("createWorker — admin import endpoint", () => {
    ...
    beforeEach(() => {
        captured = null;
        orchestrateImport = vi.fn(async (_namespace: unknown, request: { batches: typeof captured extends null ? never : Exclude<typeof captured, null>["batches"] }) => {
    ...
    test("rejects without an admin bearer (403)", async () => {
        ...
        const response = await worker.fetch(
            new Request("https://app.example/_cirrus/admin/import", { method: "POST" }),
            {},
            fakeCtx,
        );
...
describe("import streaming — large body", () => {
    test("handles a 10k-row NDJSON body without crashing", async () => {
        const orchestrateImport = vi.fn(
            async (_namespace: unknown, request: { batches: { rows: { table: string }[]; shardKey: string }[] }) => {
                const inserted: Record<string, number> = {};
