...
const SPEC = {
    info: { title: "Cirrus RPC", version: "1.2.3" },
...
describe("createWorker — openrpc admin endpoint", () => {
    it("rejects without a valid admin bearer (403)", async () => {
        ...
        const response = await worker.fetch(new Request("https://app.example/_cirrus/admin/openrpc", { method: "GET" }), {}, fakeContext);
    ...
    it("serves the injected spec verbatim (200)", async () => {
        ...
        const response = await worker.fetch(
            new Request("https://app.example/_cirrus/admin/openrpc", { headers: { authorization: `Bearer ${ADMIN_TOKEN}` }, method: "GET" }),
            {},
            fakeContext,
        );
    ...
    it("returns an empty-but-valid OpenRPC document when no spec is configured (200)", async () => {
        ...
        expect(body).toHaveProperty("info.title", "Cirrus RPC");
    });
    ...
    it("rejects non-GET (405)", async () => {
        ...
        const response = await worker.fetch(
            new Request("https://app.example/_cirrus/admin/openrpc", { headers: { authorization: `Bearer ${ADMIN_TOKEN}` }, method: "POST" }),
            {},
            fakeContext,
        );
