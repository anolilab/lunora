...
| `queueHandler`                                             | The queue fan-out seam; `POST /_lunora/scheduled` needs no option — see _Running crons and queues where the host has neither_ |
...
feed it to `lunora backup restore <file>`; for point-in-time recovery use
`lunora backup pitr --at <time> --restore`.
...
## Running crons and queues where the host has neither

Some hosts hand a Worker no cron trigger and no queue consumer of its own —
Cloudflare's Workers for Platforms is the case this exists for: a script in a
dispatch namespace gets neither, whatever its own `wrangler.jsonc` declares. The
work still has to run, so the platform drives it over HTTP instead, through two
reserved endpoints `createWorker` serves. Both are **admin-gated** (the same
bearer as `/_lunora/admin/*`) and both refuse anything but `POST`.

`POST /_lunora/scheduled` replays one cron firing:

```bash
curl -X POST https://<tenant>/_lunora/scheduled \
  -H "authorization: Bearer $LUNORA_ADMIN_TOKEN" \
  -d '{ "cron": "0 9 * * *" }'
```

Dispatch is keyed on the expression and goes through the same path a native
trigger takes — user crons, code crons and the scheduled backup all behave
exactly as they would on a real firing. A body with no `cron` is a `400`; the
platform is expected to know which expression is due.

`POST /_lunora/queue` hands over a batch the platform's own consumer drained,
and answers with the ids to redeliver:

```bash
curl -X POST https://<tenant>/_lunora/queue \
  -H "authorization: Bearer $LUNORA_ADMIN_TOKEN" \
  -d '{ "queue": "jobs", "messages": [{ "id": "m1", "body": { "userId": "u1" } }] }'
# => { "retry": ["m1"] }
```

It needs a `queueHandler` on `createWorker` (`(batch, env, ctx) => { retry? }`);
without one the endpoint is a `400` rather than a silent success, so a
misconfigured tenant fails loudly instead of dropping every batch. Messages with
...
the runtime resolves no IP at all — and the gate then **refuses** the request with
a `500` naming the fix, rather than pooling every caller behind one bucket. That is
deliberate on both counts: trusting the header off Cloudflare would let an attacker
rotate `Cf-Connecting-IP` for a fresh bucket per request and the limit would stop
applying to exactly the traffic it exists to stop, while a shared bucket inverts a
per-caller limit into a one-request-per-period lever anyone can pull for the whole
deployment. Off Cloudflare, pass `key` and identify callers by something they
cannot forge:
...
A non-Cloudflare origin sitting **behind** Cloudflare is the case where the refusal
is pure loss rather than protection. The edge stamps `CF-Connecting-IP`, the origin
accepts only Cloudflare traffic, so the header genuinely is trustworthy — yet the
runtime still resolves no IP, because the code runs on the origin and nothing in
the request tells it apart from a directly-exposed host. A limit that could have
been per-IP refuses every request instead.
