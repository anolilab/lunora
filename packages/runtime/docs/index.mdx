...
appear as one history. Snapshots the cron writes carry no `sha256`, so
`lunora backup restore --verify` refuses them rather than reporting an
unverified restore as a verified one.
