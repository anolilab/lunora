...
The runtime's typed error class. Throwing a
`LunoraError(message, { code, status })` inside a handler causes a
`{ error: { code, message, ... } }` body to come back over RPC instead of an
HTTP 500. `toErrorResponse(err)` is the same mapping exposed for add-ons.

The message comes **first**; `code` and `status` are options:
