...
rows into memory before the first byte is written, so the snapshot has to fit
alongside them in a Worker's ~128 MB. The run therefore **refuses at 64 MiB of
NDJSON** (`BACKUP_TOO_LARGE`) instead of being killed for running out of
memory — the cron invocation fails loudly and nothing is written. Narrow it with
`backupTables`, take snapshots more often, or move the job off-platform with
`lunora backup create --bucket`, which has more headroom (256 MiB) because it
runs on a machine rather than in an isolate.

Raising that ceiling means making the export itself streaming; a bigger object
or a multipart upload would not help, because the rows are already in memory
...
function on the worker (`createStorage(...).download`); the generated app
worker wires it alongside the other `storage*` options. Without it the endpoint
