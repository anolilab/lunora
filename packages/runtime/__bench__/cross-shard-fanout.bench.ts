 ...
 * each" workload.
 */
const buildGroupByPayload = (entriesPerShard: number): Array<{ key: Record<string, unknown>; value: number }> =>
    Array.from({ length: entriesPerShard }, (_, index) => ({ key: { category: `cat-${String(index)}` }, value: 1 }));
...
const buildSetup = (count: number): Setup => {
    ...
    const topKResponses = new Map<string, unknown>(
        shardKeys.map((key) => [key, Array.from({ length: 50 }, (_, index) => ({ by: `m-${key}-${String(index)}`, value: index }))]),
    );
