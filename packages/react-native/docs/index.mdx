...
- **`getAuthHeaders`** — `() => Record<string, string> | undefined`. The returned
  headers are attached to every HTTP RPC request **and** the WebSocket upgrade,
  read fresh each time so signing in/out takes effect without re-creating the
  client. Return `undefined` when signed out.
...
The better-auth Expo bridge:

- **`expoAuthHeaders(authClient)`** — adapts a better-auth Expo client (its
  `getCookie()`) into the `getAuthHeaders` factory. Lunora's `resolveIdentity`
  reads the cookie through better-auth's `getSession`, authenticating both HTTP
  RPC and the live WebSocket.
- **`expoClient`, `setupExpoFocusManager`, `setupExpoOnlineManager`** — re-exported
  from `@better-auth/expo/client`.

```tsx
import { createAuthClient } from "better-auth/react";
import { expoClient, expoAuthHeaders } from "@lunora/react-native/auth";
...
On the server, add better-auth's `expo()` plugin so the app scheme is a trusted
origin (see [`@lunora/auth`](/docs/packages/auth)).
