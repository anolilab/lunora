...
// Self-contained flat config for @lunora/react-native. Each package owns its own
// setup (no shared local preset); rules build on @anolilab/eslint-config.
export default createConfig(
    {
        // Enable type-aware linting AND let @anolilab read the tsconfig. Reading the
        // tsconfig is what gates correct behaviour: `jsx: "react-jsx"` flips the React
        // preset to the automatic runtime (react-in-jsx-scope off + jsx-runtime applied),
        // and type-aware rules (no-unsafe-*, no-unnecessary-condition, require-await) only
        // run with real type info. Without tsconfigPath both silently misfire.
