...
queries called through `ctx.runQuery` / `ctx.runMutation`. A mutation dispatched
that way gets the same transaction a top-level one gets, which is what makes the
"do the transactional part in a mutation, call it from the action" split safe.
