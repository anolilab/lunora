...
- `.searchIndex(name, { field, filterFields?, language?, staged? })` — full-text index over `field` (a dot path reads a nested field); `filterFields` (≤16) are the columns `.eq()` may narrow by inside the search; `language` adds that language's stopwords to the always-on accent folding (`de`/`en`/`es`/`fr`/`it`/`nl`/`pt`); `staged: true` skips the migration-time backfill on a large table
