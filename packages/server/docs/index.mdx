...
- `.searchIndex(name, { field, filterFields?, staged? })` — full-text index over `field` (a dot path reads a nested field); `filterFields` (≤16) are the columns `.eq()` may narrow by inside the search; `staged: true` skips the migration-time backfill on a large table
