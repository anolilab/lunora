 ...
 * `@cirrus/workflow`'s `WorkflowInstanceStatus` (itself a mirror of Cloudflare's
 ...
 * dependency on `@cirrus/scheduler`.
 */
type WorkflowInstanceStatus = "complete" | "errored" | "paused" | "queued" | "running" | "terminated" | "unknown" | "waiting" | "waitingForPause";

/** Result of {@link WorkflowInstance.status}. Mirrors `@cirrus/workflow`'s `WorkflowStatusResult`. */
...
/** Options accepted by {@link WorkflowHandle.create}. Mirrors `@cirrus/workflow`'s `WorkflowCreateOptions`. */
...
/** A live handle to a single workflow instance. Mirrors `@cirrus/workflow`'s `WorkflowInstanceLike`. */
 ...
 * Mirrors `@cirrus/workflow`'s `WorkflowHandle`.
 */
 ...
 * workflow declared in `cirrus/workflows.ts` is reachable by its export name;
 * codegen narrows the `get(name)` overloads to the known workflow names + their
 * inferred param types. Mirrors `@cirrus/workflow`'s `Workflows`.
 */
interface Workflows {
