...
| `usePresence(roomId, { heartbeat, listPresent, … })`                       | `{ present, sessionId, setData, error }`                                                                                         | Drives the `definePresence` heartbeat + list functions. `roomId` is a positional first argument, not an option.                        |
...
| `useAuth()`                                                                | `{ user, token, status, setToken }`                                                                                              | Call `setToken(jwt)` after sign-in; RPC then carries the token.                                                                        |
