...
const useFlag = <T extends FlagValue>(key: string, defaultValue: T): T => {
    ...
    // A different key/type is a different flag. Reset to this flag's default
    // DURING render (guarded by a ref) so the first paint after the change
    // never commits the previous flag's resolved value — which would flash the
    // wrong experiment arm. The effect below re-subscribes and also seeds the
    // default; doing it here as well closes the one-frame gap before that
    // effect runs. Same render-phase reset pattern usePaginatedCore uses.
    const resetKey = `${key}::${type}`;
    ...
    // The latest default, read at subscribe time so re-creating an equal default
    // (a fresh object/array literal) doesn't churn the subscription — the effect
    // keys off `key`/`type`, which capture every change the server can see.
    ...
    useEffect(() => {
        let cancelled = false;
        const currentDefault = latestDefault.current;
        ...
        const unsubscribe = subscribeFlag<T>(client, { default: currentDefault, key }, (next) => {
            if (!cancelled) {
                setValue(next);
            }
        });
 ...
 * defaults until each evaluation lands). This is the batched form of
 * {@link useFlag} — one effect manages one subscription per key, so it stays
 * rules-of-hooks-safe even as the flag set changes between renders. Like
 * {@link useFlag} it evaluates under the socket's server-verified identity only.
 */
const useFlags = <T extends Record<string, FlagValue>>(flags: T): T => {
    ...
    // set changes, so the first paint returns a record shaped like the new
    // `flags` — not the previous state object, which would be missing the new
    // keys and still carry the old ones (violating the declared `T`).
    // The effect below also seeds the defaults; this closes the one-frame gap.
    const resetKeyRef = useRef(spec);

    // react-doctor-disable-next-line react-hooks-js/refs -- intentional: React's sanctioned "reset state when an input changes" pattern — compare a render-phase ref to the current reset key and set state during render, guarded so it runs once per change.
    if (resetKeyRef.current !== spec) {
        // react-doctor-disable-next-line react-hooks-js/refs -- intentional: writing the ref guard here is what makes the render-phase reset fire exactly once per input change.
        resetKeyRef.current = spec;
        setValues(flags);
    }
    ...
    useEffect(() => {
        let cancelled = false;
        const currentFlags = latestFlags.current;
