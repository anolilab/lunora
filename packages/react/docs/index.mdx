...
Returns `{ call, pending, data, error, isError, reset }`. `call` is the
awaitable invocation and rejects on failure; `pending` is ref-counted across
overlapping calls from this hook, so a sibling component calling the same action
never affects it.
