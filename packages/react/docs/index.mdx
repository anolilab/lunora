---
title: "@lunora/react"
description: React 19 hooks built on @lunora/client, with React Server Component data loading.
related:
...
Offline, `mutate` still calls `client.mutation` at once: the optimistic update
paints, and the write goes to the offline queue (or your durable outbox) to
replay on reconnect. TanStack never pauses or retries the call; the queue owns
replays.
...
Like `useMutation`, the call never pauses while offline and is never retried.
Unlike a mutation, it is not queued either: `client.action` has no offline queue
and sends no idempotency key, so a paused call would fire minutes later on
reconnect and a retry could run a third-party side effect twice. It fails fast
instead and lets you decide.
...
See the [`@lunora/client` reference](/docs/packages/client) for the full table.
