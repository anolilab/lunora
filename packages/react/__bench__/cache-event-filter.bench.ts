...
const makeRef = (ref: string): { __cirrusRef: string } => {
...
const pageEntries = Array.from({ length: PAGE_COUNT }, (_, index) => {
    ...
    const key: QueryKey = cirrusQueryKey(makeRef("messages:list"), args, undefined);
...
const buildEvents = (): QueryCacheNotifyEvent[] => {
    ...
    const targetKey: QueryKey = cirrusQueryKey(makeRef("messages:list"), targetArgs, undefined);
    ...
    for (let index = 0; index < 200; index += 1) {
        const unrelatedKey: QueryKey = ["cirrus", `unrelated:q${String(index)}`, {}, null];
