...
describe("emitOpenApiModule", () => {
    ...
    it("emits a do-not-edit header and an `openApiSpec` named export", () => {
        ...
        expect(moduleSource).toContain("export const openApiSpec = JSON.parse(");
    });
    ...
    it("parses as valid TS exporting `openApiSpec` whose value equals the JSON document", () => {
        ...
        // diagnostics — guards against a malformed module.
        ...
        // The inlined data must equal the document the `.json` artifact
        // serializes (same source of truth, no recompute/drift).
        expect(emittedJsonData(moduleSource, "openApiSpec")).toStrictEqual(document);
    });
    ...
    it("inlines the same document `emitOpenApi` stringifies (json ↔ ts agree)", () => {
        ...
        expect(emittedJsonData(emitOpenApiModule(document), "openApiSpec")).toStrictEqual(JSON.parse(emitOpenApi(input)));
    });
});
