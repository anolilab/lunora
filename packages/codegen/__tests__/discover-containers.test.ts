...
describe("emit (containers)", () => {
    ...
    it("emitServer types ctx.containers on ActionCtx only when containers exist", () => {
        ...
        const withContainers = emitServer(false, EMPTY_SCHEMA, [], discover());
        ...
        expect(emitServer(false, EMPTY_SCHEMA, [], [])).not.toContain("readonly containers: {");
    });

    it("emitShard wires createContainerContext into the built ctx", () => {
        expect.assertions(4);

        const shard = emitShard(EMPTY_SCHEMA, [], undefined, false, undefined, discover());
    ...
    it("emitShard stays container-free without definitions", () => {
        ...
        expect(emitShard(EMPTY_SCHEMA, [], undefined, false, undefined, [])).not.toContain("CIRRUS_CONTAINERS");
    });
});
