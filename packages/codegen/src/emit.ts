...
${workflows.map((workflow) => `    get(name: ${JSON.stringify(workflow.exportName)}): WorkflowHandle<WorkflowParamsOf<typeof lunoraWorkflowDefinitions.${workflow.exportName}>>;`).join("\n")}
    ...
    const server = `${GENERATED_HEADER}import { createPolicyDsl, initLunora, v as vBase } from "@lunora/server";
import type {
    ActionBuilder,
...
const emitShard = ({
    ...
    const importLines = [
        `import type { ${doTypeImports.join(", ")} } from "@lunora/do";`,
        `import { applyCdcChanges, createShardCtxDb, runDataMigration, runShardMigrations, ${relationFanout.importFragment}ShardDO as ShardDOBase } from "@lunora/do";`,
    ...
    if (hasTables) {
        importLines.push(`import { bindOrm, bindTableFacade } from "@lunora/server";`);
    }
...
const renderDrizzleFile = (tables: ReadonlyArray<TableIR>): string => {
    ...
    return `${GENERATED_HEADER}import { ${importParts.join(", ")} } from "@lunora/server/drizzle";
