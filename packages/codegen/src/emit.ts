...
/**
 * Strict allowlist for a `v.literal(...)` argument's source text:
 *   - double-quoted string with no embedded `"` or backslash
 *   - single-quoted string with no embedded `'` or backslash
 *   - integer/float (optional leading `-`)
 *   - `true` / `false` / `null`
 */
...
const validatorToType = (validator: ValidatorIR): string => {
    switch (validator.kind) {
        ...
        case "literal": {
            ...
            // `true`, `null`). Validate against a strict TS-literal allowlist so
            // arbitrary expressions can't be embedded into emitted types.
