...
import type { AgentToolEventsApi, AgentToolEventsResult } from "../src/agent-tool-events";
...
describe(agentToolEvents, () => {
    it("derives the durable tool lifecycle (call, result, awaiting-approval) from agentMessages", () => {
        ...
        const tool: AgentToolEventsResult = agentToolEvents({ api: buildApi(), client: fake.asClient, destroyRef: destroy.asDestroyRef, threadKey: "t1" });

        // The lone history channel opens on setup.
        expect(fake.subscriptions.map((sub) => sub.functionPath)).toStrictEqual([MESSAGES_REF]);
        expect(tool.events()).toStrictEqual([]);
    ...
    it("forwards the history limit to agents:agentMessages", () => {
        ...
        agentToolEvents({ api: buildApi(), client: fake.asClient, destroyRef: destroy.asDestroyRef, limit: 10, threadKey: "t1" });
    ...
    it("appends live progress events (kind === 'progress') after the durable lifecycle", async () => {
        ...
        const tool: AgentToolEventsResult = agentToolEvents({
            api: buildApi(),
