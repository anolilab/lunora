...
`@angular/core` (`^19.2.0 || ^20.0.0`) is a peer dependency — the host app
supplies the Angular runtime.
...
| `injectLunoraClient` | function         | Read the `LunoraClient` from the current injector. Throws outside an injection context (falls back to the root-scoped default). |
