...
| `liveQuery`          | function         | Live query as an Angular `Signal`. Args are not reactive here; pass `"skip"` to short-circuit.                                                                                    |
...
`runAction`, `connectionStatus`, and the client provider. It does not (yet) ship the parity
