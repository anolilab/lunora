...
const createShardCtxDb = (options: CtxDbOptions): DatabaseWriterLike => {
     ...
     */
    const hydrateDocsById = (tableName: string, ids: ReadonlyArray<string>): Map<string, Record<string, unknown>> => {
    ...
    const writer: DatabaseWriterLike = {
        ...
        async rankPage(tableName, indexName, rankPageOptions = {}) {
            ...
            const { continueCursor, hasMore, rows } = computeRankPage(tableName, indexName, rankPageOptions);
