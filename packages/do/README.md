...
`broadcastDelta` is the raw-delta fan-out for a shard like this one, which has
no `executeSubscription` override and so cannot re-run a subscriber's query. It
delivers to sockets whose registered subscription carries a `table` string equal
to `delta.table` — an exact match, with no table-dependency analysis behind it.
`@lunora/client` has no table name to register there and sends the function path
instead, and so does every non-JS SDK — their `subscribe` methods do not expose
the field at all. So no client shipped here reaches a delta stamped with a real
table name; addressing one means building the subscribe frame by hand. Projects
generated by `lunora codegen` route this hook to `recordChangedTable` and get
server-side re-execution instead, which is the path to prefer.
