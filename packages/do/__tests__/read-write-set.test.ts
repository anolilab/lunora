...
import { buildIndexRange, indexKeysForRow, keysTouchRanges, parseRangeDepKey, rangeContains, rangeDepKey } from "../src/read-write-set";

const INDEX = "by_channel_ts";

const FIELDS = ["channelId", "ts"];

const range = (conditions: StagedCondition[], fields: readonly string[] = FIELDS): KeyRange | undefined =>
...
describe("buildIndexRange", () => {
    it("confines an equality prefix to its own partition", () => {
        ...
        expect(touches(eqA, { channelId: "A", ts: 9_999 })).toBe(true);
...
describe("rangeDepKey", () => {
    it("round-trips through the dependency-set encoding", () => {
        expect.assertions(1);

        const built = range([{ comparator: "=", field: "channelId", value: "A" }]) as KeyRange;

        expect(parseRangeDepKey(rangeDepKey(built))).toStrictEqual(built);
    });

    it("does not mistake a row-id or scan dependency for a range", () => {
        expect.assertions(2);

        expect(parseRangeDepKey("messages:abc123")).toBeUndefined();
        expect(parseRangeDepKey("messages:*scan")).toBeUndefined();
    });
});
