...
export interface QueueJob {
    ...
    /**
     * Set by `createQueueConsumer` on the delayed copy it re-enqueues when a
     * job's dispatch is declined on its last delivery: the id of the message it
     * replaces. The copy dispatches under that id, so the shard serves the
     * declined call's result instead of running it a second time, and a copy
     * is never re-enqueued again.
     */
    requeuedFrom?: string;
    /** Routing hint forwarded to the Worker so the call lands on the right shard. */
    shardKey?: string;
...
export interface QueueConsumerOptions {
     ...
     * delivery, where a declined dispatch is re-enqueued through `queue` (or
    ...
    /**
     * The producer binding of the queue this consumer reads (the one handed to
     * `createQueueWorkpool`). With it, a job whose dispatch is declined on its
     * last delivery is re-enqueued as a copy delayed past the in-flight claim,
     * with a fresh retry budget, instead of being dead-lettered or dropped
     * while its call is still running. Without it that loss is only logged.
     */
    queue?: QueueLike<QueueJob>;
}
