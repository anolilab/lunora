...
scheduler keeps the record and re-arms it on the ordinary retry ladder — the job
is still delivered at least once. What it no longer does is run twice at the
same time. The retry that follows lands after the first attempt has settled and
is served that attempt's cached result.

Staleness needs no timeout, because the claim lives in the same isolate as the
handler. A Durable Object is single-instance: if the isolate that took a claim is
gone, so is the handler it was running, so a successor instance finds no claim
and runs the job. Nothing waits out a guessed horizon.
...
- A job whose handler keeps declining because it really is still running rides
  the retry ladder like any other unsuccessful attempt, and is parked in the
  dead-letter once its `maxAttempts` budget is spent. The work itself completed;
  the record is in `/dead` and can be retried from there.

<Callout type="info">
    **A scheduled `action` no longer has to be idempotent to survive an overlapping re-delivery** — the shard refuses the second one. It still has to be
    idempotent to survive an ordinary **sequential** retry whose first attempt's response was lost in transit before its dedup row was written, which is the
    plain at-least-once case every scheduled job has. For genuinely long work, `createQueueWorkpool` + `createQueueConsumer` remains the better fit: it bounds
    each job at five minutes and reports completion explicitly.
</Callout>
