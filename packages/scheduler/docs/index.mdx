...
<Callout type="info">
    `interval` takes exactly one of `{seconds | minutes | hours}`. `hourly`, `daily`, `weekly`, and `monthly` schedule at a fixed UTC wall-clock time. Use
    `.cron` for the raw 5- or 6-field grammar when the ergonomic forms don't fit.
</Callout>
