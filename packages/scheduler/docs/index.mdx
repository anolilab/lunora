...
or crashing while a dispatch is open. Here the three kinds diverge.
...
| `action`           | **Can run twice, concurrently.** See below.                              |

An action is dispatched without the shard's single-writer gate — taking it would
let any caller freeze a whole shard for the length of an action's outbound I/O —
and its dedup row is written only once the handler returns. A second delivery
that arrives while the first is still running therefore finds nothing to dedup
against and runs the handler again, alongside it.

The scheduler narrows that window rather than leaving it open: a claimed job
stays reserved for **15 minutes** before another instance may fire it, which is
the platform's own ceiling on how long a dispatch this scheduler started can
still be open (an alarm invocation, and the fetch it owns, are torn down at that
mark). The cost is at the other edge: a job whose instance genuinely died waits
out that reservation before it is retried. It is delayed, never dropped.

<Callout type="warn">
    **A scheduled `action` must be idempotent if it can run past 15 minutes**, or it must mark its own completion — a row it writes first and checks on entry, a
    provider-side idempotency key, anything that makes a second concurrent run a no-op. Nothing below the handler can see that a handler has outlived the
    request that started it. This is the same ceiling `exec` documents for container commands, and one more reason to move genuinely long jobs to
    `createQueueWorkpool` + `createQueueConsumer`.
</Callout>
