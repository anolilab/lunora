...
- **`.get(name)`**: one container instance per name. Use it for stateful,
  ...
  same name always reaches the same instance. Names starting with `pool-` are
  reserved for the instances `.any()` / `.pool()` pick and are rejected, so an
  entity can never share an instance (disk, lifecycle) with a pool member.
...
refuses any path resolving into it — in any letter case, percent-encoded, or
with `;params` on the segment — so serve your application's routes elsewhere and
reach exec through `exec`. The container Durable Object enforces it a second
time: a request into `/__lunora/*` (any segment, same spellings) that `exec`
did not send is answered `403` and never reaches your container.
...
the injected values apply only to implicit starts or a bare `start()`. That is
deliberate: `start({ envVars: {} })` (or with only what the job needs) is how
you boot an untrusted or model-driven exec sandbox without the credentials the
definition declares. Pass every variable the container needs when you use it.
