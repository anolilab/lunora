...
- **`.get(name)`**: one container instance per name. Use it for stateful,
  ...
  same name always reaches the same instance. Names starting with
  `__lunora-pool-` are reserved for the pool below and rejected, so an entity
  can never share an instance (disk, lifecycle) with a pool member.
