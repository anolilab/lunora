...
<Callout type="warn">
    ...
    already pauses for a human on `exec` **and** on any `fetch` that resolves to `/__lunora/exec`, since both reach the same command path. Restrict what can run
    inside the container too, rather than relying on the caller.
</Callout>
