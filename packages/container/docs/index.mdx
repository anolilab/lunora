...
restarts. A `start({ envVars })` that differs from the env the container is
already running with is rejected with `CONFLICT` rather than silently ignored —
`stop()` the instance first. Pass every variable the container needs.
