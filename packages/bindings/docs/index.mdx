...
export const thumbnail = action({
    handler: async (ctx) => {
        const out = await ctx.images.transform(sourceStream, { width: 128, format: "webp" });
        return out;
    },
});
```
...
export const search = action({
    args: { query: v.string() },
    handler: async (ctx, { query }) => ctx.vectors.search("docs", query, { topK: 5 }),
});
```
...
export const topEvents = action({
    handler: async (ctx) => ctx.r2sql.from("events").select("type").count().groupBy("type").orderBy("count", "desc").limit(10),
});
```
