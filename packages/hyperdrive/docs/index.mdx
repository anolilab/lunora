...
Two caveats specific to this backend: collation is the engine's, so Postgres
matches bytes where MySQL folds case and accents; and ranking aggregates over
every matching token row, so the 1024-document limit bounds what you get back,
not what the database reads.
