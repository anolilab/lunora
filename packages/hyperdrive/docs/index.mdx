...
bridge above, on your own schedule). Full-pull is the only delete-detection mode —
an incremental mode (pull only changed rows, for slices past the ~10k full-pull
sweet spot) is deferred to a post-1.0 release and no longer typechecks.
