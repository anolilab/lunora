...
prefix. Rows that predate the index are backfilled when the companion is created
— pass `staged: true` on a large table and run the backfill out-of-band instead.
