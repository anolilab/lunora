...
The `external_source_unscoped` advisor lint fails the build if a sourced + sharded
table omits `tenantBy`, and `external_source_on_global` rejects combining `.source()`
with `.global()`. Clients consume the slice with `useShape(api.shapes.tenantDocs)` —
no client change, since it is an ordinary table to them.
