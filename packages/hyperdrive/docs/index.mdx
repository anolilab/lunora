...
createShardDO({
    vectors: (env) => {
        const client = fromPostgresJs(postgres(createHyperdrive((env as Env).HYPERDRIVE).connectionString));

        return { posts_search: createPgVectorIndex({ client, dimensions: 768, metric: "cosine", name: "posts_search" }) };
    },
});
```
...
The table (`__vec_<name>` by default), the `vector` extension, and an HNSW index
in the metric's operator class are provisioned lazily on first use — every
...
<Callout type="warn">
    Three deliberate differences from Vectorize. **Metadata filters are equality
    only** (JSONB containment) — a comparison operator like `{ views: { $gt: 10 } }`
    throws rather than being dropped, because a silently ignored filter returns
    confidently wrong neighbours. **`returnMetadata: "indexed"` behaves as
    ...
    while a Postgres write is already durable when its promise resolves. A filter
    this store cannot express — a comparison operator, or Vectorize's dot-addressed
    `"author.role"` syntax — throws rather than being dropped, because a silently
    ignored filter returns confidently wrong neighbours.

</Callout>

<Callout type="info">
    Two ceilings worth knowing. `dimensions` is capped at 2000, pgvector's HNSW limit — it is rejected at construction rather than failing opaquely at `CREATE
    INDEX` (Vectorize itself tops out at 1536, so most ports are unaffected). And because pgvector applies `WHERE` **after** scanning the ANN index, a filtered
    query widens the search first (`hnsw.iterative_scan`, pgvector 0.8+); on an older server that GUC is refused and a heavily-filtered query may return fewer
    matches than Vectorize would.
</Callout>
...
(0 = identical), or the plain inner product.
...
| `createPgVectorIndex({ client, name, dimensions })` | A `pgvector`-backed index for `ctx.vectors` (no Vectorize binding) |
