...
describe("createSqlCtxDb — adding a field to an already-provisioned global table", () => {
    ...
    it("adds the column to the existing table, so a write that names it succeeds", async () => {
        ...
        // The row written before the column existed still reads, with the new field
        // ABSENT rather than null: `ALTER` backfills SQL NULL, and the decoder maps
        // a NULL in an optional column to "not there" (see the NULL-decoding suite
        // below). Worth pinning either way — a consumer testing `=== null` on a
        // column added this way silently takes the wrong branch.
        const rows = await after.findMany("notes", { where: { slug: "a" } });

        expect((rows.page[0] as Record<string, unknown>)["pinnedAt"]).toBeUndefined();
    });
    ...
    it("adds a newly REQUIRED field as a nullable column, so existing rows survive it", async () => {
        expect.assertions(2);

        const before = createSqlCtxDb({ dialect: makeSqliteDialect(), exec: harness.exec, schema });
        const existing = await before.insert("notes", { archived: false, body: "first", priority: 1, slug: "a" });

        // SQLite cannot ADD COLUMN … NOT NULL without a default, and inventing one
        // would write a value of the store's choosing into every existing row — so
        // the column lands nullable and the validator keeps enforcing `notNull` on
        // write. The row that predates the field reads it back as `null`.
        ...
        const id = await after.insert("notes", { archived: false, body: "x", owner: "u1", priority: 1, slug: "c" });

        await expect(after.get(id)).resolves.toMatchObject({ owner: "u1" });
        await expect(after.get(existing)).resolves.toMatchObject({ owner: null });
    });
});
