 ...
 *
 * ## Why kysely + node:sqlite, and not Lunora's own adapter
 *
 * `scim()` refuses to serve unless the adapter exposes native transactions. Lunora's
 * `lunoraAuthAdapter` / `lunoraD1Adapter` are single-table CRUD over `ctx.db` and cannot
 * offer them, and better-auth's kysely adapter only provides them when the caller opts
 * in with `transaction: true`. The lifecycle therefore runs on the substrate that
 * supports it, and the constraint itself is pinned by a test below rather than left as
 * folklore.
 */
...
describe("scim provisioning lifecycle", () => {
    beforeEach(async () => {
        ...
        auth = createAuth({
            ...
            // `transaction: true` is what satisfies the SCIM plugin's transaction
            // requirement; without it the plugin refuses to serve a single request.
            database: { dialect: new NodeSqliteDialect({ database }), transaction: true, type: "sqlite" as const },
        ...
        // Real migrations rather than a hand-written schema, so this runs against the
        // tables an app would actually get from `lunora migrate`.
