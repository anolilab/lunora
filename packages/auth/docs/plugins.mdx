...
`admin.createUser` — because it hooks `user.create.before` rather than one route.
...
| `listSignUpInvitations(auth, { pendingOnly? })` | Every invitation newest-first; `pendingOnly` drops the accepted and the expired.     |
| `revokeSignUpInvitation(auth, { email })`       | Withdraw it.                                                                         |

```ts
const invite = await createSignUpInvitation(auth, {
    email: "ada@example.com",
    expiresInSeconds: 3 * 24 * 60 * 60, // default: 7 days
...
same trust model as [`createAuthAdmin`](/docs/packages/auth#studio-user-dashboard).
...
<Callout type="warn">
    **An invitation is keyed by email address, and nothing else.** There is no secret token, so whoever signs up first with an invited address gets the seat —
    an attacker who *knows* an invited address can take it before its owner does. `emailAndPassword.requireEmailVerification` is what closes that, and the
    plugin warns on startup when password sign-up runs without it. OAuth sign-ups carry a provider-verified address and are safe either way.
</Callout>

Accounts created with **no** email at all (the `anonymous` and `siwe` plugins)
are not gated — there is no address to match an invitation against. Installing
either alongside `inviteOnly()` re-opens self-serve sign-up.

By default the very first account is let through uninvited, so a fresh
deployment can be bootstrapped without seeding a row by hand. Pass
`inviteOnly({ allowFirstUser: false })` and seed the first invitation yourself
