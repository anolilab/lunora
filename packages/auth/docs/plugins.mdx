...
| `mcp` / `requireMcpAuth` / `mcpHandler` | OAuth-protect a Model Context Protocol server. Pairs with [`@lunora/mcp`](/docs/packages/mcp). `withMcpAuth` was renamed `requireMcpAuth` in better-auth 1.7. | [better-auth: MCP](https://www.better-auth.com/docs/plugins/mcp)                                   |
...
<Callout type="info">
`captcha` has no dedicated `better-auth/plugins/<name>`
