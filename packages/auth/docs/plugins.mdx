...
`invite.token` is the **only** time that value exists in the clear — the database
...
<Callout type="warn">
    **The invitation link is a bearer credential.** Anyone holding it can take that seat, so deliver it to the invitee rather than to a shared or forwarded
    inbox, and reissue rather than resend if you are unsure where a link went.
