...
<Callout type="info">
    **Declare `.input()` before `.use()`.** A `.use()` middleware receives the validated arguments as `ctx.args`, so a signup gate can select the address
    ...
    type-check. It is a frozen shallow copy taken after validation — middleware cannot rewrite what the handler then receives, and the gate never sees input
    that has not crossed the validator.
