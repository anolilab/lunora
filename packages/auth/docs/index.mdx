...
```ts
import { createAuth, lunoraD1Adapter } from "@lunora/auth";
import { admin, organization, scim } from "@lunora/auth/plugins";
...
deactivate accounts (and sync groups) without anyone signing in.
...
`ctx.authApi`, sign-in, sessions, and SCIM all work. **The studio's auth pages and the
audit feed do not** — `authAdmin` and `authAuditReader` read the auth tables directly
from the worker, which DO storage does not permit, so they report "not configured"
rather than returning empty data. Identity resolution costs a round-trip to the object
on requests that touch `ctx.auth`; enable better-auth's `session.cookieCache` to serve
most of them from the signed cookie instead, accepting a staleness window on revocation.
