...
through. Note that auth routes bypass the runtime's request-body limits — the caps in
...
then live outside Cloudflare's first-party storage, away from `ctx.db`.
...
```ts
import { createAuth, lunoraDoAdapter } from "@lunora/auth";
...
export class AuthDO {
    ...
    constructor(state: DurableObjectState, env: Env) {
        this.#auth = createAuth({
            secret: env.AUTH_SECRET,
            database: lunoraDoAdapter(state.storage),
            plugins: [scim({ connections: [{ id: "okta-acme", credentials: [{ type: "bearer", id: "primary", token: env.SCIM_TOKEN }] }] }), admin()],
        });
    }

    async fetch(request: Request) {
        return (await handleAuthRequest(this.#auth, request)) ?? new Response("not found", { status: 404 });
    }
}
```

Route `/api/auth/*` from your worker to the object, and the object owns the auth tables.
Verified against a real Durable Object in workerd (`__tests__/workerd/do-adapter.workerd.test.ts`),
not just a SQLite stand-in.
...
- [@lunora/hyperdrive](/docs/packages/hyperdrive) — the Postgres/MySQL route SCIM needs (D1 has no transactions).
