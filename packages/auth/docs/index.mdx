...
**Two lifecycle semantics that surprise people** — both pinned by
`__tests__/scim-provisioning.behaviour.test.ts`, so they are what the shipped version
actually does rather than what the SCIM spec might imply:
...
```bash
wrangler hyperdrive create lunora-auth --connection-string="postgres://user:pass@host:5432/db"
```
