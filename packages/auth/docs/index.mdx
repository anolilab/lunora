...
over `ctx.db` — and D1 itself, whose driver has no interactive transactions. Use a
transactional database reached through better-auth's kysely adapter with
`transaction: true`, e.g. Postgres or MySQL via
[`@lunora/hyperdrive`](/docs/packages/hyperdrive):

```ts
createAuth({
    database: { dialect: myPostgresDialect, type: "postgres", transaction: true },
