import type { CirrusClient, FunctionReference, SubscriptionError, SubscriptionErrorCallback, Unsubscribe } from "@cirrus/client";
import { describe, expect, it, vi } from "vitest";

import { createQuerySubscription, SKIP, toSubscriptionError } from "../src/query-subscription";
...
describe("createQuerySubscription", () => {
    ...
    it("does not deliver server errors after teardown", () => {
        ...
        const unsubscribe = createQuerySubscription(mock.asClient, makeRef("admin:list"), {}, { onData: vi.fn(), onError });
    ...
    it("rethrows an attach throw when no error sink is present", () => {
        ...
        expect(() => createQuerySubscription(mock.asClient, makeRef("messages:list"), {}, { onData: vi.fn() })).toThrow("client is closed");
    });
    ...
    it("only passes an onError callback to the client when a sink wants one", () => {
        ...
        const tearWithError = createQuerySubscription(withError.asClient, makeRef("a"), {}, { onData: vi.fn(), onError: vi.fn() });
        ...
        const tearWithoutError = createQuerySubscription(withoutError.asClient, makeRef("a"), {}, { onData: vi.fn() });
