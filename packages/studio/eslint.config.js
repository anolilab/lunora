...
// Self-contained flat config for @lunora/studio. Each package owns its own
...
export default createConfig(
    ...
    // React Compiler is enabled for this package (babel-plugin-react-compiler in
    // packem.config.ts). It auto-memoizes props, context values, and callbacks at
    // build time, so the lint rules that demand hand-written useMemo/useCallback to
    // stabilize inline functions/objects/arrays are now redundant — and actively
    // wrong, since they'd push us back to the manual memoization the compiler
    // replaces. Turn them off for source (tests already disable the react-perf pair).
    {
        rules: {
