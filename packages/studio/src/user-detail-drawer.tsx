...
export const UserDetailDrawer = ({ capabilities, onChanged, onClose, user }: UserDetailDrawerProps): ReactElement => {
    ...
    const onRoleInputChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setRoleInput(event.target.value);
    }, []);
    const onBanReasonChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setBanReason(event.target.value);
    }, []);
    const onBanDaysChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setBanDays(event.target.value);
    }, []);
    const onNewPasswordChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
    ...
    }, [banDays, banReason, client, runAction, user.id]);

    const onUnban = useCallback((): void => {
        runAction(async () => {
            await client.unbanAuthUser({ userId: user.id });
        });
    }, [client, runAction, user.id]);

    const onSetPassword = useCallback((): void => {
        runAction(async () => {
    ...
    }, [client, onClose, runAction, user.id]);
    ...
    return (
        <ModalShell label={t("User details")} onClose={onClose} panelTestId="ud-panel" testId="ud-overlay" variant="drawer">
            ...
            <div className="flex flex-col gap-1" data-testid="ud-fields">
                <h3 className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">{t("Fields")}</h3>
