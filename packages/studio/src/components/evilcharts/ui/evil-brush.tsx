...
const EvilBrush = ({
    ...
    const [internalRange, setInternalRange] = React.useState<EvilBrushRange>(() => {return {
...
{return null;}
    ...
    return (
        <div className={cn("group relative select-none", className)} data-chart={skipStyle ? undefined : chartId} ref={containerRef} style={{ height }}>
            ...
            {/* Dim overlay – left */}
            <motion.div
                className="bg-background/70 pointer-events-none absolute inset-y-0 start-0 rounded-s-md backdrop-blur-[2px]"
                style={{ width: leftOverlayWidth }}
            />
            {/* Dim overlay – right */}
            <motion.div
                className="bg-background/70 pointer-events-none absolute inset-y-0 end-0 rounded-e-md backdrop-blur-[2px]"
