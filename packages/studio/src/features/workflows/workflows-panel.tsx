...
const WorkflowsPanel = (): ReactElement => {
    ...
    const upsertInstance = useCallback((next: ObservedInstance): void => {
    ...
    }, [client, instanceIdInput, paramsText, selectedExportName, t, upsertInstance]);

    const startInstance = useCallback((): void => {
        fireAndForget(start());
    }, [start]);

    const refreshInstance = useCallback(
        async (exportName: string, id: string): Promise<void> => {
            setRefreshingId(id);
    ...
    const onSelectedExportChange = useCallback((event: ChangeEvent<HTMLSelectElement>): void => {
        setSelectedExport(event.target.value);
    }, []);

    const onInstanceIdChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setInstanceIdInput(event.target.value);
    }, []);

    const onParamsChange = useCallback((event: ChangeEvent<HTMLTextAreaElement>): void => {
