...
export const SchemaEditorOverlay = ({ onApplied, tableNames }: SchemaEditorOverlayProps): ReactElement => {
    ...
    const submit = useCallback(
        async (edit: AdditiveEdit): Promise<void> => {
            setBusy(true);
            setResult(null);
    ...
    }, [submit, indexFields, indexName, targetTable, unique]);

    const openTable = useCallback((): void => {
        setResult(null);
        setMode("addTable");
    }, []);
    const openColumn = useCallback((): void => {
        setResult(null);
        setMode("addColumn");
    }, []);
    const openIndex = useCallback((): void => {
        setResult(null);
        setMode("addIndex");
    }, []);
    ...
    const onColumnNameChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setColumnName(event.target.value);
    }, []);
    const onValidatorChange = useCallback((event: ChangeEvent<HTMLSelectElement>): void => {
        setValidator(event.target.value);
    }, []);
    const onTargetTableChange = useCallback((event: ChangeEvent<HTMLSelectElement>): void => {
        setTargetTable(event.target.value);
    }, []);
    const onIndexNameChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setIndexName(event.target.value);
    }, []);
    const onIndexFieldsChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setIndexFields(event.target.value);
    }, []);
    const onUniqueChange = useCallback((event: ChangeEvent<HTMLInputElement>): void => {
        setUnique(event.target.checked);
    }, []);
    const onOpenMigrations = useCallback((): void => {
