...
const ApiTab = ({ functions, initialShardKey, openApiSpec, openRpcSpec }: ApiTabProps): ReactElement => {
    ...
    const viewLabel = { reference: t("Reference"), snippets: t("Snippets") };

    const formatLabel = { openapi: t("OpenAPI"), openrpc: t("OpenRPC") };

    const selectView = (event: React.MouseEvent<HTMLButtonElement>): void => {
        setView(event.currentTarget.dataset.view as ApiView);
    };

    const selectFormat = (event: React.MouseEvent<HTMLButtonElement>): void => {
