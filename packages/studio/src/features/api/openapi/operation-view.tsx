...
const OperationView = ({ operation }: OperationViewProps): ReactElement => {
    ...
    return (
        <div
            ...
            key={operation.key}
        >
            <header className="sticky top-0 z-10 -mx-6 -mt-6 flex flex-col gap-2 border-b border-border bg-background px-6 pt-6 pb-3">
            ...
            <section className="flex flex-col gap-2">
                <h2 className="font-mono text-[11px] tracking-wide text-muted-foreground uppercase">{t("Try it")}</h2>
                <TryIt />
            </section>

            <section className="flex flex-col gap-2">
                <h2 className="font-mono text-[11px] tracking-wide text-muted-foreground uppercase">{t("Request arguments")}</h2>
                <SchemaTable schema={operation.argsSchema} testId="api-operation-args" />
            </section>

            <section className="flex flex-col gap-3">
                <h2 className="font-mono text-[11px] tracking-wide text-muted-foreground uppercase">{t("Response body")}</h2>
                {operation.responses.map((response) => (
                    <div className="flex flex-col gap-1.5 rounded-xl border border-border bg-card p-3" key={response.status}>
