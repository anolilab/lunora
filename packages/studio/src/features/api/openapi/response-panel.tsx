...
const ResponsePanel = (): ReactElement => {
    ...
    const defaultId = useMemo(() => (ran ? LIVE_TAB : (tabs.find((tab) => tab.status.startsWith("2"))?.id ?? tabs[0]?.id ?? "")), [ran, tabs]);
    ...
    return (
        <div className="flex min-h-0 flex-col rounded-md border border-border bg-sidebar/50" data-testid="api-response-panel">
