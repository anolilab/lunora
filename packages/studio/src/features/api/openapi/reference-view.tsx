...
const ReferenceView = ({ model }: ReferenceViewProps): ReactElement => {
    ...
    const groups = useMemo(() => filterGroups(model.groups, filter), [model, filter]);
                    ...
                    {groups.map((group) => (
                        <div className="flex flex-col gap-1" key={group.name}>
                            <div className="flex items-center justify-between px-2">
                                <span className="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">{group.name || t("(root)")}</span>
