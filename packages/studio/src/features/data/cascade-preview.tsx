...
const CascadeBadge = ({ table }: { table: string }): ReactElement => (
    <span className="rounded bg-amber-500/10 px-1 text-amber-700 dark:text-amber-400" data-testid={`cascade-cascade-${table}`}>
        cascade
    </span>
);
    ...
    return (
        <ModalShell label="Cascade impact preview" onClose={onClose} panelTestId="cascade-panel" testId="cascade-overlay" variant="dialog">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-foreground" data-testid="cascade-title">
                    {t("Cascade impact")}
                </h3>
