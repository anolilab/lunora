...
export const useFacets = (): UseFacets => {
    ...
    const fetchFacet = useCallback(async (column: string, fetcher: FacetFetcher): Promise<void> => {
        setFacets((current) =>
    ...
    const toggleFacet = useCallback(
        (column: string, fetcher: FacetFetcher | null): void => {
            setFacets((current) => {
    ...
    const clearFacets = useCallback((): void => {
        setFacets({});
        facetsRef.current = {};
    }, []);
