...
const useGenerateRows = (onRefresh: () => void): UseGenerateRowsModel => {
    ...
    const openDialogAsync = useCallback(
        async (targetTable: string, targetShard: string): Promise<void> => {
            setOpen(true);
    ...
    const openDialog = useCallback(
        (targetTable: string, targetShard: string): void => {
    ...
    const insertBatch = useCallback(
        async (rows: ReadonlyArray<Record<string, unknown>>, onDone: () => void): Promise<string | undefined> => {
        ...
        [client, table, shardKey, onRefresh],
    );
