...
const DataQueryBar = ({
    ...
    const applyByIndex = (event: MouseEvent<HTMLButtonElement>): void => {
    ...
    const deleteByIndex = (event: MouseEvent<HTMLButtonElement>): void => {
        const query = saved[Number(event.currentTarget.dataset["index"])];
        ...
        <div className="flex flex-wrap items-center gap-1.5" data-testid="db-query-bar">
            ...
            {saved.length > 0 && (
                <div className="flex flex-wrap items-center gap-1" data-testid="db-saved-queries">
                    <span className="font-mono text-[11px] tracking-wide uppercase text-muted-foreground">{t("Saved")}</span>
