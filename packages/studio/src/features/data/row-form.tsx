    ...
    return (
        <label className="flex flex-col gap-1 text-xs" htmlFor={`db-field-${column}`}>
            <span className="flex items-center gap-1.5 font-medium text-muted-foreground">
                ...
                {target !== undefined && <span className="rounded bg-muted px-1 text-[10px] tracking-wide uppercase">→ {target}</span>}
            </span>
...
const RowFormEditor = ({ documentText, onCancel, onDocumentTextChange, onSave, refs }: RowFormEditorProps): ReactElement => {
    const t = useT();
    const fields = useMemo(() => parseDocument(documentText), [documentText]);
    ...
    const showFormMode = useCallback((): void => {
        setRawMode(false);
    }, []);

    const showJsonMode = useCallback((): void => {
        setRawMode(true);
    }, []);
