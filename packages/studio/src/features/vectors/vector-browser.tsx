...
export const VectorBrowser = ({ loadIndexes, runQuery }: VectorBrowserProps = {}): ReactElement => {
    ...
    const selectedIndex = useMemo(() => indexes?.find((index) => index.name === selected) ?? null, [indexes, selected]);

    const onSelect = useCallback((event: React.MouseEvent<HTMLTableRowElement>): void => {
        setSelected(event.currentTarget.dataset.index ?? null);
        setMatches(null);
        setQueryError(null);
    }, []);

    const onQueryTextChange = useCallback((event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    }, [client, queryText, runQuery, selected]);
    ...
    return (
        <div className="flex flex-col gap-4" data-testid="lunora-vector-browser">
                ...
                <div className="rounded-md border border-border">
                    <Table data-testid="vector-table">
                        ...
                        <TableBody>
                            {indexes.map((index) => (
                                <TableRow
                                    className={index.name === selected ? "bg-muted/50" : "cursor-pointer"}
            ...
            {selectedIndex !== null && (
                <section className="rounded-md border border-border p-3" data-testid="vector-detail">
