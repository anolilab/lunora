...
const useStagedEdits = (): StagedEditsModel => {
    ...
    const stage = useCallback((rowId: string, column: string, value: unknown): void => {
        setStaged((previous) => {
    ...
    const stagedValue = useCallback(
        (rowId: string, column: string): undefined | { value: unknown } => {
    ...
    const count = useMemo<number>(() => Object.values(staged).reduce((sum, columns) => sum + Object.keys(columns).length, 0), [staged]);
...
const StagedDiffPanel = ({ changes, committing, onCommit, onDiscard }: StagedDiffPanelProps): ReactElement => {
    ...
    return (
        <div className="flex flex-col gap-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-3" data-testid="db-staged">
            <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-medium text-foreground" data-testid="db-staged-count">
