...
import type { TableInfo } from "./admin.js";
...
const FunctionDoc = ({ file, fn, kind }: FunctionDocProps): ReactElement => {
    ...
    const input = useMemo<SnippetInput>(() => {
        return { file, fn, kind };
    }, [file, fn, kind]);
    const tabLabel = useMemo<Record<SnippetTab, string>>(() => {
        return { cli: t("CLI"), client: t("Client"), react: t("React") };
    }, [t]);
...
const TableDoc = ({ name }: { readonly name: string }): ReactElement => {
    const t = useT();
    const code = useMemo(() => buildTableSnippet(name), [name]);
...
const ApiDocsPanel = ({ functions, initialShardKey }: ApiDocsPanelProps): ReactElement => {
    ...
    const tableNames = useMemo(() => (tables ?? []).map((table) => table.name).toSorted((a, b) => a.localeCompare(b)), [tables]);

    const selectFunction = useCallback((path: string): void => {
        setSelected({ kind: "fn", path });
    }, []);

    const selectTable = useCallback((name: string): void => {
        setSelected({ kind: "table", name });
    }, []);

    const selectedFunction = useMemo(
        () => (selected?.kind === "fn" ? functionList.find((descriptor) => descriptor.path === selected.path) : undefined),
        [selected, functionList],
    );
            ...
            <nav aria-label={t("API resources")} className="flex flex-col gap-4 overflow-y-auto" data-testid="api-rail">
                {grouped.map((group) => (
                    <div className="flex flex-col gap-1" key={group.file}>
                        <span className="px-2 text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">{group.file || t("(root)")}</span>
                ...
                {tableNames.length > 0 && (
                    <div className="flex flex-col gap-1">
                        <span className="px-2 text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">{t("Tables")}</span>
                        {tableNames.map((name) => (
                            <RailButton
