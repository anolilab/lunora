...
// Self-contained flat config for @lunora/flags. Each package owns its own
...
export default createConfig(
    ...
    // Scoped framework / Web-platform allowances (NOT blanket rule-off):
    {
        rules: {
            ...
            "n/no-unsupported-features/node-builtins": [
                "error",
                {
                    // Web-platform globals used by the browser push helper (`@lunora/notify/web`)
                    // and the edge runtime; eslint-plugin-n flags them against Node's version data.
                    ignores: ["crypto", "CryptoKey", "SubtleCrypto", "Storage", "sessionStorage", "localStorage", "navigator", "atob"],
                },
            ],
    ...
    // `null` is load-bearing here: D1 columns bind/read SQL NULL and the Web Push API's
    // `PushSubscription.expirationTime` is `number | null`. Matches @lunora/d1 & sql-store.
    {
        rules: {
            "unicorn/no-null": "off",
        },
