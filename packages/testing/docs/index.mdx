...
| `query(inline)`              | Run an inline `(ctx) => …` with a `QueryCtx`. Read-only in the type only — at runtime it holds the same writer `mutation` gets, so a cast can write (outside any transaction, and without notifying `subscribe`). |
...
| `t.scheduler.advance(ms, options?)` | Advance the virtual clock by `ms`, running every job due at or before the new now. Returns the count executed. |
| `t.scheduler.runPending(options?)`  | Run all pending jobs regardless of `scheduledFor`. Returns the count executed.                                 |
...
`t.scheduler.failures()`, matching production's dead-letter park.

Two things that catch tests out. A failure with retries left is **silently
re-enqueued** with exponential backoff on the virtual clock, so `failures()`
stays empty until the whole schedule has elapsed — 30s + 60s + 120s + 240s +
480s = 930s of virtual clock at today's defaults, not one tick. And once it does
land, `advance()` / `runPending()` **reject**: a terminal failure is re-thrown
verbatim (several become an `AggregateError`). Pass
`{ throwOnError: false }` to suppress the re-throw and inspect `failures()`
instead:

```ts
await t.scheduler.advance(1_000_000, { throwOnError: false });
