...
dropped, which matches production behaviour for unknown paths. Jobs run through the
...
built-in scorers are heuristic (`containsScorer`, `absentScorer`,
`regexScorer`, `exactMatchScorer`, `keywordScorer`) plus `llmScorer`, an
LLM-as-judge whose `judge` is **injected** — a `(prompt) => Promise<string>`
you wire to your own model — so this package stays model-agnostic and the
judge is mockable. `absentScorer` is the one that can express a refusal: "the
write was never dispatched", "the tool above this tier was never offered".
