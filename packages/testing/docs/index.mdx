...
helpers** re-exported from [`@lunora/mail/testing`](/docs/packages/mail) for
...
<Callout type="warn">
    The stub only throws when a handler actually touches it — functions that never reach for `ctx.storage` / `ctx.scheduler` / `ctx.vectors` / `ctx.fetch` still
    run. HTTP actions, scheduled-function draining, real R2 storage, `.global()` / D1 tables, and Vectorize are deferred to a follow-up.
</Callout>
...
- [@lunora/mail](/docs/packages/mail) — captures the outbound email the helpers read
- [@lunora/auth](/docs/packages/auth) — the verification / reset / magic-link flows you test end-to-end
