...
Every validator carries a runtime `parse` / `safeParse` plus a phantom `__type` field that `Infer<…>` reads to recover the TS type. Codegen ([`@cirrus/codegen`](../cirrus-codegen)) inspects the `kind` tag to render `dataModel.ts` and the typed `api.ts`.
...
No workspace dependencies. Usually consumed transitively via [`@cirrus/server`](../cirrus-server), which re-exports `v`.
