...
describe("createWorkflowRunner", () => {
    it("pOSTs to the scheduler dispatch endpoint with the admin bearer and returns the JSON result", async () => {
        ...
        const run = createWorkflowRunner({
            env: { CIRRUS_ADMIN_TOKEN: "secret", CIRRUS_ORIGIN_URL: "https://app.example.com/" },
            fetchImpl,
        });
        ...
        expect(url).toBe("https://app.example.com/_cirrus/scheduler/dispatch");
    ...
    it("requires CIRRUS_ORIGIN_URL", async () => {
        const run = createWorkflowRunner({ env: { CIRRUS_ADMIN_TOKEN: "secret" }, fetchImpl: async () => okResponse("") });

        await expect(run(ref("a:b"))).rejects.toThrow(/CIRRUS_ORIGIN_URL/);
    });

    it("requires CIRRUS_ADMIN_TOKEN", async () => {
        const run = createWorkflowRunner({ env: { CIRRUS_ORIGIN_URL: "https://app.example.com" }, fetchImpl: async () => okResponse("") });

        await expect(run(ref("a:b"))).rejects.toThrow(/CIRRUS_ADMIN_TOKEN/);
    });
});
...
describe("createWorkflowRunContext", () => {
    it("assembles the handler context with params, step, env, and a runner", () => {
        ...
        const ctx = createWorkflowRunContext({ env: { CIRRUS_ORIGIN_URL: "x" }, event, exportName: "orderPipeline", step });

        expect(ctx.params).toEqual({ orderId: "o1" });
        expect(ctx.event).toBe(event);
        expect(ctx.step).toBe(step);
        expect(ctx.env).toEqual({ CIRRUS_ORIGIN_URL: "x" });
