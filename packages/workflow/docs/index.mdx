...
with the first application's result instead of applying again.
...
shard's single-writer gate. An action or query gets the weaker form: a
_sequential_ replay short-circuits, but two genuinely concurrent dispatches of
one id can both miss the cache and both run.
