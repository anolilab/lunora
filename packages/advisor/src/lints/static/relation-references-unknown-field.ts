 ...
 */
const missingColumns = (byName: Map<string, AdvisorTable>, holder: AdvisorTable, relation: AdvisorRelation): ColumnProblem[] => {
    ...
    if (!hasColumn(fkTable, relation.field)) {
        problems.push({ column: relation.field, owner: fkTable.name, side: "field" });
    }

    if (!hasColumn(referencedTable, relation.references)) {
    ...
    run: (context) => {
        ...
        return context.schema.tables.flatMap((table) =>
            table.relations.flatMap((relation) =>
                missingColumns(byName, table, relation).map((problem) =>
