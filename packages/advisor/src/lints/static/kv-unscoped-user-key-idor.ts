...
const kvUnscopedUserKeyIdor: Lint = {
    ...
    run: (context) => {
        ...
        return context.kvKeyAccesses.map((access) =>
            emit(kvUnscopedUserKeyIdor, {
                cacheKey: `kv_unscoped_user_key_idor:${access.file}:${access.line.toString()}`,
                detail: `\`ctx.kv.${access.method}\` in \`${access.exportName}\` (${access.file}:${access.line.toString()}) uses a KV key derived from \`args\` with no server-side scoping — any caller can read/overwrite/delete another user's entry (IDOR). Prefix the key with a server-trusted identity (e.g. \`\${ctx.auth.userId}:…\`).`,
