...
const map = scoreAdvisor(context, runAdvisor(context, { source: "static" }));
...
each run. `compareToBaseline` reports four independent regression signals: the
global score fell, an existing procedure got worse, a procedure started failing,
or the project bucket gained rules.
