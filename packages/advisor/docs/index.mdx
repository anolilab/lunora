...
`tableSamples` off the `LintContext`. The Studio backend fills `shardTraffic`,
`tableScans`, and `indexHits` from the shards' admin signal.

`tableSamples` has **no shipped feeder**. Nothing in the runtime or the Studio
reads bounded row samples out of a shard, so `constraint_validator` is a no-op
unless the caller supplies samples of its own — the Studio drops it from the
lint set it runs rather than running it against an input it cannot fill. The
lint and its `AdvisorTableSample` input shape are exported, so you can drive it
from your own sampling read; there is no built-in one to turn on.
