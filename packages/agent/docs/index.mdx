...
`agents:agentPatchThread`) plus the public thread queries (`agents:agentMessages`,
`agents:agentThread`), and wires the typed `ctx.agents.<name>` producer.
...
persisted history each turn. Only one run should be active per thread at a
time: two concurrent runs on the same key would interleave their messages into
one history (they share the thread's sequence counter).
