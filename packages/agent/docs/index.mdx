...
> **`activeTools` foot-gun.** The memory tools are ordinary tools — subject to
> `activeTools`, `toolChoice`, `stopWhen`, and bounded by `maxTurns`. If you set
> `activeTools`, you **must** include the memory tool name (e.g. `searchMemory`)
> or the model can never retrieve.
