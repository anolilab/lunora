...
## Sandbox tools (batteries-included browser + containers)

Two batteries-included tools give an agent a **headless browser** and a
**sandboxed container** with minimal glue. Import them from `@lunora/agent` (or
the `@lunora/agent/sandbox` subpath) and drop them into `tools` — codegen does
the rest:
