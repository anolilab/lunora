...
- **`telemetry`** — an ai@7 `TelemetryOptions` handed to every turn; set
  ...
  and the dependency-injected `sentryTelemetry` / `braintrustTelemetry` bridges
...
`consoleTelemetry` and `combineTelemetry` have zero runtime dependencies; install
`@sentry/cloudflare` / `braintrust` only if you use those bridges.
