...
import renderDashboardHtml from "../src/render-html.js";

describe("renderDashboardHtml", () => {
    it("injects the basepath and references the given asset URLs", () => {
        expect.assertions(4);

        const html = renderDashboardHtml({ basePath: "/__cirrus", scriptSrc: "/__cirrus/dashboard.js", styleHref: "/__cirrus/styles.css" });

        expect(html).toContain('window.__CIRRUS_BASE_PATH__="/__cirrus";');
        expect(html).toContain('src="/__cirrus/dashboard.js"');
    ...
    it("injects the admin token when provided, escaping `<` for safe inline embedding", () => {
        ...
        const html = renderDashboardHtml({ adminToken: "tok<en", basePath: "/", scriptSrc: "/dashboard.js", styleHref: "/styles.css" });
