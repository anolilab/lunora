...
React 18 / 19 hooks for the Cirrus framework. Wraps a [`@cirrus/client`](../cirrus-client) instance with Suspense-friendly hooks that subscribe via `useSyncExternalStore`, so concurrent rendering, transitions, and selective hydration all behave correctly.
...
Workspace dependency: [`@cirrus/client`](../cirrus-client). Peer deps: `react` and `react-dom` (`^18 || ^19`).
