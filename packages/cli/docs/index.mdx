...
```bash
lunora doctor --format json | jq '.checks[] | select(.level == "error")'
lunora migrate status backfill-names --format json | jq '.result'
lunora env diff --format json | jq -r '.localOnly[]'
```
