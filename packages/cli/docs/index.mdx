...
Every `lunora dev` start writes two files into the gitignored `.lunora/`, and
names both in its startup banner:

| File                        | Carries                                                            |
| --------------------------- | ------------------------------------------------------------------ |
| `.lunora/dev.json`          | live state — URL, PID, and `readyAt` once the server first answers |
| `.lunora/dev-bindings.json` | the binding manifest, plus the origin it serves on                 |
...
`--emit-bindings <file>` moves the manifest elsewhere, and naming a path also
makes a derivation failure fatal — a named path means something is waiting on it.
...
These files are deterministic, so commit them. CI re-runs codegen and diffs
