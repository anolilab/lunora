...
`install` writes two servers: **`lunora-docs`**, the hosted documentation server
at `https://lunora.sh/mcp` (no credentials), and — inside a Lunora project —
**`lunora`**, which runs `lunora mcp serve` locally. It knows each client's own
