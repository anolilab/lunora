...
`LUNORA_ADMIN_TOKEN`, then `.dev.vars` — the last only for a loopback target, so
a dev secret is never sent to a deployed worker. Against your own dev server that
means no flags at all.
...
A bucket-backed snapshot is uploaded as one body, so it has to fit in the CLI's
memory: above 256 MiB `create --bucket` refuses and names the workaround
(`--tables`, or `--dir` plus `wrangler r2 object put`).
