---
name: cirrus-setup-mail
description: Adds transactional email to a Cirrus app. Use for sending mail (verification, password reset, invites, notifications) via `cirrus registry add mail`, the `sendEmail` / `queueEmail` actions, the `SEND_EMAIL` Cloudflare Email Workers binding, Resend, React email templates, and the dev mail catcher.
---

# Cirrus Setup Mail

Wire transactional email into a Cirrus app using the `mail` registry item, which
is built on `@cirrus/mail` (a Cloudflare Email Workers transport with
...
- Delivering verification / password-reset mail from `@cirrus/auth`.
...
1. Adds `@cirrus/mail` and `@cirrus/server` to `package.json` (run
   `pnpm install` afterwards).
2. Copies `cirrus/mail/index.ts` (the `sendEmail` / `queueEmail` actions) into
...
| `RESEND_API_KEY` | secret (optional)                    | Use a hosted provider instead — pass `apiKey` to `createMailer` in `cirrus/mail`. |
...
`cirrus/mail/index.ts`.

In `cirrus dev` (`WORKER_ENV=development`) the scaffold swaps in `@cirrus/mail`'s
**capture transport** automatically: every send — including `@cirrus/auth`'s
...
where you call the mailer — edit `cirrus/mail/index.ts` to pass `react` straight
into `mailer.send`:
...
3. **Using `queueEmail` without a Queue binding.** It requires a Cloudflare
   Queue producer binding; until you add one, `@cirrus/mail` throws
...
- [ ] `cirrus registry add mail` run, `pnpm install` done.
- [ ] `SEND_EMAIL` binding configured (verified destination) or
      `RESEND_API_KEY` set; `MAIL_FROM` set.
- [ ] `cirrus codegen` run so `api.mail.*` is generated.
