---
name: cirrus-setup-scheduler
description: Schedules deferred and recurring work in a Cirrus app. Use for `ctx.scheduler.runAfter` / `runAt` (delayed function dispatch), cron jobs via `cirrus registry add crons` (the `cronJobs()` builder), the `SchedulerDO` / `SCHEDULER` binding, retries, and the workpool for bounded concurrency.
---

# Cirrus Setup Scheduler

Schedule work in Cirrus two ways, both backed by `@cirrus/scheduler` (re-exported
from `@cirrus/server`):
...
- **Recurring jobs (crons)** — declare them in `cirrus/crons.ts` with
  `cronJobs()`. Add the starter with `cirrus registry add crons`.
...
- You just need to react to live data changes — use a reactive `query` /
  subscription (`cirrus-realtime`), not a scheduled job.
...
- The `SchedulerDO` binding (`SCHEDULER`) is **auto-inferred and reconciled**
  into `wrangler.jsonc` by `@cirrus/config` once `@cirrus/scheduler` is in use —
  run `cirrus codegen` / `cirrus doctor` to confirm. Scheduled jobs run with no
...
This adds `@cirrus/server` to `package.json` (run `pnpm install`) and copies
`cirrus/crons.ts` (a `cronJobs()` registry with one illustrative job) and
`cirrus/crons/jobs.ts` (the example `run` internal mutation it fires) into your
...
expression, and emits `cirrus/_generated/crons.ts` (the dispatcher the Worker's
...
```ts
import { createWorkpool } from "@cirrus/scheduler";
...
3. **Forgetting `cirrus codegen` after editing crons.** The `triggers.crons`
...
- [ ] `SCHEDULER` (SchedulerDO) binding present in `wrangler.jsonc`
      (`cirrus doctor` clean) when using the scheduler.
- [ ] Recurring jobs declared in `cirrus/crons.ts` via `cronJobs()` (after
      `cirrus registry add crons`).
- [ ] `cirrus codegen` run so `_generated/crons.ts` + `triggers.crons` are
      synced.
