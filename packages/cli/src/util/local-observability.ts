...
const queryLocalObservability = async (sql: string, options: LocalObservabilityOptions = {}): Promise<ExplorerResponse["result"]> => {
    ...
    if (payload.success === false) {
        // `flatMap` rather than `map(...).filter(Boolean)`: one pass, and it drops
        // an error entry with no message instead of joining an empty string in.
        const detail = payload.errors?.flatMap((entry) => (entry.message ? [entry.message] : [])).join("; ");
