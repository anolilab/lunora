/**
 * Branded Lunora welcome page for the create-vite overlay.
 *
 * A stock `create-vite` base ships a framework "Get started / Edit App" splash.
 * The overlay replaces it with the same branded welcome the bespoke templates
 * render — a full-bleed hero (logo, doc/blog/studio/ecosystem cards, a
 * light/dark toggle) — so an overlaid project looks like a Lunora app on first
 * load, not a generic Vite starter.
 *
 * Two pieces per framework. {@link WELCOME_CSS} is the scoped stylesheet,
 * written over whichever base stylesheet the framework's entry already imports
 * (so there's no second import and create-vite's conflicting body/centering
 * rules are replaced). The App component (or, for vanilla, the bootstrap)
 * renders the markup.
 *
 * The markup is faithful to the bespoke templates' shared welcome stylesheet +
 * their welcome routes; the only per-framework differences are the syntax (JSX
 * `className` vs template `class`, inline-style object vs string, the toggle's
 * event wiring) and the footer's framework label.
 */

/* eslint-disable no-secrets/no-secrets -- emitted starter markup (logo path data, scoped class names, mock-UI bars) reads as high-entropy but contains no credentials */

/** The Lunora logo glyph path — deduped so it isn't repeated across the five components. */
const LOGO_PATH =
...
/**
 * The scoped welcome stylesheet. Written over the base stylesheet each
 * framework's entry already imports (`src/index.css`, `src/style.css`,
 * `src/app.css`) so create-vite's body/centering rules don't fight the
 * full-bleed hero. Everything is scoped under `.lunora-welcome` (collision-safe).
 */
...
export { REACT_APP, SOLID_APP, SVELTE_APP, VANILLA_MAIN, VUE_APP, WELCOME_CSS };
