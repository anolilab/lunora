...
/**
 * `cirrus add &lt;feature>` — add a feature or registry item to an existing
 * project. A thin, discoverable front door over `cirrus registry add`: the
...
const addCommand: Command = {
    ...
    description: "Add a feature or registry item (auth, email, storage, crons, …) to the current Cirrus project",
    examples: [
        ["cirrus add auth", "Add authentication (asks which provider)"],
        ["cirrus add auth --provider clerk", "Add Clerk auth without prompting"],
        ["cirrus add email", "Add transactional email (Cloudflare Email Workers + dev mail catcher)"],
        ["cirrus add storage", "Add the R2 storage registry item"],
        ["cirrus add crons", "Add the scheduled-jobs registry item"],
    ],
    group: "Project",
    loader: () =>
