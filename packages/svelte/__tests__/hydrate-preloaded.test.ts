import type { CirrusClient, Preloaded } from "@cirrus/client";
...
/**
 * A minimal stand-in for the parts of `CirrusClient` the adapter touches.
...
describe(hydratePreloaded, () => {
    ...
    it("attaches the live subscription on subscribe and re-emits deltas", () => {
        ...
        expect(subscribe.mock.calls[0]?.[0]).toEqual({ __cirrusRef: "messages:list" });
