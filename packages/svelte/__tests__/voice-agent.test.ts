...
describe(voiceAgent, () => {
    ...
    it("ends the call when the signed-in identity changes", async () => {
        const { handle, mic, setAuthToken, socket } = renderVoice();
        ...
        expect(get(handle.status)).toBe("idle");
        expect(get(handle.error)?.message).toMatch(IDENTITY_CHANGED);
        expect(micStop).toHaveBeenCalledTimes(1);
        expect(socketClose).toHaveBeenCalledTimes(1);
    });
    ...
    it("keeps the call alive through a same-subject token refresh", async () => {
        ...
        expect(get(handle.status)).not.toBe("idle");
        expect(get(handle.error)).toBeUndefined();
    ...
    it("releases the identity watch when the call ends", async () => {
        const { handle, setAuthToken } = renderVoice();
