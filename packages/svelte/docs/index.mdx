...
<script lang="ts">
    ...
    // `room` is a plain handle, not a store — `present` is the readable on it.
    const { present } = room;
    onDestroy(room.teardown);
</script>

{#each $present ?? [] as p (p.userId)}<span>{p.data?.name}</span>{/each}
```
...
`auth` returns `{ status, user, token, setToken }`: `status`, `user` and `token`
are readable stores, and `setToken(jwt)` sets the token on the client after
sign-in. `connectionStatus` returns a readable store of the aggregate
live-socket state (`idle` → `connecting` → `connected` → `offline`), for driving
a connection indicator.

`$status` is the auth gate — `"loading"`, `"authenticated"`, `"unauthenticated"`
or `"unreachable"` (a credential is held but the identity endpoint could not be
reached; gates as authenticated). Branch on it, not on `$user`: `$user` is null
both when signed out and when a held credential's identity has not resolved, so
a `$user` gate shows the sign-in prompt to a signed-in user on an offline
reload. See the [`@lunora/client` reference](/docs/packages/client) for the
table. `authGate()` returns `{ isAuthenticated, isLoading }` derived from the
same `status`, when a boolean is all a template needs.

```svelte
<script lang="ts">
    import { auth, connectionStatus } from "@lunora/svelte";

    const { setToken, status: authStatus, user } = auth();
    const connection = connectionStatus();
</script>

{#if $authStatus === "loading"}Checking…{:else if $authStatus === "unauthenticated"}Signed out{:else}Signed in as {$user?.name}{/if}
<span data-status={$connection}>{$connection}</span>
```
