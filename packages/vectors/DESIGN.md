...
export const searchDocs = query({
    args: { q: v.string() },
    handler: async (ctx, { q }) => {
        const matches = await ctx.vectors
            .indexFor("docs", "body")
            .query({ input: q, embed, topK: 10 });
...
- Closer to Cloudflare's own mental model (a Vectorize index *is* a top-level resource with its own dimensions/metric)
