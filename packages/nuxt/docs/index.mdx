...
Nuxt runtime. `h3` is also an optional peer (`^1.0.0 || ^2.0.0`); see
...
<Callout type="info">
    **h3 version.** The peer range is `^1.0.0 || ^2.0.0`, and the request handler feature-detects both at runtime. If you pin an explicit `h3` dependency
    yourself, pin it to `^1.15.11` — npm's `h3@2.0.0` is a deprecated stub with no code, exports, or type declarations (the real v2 line is still
    prerelease-only), while Nuxt 4 / Nitro 2 run h3 v1 at runtime anyway.
</Callout>
