...
KV namespace bindings are user-defined, so `@lunora/kv` is opt-in: add a `kv_namespaces` entry to your `wrangler.jsonc`, then import `@lunora/kv` (or read `ctx.kv`) from a `lunora/` source. Codegen then wires a typed `ctx.kv` onto every context — queries, mutations, and actions — backed by the `KV` binding (`env.KV`) by default.
...
- [`@lunora/server`](https://www.npmjs.com/package/@lunora/server) — read `ctx.kv` from queries, mutations, and actions.
