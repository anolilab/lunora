...
export const createKv = (options: CirrusKvOptions): Kv => {
    ...
    const list = async <M = unknown>(listOptions: KvListOptions = {}): Promise<KvListResult<M>> => {
        ...
        // scoped instance only ever lists its own keys.
        ...
        const limit = listOptions.limit === undefined ? undefined : Math.min(Math.max(1, Math.floor(listOptions.limit)), MAX_LIST_LIMIT);
