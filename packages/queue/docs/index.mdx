...
One case cannot be saved. If the decline lands on the message's **last** delivery, the retry takes it to the `deadLetterQueue`, or drops it if the queue has none. The consumer logs an error when this happens. Give a queue that calls slow actions a `maxRetries` of at least 2, plus a `deadLetterQueue`.
