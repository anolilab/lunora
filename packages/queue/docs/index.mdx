...
Cloudflare Queues has no retry that does not count against `maxRetries`, so a decline still costs one attempt. What the consumer bounds is how many it costs. A declined message is not rethrown into an immediate redelivery, which would meet the same decline again. It is retried with a 15-minute delay, the longest the shard's claim can stand. So one slow call costs at most one extra attempt. The next delivery is then served the finished result from the replay cache, or runs the call if the first run died. A declined message is never acked, so delivery stays at-least-once.
