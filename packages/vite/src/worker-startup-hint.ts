...
/** Matches the runner-worker bundle path in an error stack. */
const RUNNER_WORKER_RE = /runner-worker[/\\]index\.js/u;
/** Matches the export-types probe function name in an error stack. */
...
const wrapHook = (hook: unknown): unknown => {
    ...
    if (hook !== null && typeof hook === "object" && "handler" in hook && typeof (hook as Record<string, unknown>)["handler"] === "function") {
...
export { augmentWorkerStartupError, isWorkerEntryEvalError, withWorkerStartupHint, WORKER_STARTUP_HINT };
