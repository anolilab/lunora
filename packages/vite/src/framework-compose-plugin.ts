 ...
 *
 * `import` is the module specifier the entry imports; `handler` is a JS
 * expression (evaluated in the generated module's scope, where `ssrModule` is
 * the namespace import of `import`) that yields an `HttpRouterLike`
 * (`{ fetch(request, env?, ctx?) }`). Kept as data — not codegen branches — so
 * the set of supported class-A frameworks is one readable table.
 *
...
interface ClassAWiring {
    /** JS expression yielding the `httpRouter` ({ fetch }) from the imported `ssrModule` namespace. */
    handler: string;
    /** Module specifier the generated entry imports the SSR build/handler from. */
    import: string;
}
...
const CLASS_A_WIRING: Readonly<Partial<Record<DetectedFramework, ClassAWiring>>> = {
    "react-router": {
        ...
        // is exactly the `httpRouter.fetch` contract.
    ...
    "solid-start": {
        ...
        import: "@solidjs/start/server-handler",
    },
    "tanstack-start": {
...
const buildWorkerEntrySource = (framework: DetectedFramework, generatedImportBase: string): string => {
    ...
    const reactRouterImport = framework === "react-router" ? 'import { createRequestHandler } from "react-router";\n' : "";
    // Solid/TanStack reference `ssrModule.default`; React Router builds its
    // handler inline from `createRequestHandler`, so it needs no namespace import.
    const ssrImport = framework === "react-router" ? "" : `import * as ssrModule from "${wiring.import}";\n`;
...
${reactRouterImport}${ssrImport}import { CIRRUS_FUNCTIONS } from "${generatedImportBase}/functions.js";
