...
const buildWorkerEntrySource = (
    ...
    // Same rule, same reason, one feature later: `defineAgent` compiles onto a
    // Cloudflare Workflow, and wrangler refuses to deploy a `workflows[].class_name`
    // the worker does not export. A class-A app has no hand-written entry to add
    // the re-export to, so without this an app that declares an agent deploys with
    // nothing to run — which is exactly what codegen warns about.
    const agentsReexport = hasAgents ? `\nexport * from "${base}/agents";\n` : "";
...
${containersReexport}${agentsReexport}
    ...
    return {
        load(id) {
            if (id === RESOLVED_LUNORA_WORKER_ID && isAutoComposable(context) && context.framework !== undefined) {
                ...
                // Same fs-existence rule as containers: `_generated/agents.ts` is
                // emitted only when the project declares an agent.
                const hasAgents = existsSync(join(generatedImportBase, "agents.ts"));
