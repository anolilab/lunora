...
- `allowedHosts: [...]` is a strict allowlist that refuses any hostname not exactly on the list (case-insensitive, trailing-dot- and IPv6-bracket-normalized). This is the only guard that _fully_ closes rebinding, so prefer it whenever you can enumerate the destinations.
