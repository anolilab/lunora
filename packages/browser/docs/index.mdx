...
free of it deliberately. So codegen emits `config.browser ? config.browser(env)
: browserStub`, and `browserStub` throws a directed error on every method. Pass
