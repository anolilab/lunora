...
- `resolveDns` — **on by default when `allowedHosts` is unset**: a DoH re-check that resolves the hostname over Cloudflare DNS and refuses if any A/AAAA record is private. Best-effort (it adds a round-trip and is TOCTOU-imperfect; it falls back to the string guard if the lookup fails). Setting `allowedHosts` turns it off by default — the allowlist is the stronger guard and may legitimately name an internal host that a resolved-address check would refuse; pass `resolveDns: true` to run both. Set `resolveDns: false` for trusted, non-user-controlled URLs where the round-trip matters.
