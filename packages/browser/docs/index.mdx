...
| `resolveDns`          | `boolean`            | Best-effort DNS-rebinding re-check: resolve the hostname over Cloudflare DoH and refuse private results. Default false.   |
...
When the navigation URL is **user-controlled**, harden further against DNS rebinding — a public hostname that resolves to a private/metadata IP passes the string guard above. Two options:
...
- `resolveDns: true` — an opt-in DoH re-check that resolves the hostname over Cloudflare DNS and refuses if any A/AAAA record is private. Best-effort (it adds a round-trip and is TOCTOU-imperfect; it falls back to the string guard if the lookup fails), so reach for `allowedHosts` when you need a hard guarantee.
