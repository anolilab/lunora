...
// Self-contained flat config for @lunora/platform-rivet. Each package owns its own
...
export default createConfig(
    ...
    // One adapter per file (rivet-kv-store.ts, rivet-shard-directory.ts,
    // conformance/index.ts, …), each with a single named export.
    // `import/prefer-default-export` would push those to a default export, but
    // we keep uniform named imports, matching the repo convention and
    // CLAUDE.md's no-mixed-default rule (see cli/studio/x402).
    {
        rules: {
    ...
    // The conformance smoke tests register every `it` dynamically through
    // defineHostContractSuite's / defineEngineContractSuite's injection;
    // sonarjs cannot see through that and reports the file as empty at
    // position 0:1, which no inline disable reaches.
    {
        files: ["**/__tests__/conformance.test.ts", "**/__tests__/engine-conformance.test.ts"],
        rules: {
