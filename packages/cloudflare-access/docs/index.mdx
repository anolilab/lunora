...
`@lunora/cloudflare-access/context` ships `accessContext()`, a middleware that attaches a typed, **synchronous** `ctx.access` facade derived from the verified identity — so a handler reads `ctx.access.email`, `ctx.access.groups`, `ctx.access.hasGroup("ops")`, or the full `ctx.access.claims` without an `await` or a cast off the generic `ctx.auth.getIdentity()` envelope.
...
When the request is anonymous it attaches the anonymous facade — `authenticated: false`, empty `groups`, `hasGroup` always `false` — so reads stay safe with no null check and authorization still fails closed. It only **surfaces** the identity; pair it with `rls(...)` (or `accessRoles(...)` → `rls(...)`) when you need enforcement.
...
- `accessContext()` _(`@lunora/cloudflare-access/context`)_ — middleware attaching a typed, synchronous `ctx.access` facade over the verified identity.
