...
Only a **verified Access identity** ever contributes roles. Under `composeResolvers` the identity on a request may have been minted by another adapter (better-auth, a custom SSO mapping), and a `groups` field on such an envelope is app data Access never verified — a profile field a user might edit. `accessRoles` recognises an Access identity by the verified claim set under `access` (the same test `ctx.access` applies) and forwards the context unchanged for anything else, so a foreign `groups` can never become an RLS role. That holds for a custom `readGroups` too: it changes where the groups are read from, never whether Access verified them.
