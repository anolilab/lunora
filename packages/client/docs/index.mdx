...
> **Cache-keyed args are the exception.** A subscription / `useQuery` / shape arg
> doubles as the reactive **cache key**, which must be a stable string — so
> `bigint`, bytes, and `Date` **can't** appear there yet (you get a `TypeError`
> telling you to pass a string / number / epoch-ms instead). Mutation and action
> args, and all results, have no such limit. (Full support is a tracked reactive-
> engine change — see `plans/090-subscription-arg-wire-fidelity.md`.)
