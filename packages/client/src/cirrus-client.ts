...
/**
 * One WebSocket per distinct shard key. `.shardBy()` routes each shard to its
 * own Durable Object, so subscriptions must ride the socket connected to that
 * shard (via `?shard=`) — otherwise writes on one shard never reach
 * subscribers parked on another (e.g. the default `__root__`).
 */
interface ShardConnection {
    pendingUnsubscribes: string[];
    readonly reconnect: ReconnectCalculator;
...
export class CirrusClient {
    ...
    /** One {@link ShardConnection} per distinct shard key (default keyed `""`). */
    ...
    public async mutation<F extends FunctionReference>(fn: F, args: ArgsOf<F>, opts: MutationCallOptions<unknown, ReturnOf<F>> = {}): Promise<ReturnOf<F>> {
        ...
        // the resubscribe. Everything is scoped to the target shard's socket.
        ...
        // Only queue once this shard has connected before — i.e. its WS dropped.
        // Before the first connection, mutations travel over HTTP so they don't
        // deadlock when no consumer has triggered a subscription on this shard.
    ...
    /** Map a shard key to its connection-map key (the default shard is `""`). */
    ...
    /** WS URL for a shard — appends `?shard=` so the runtime routes the upgrade. */
    ...
    private ensureSocket(shardKey: string | undefined): void {
        ...
        socket.onopen = (): void => {
            ...
            // Resubscribe everyone on this shard.
    ...
    private sendSubscribeIfOpen(state: SubscriptionState): void {
        ...
        if (!conn || conn.wsState !== "open" || state.acked) {
            return;
        }
        ...
        // `functionPath` drives the server's re-execute-and-push path; `table`
        // remains for legacy delta matching.
