...
- `onWriteRejected?` — fires once per **permanently** rejected write, after its
  optimistic row is rolled back (see [Surfacing rejected writes](#surfacing-rejected-writes)).
