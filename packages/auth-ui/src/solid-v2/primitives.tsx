 ...
 *
 * `keyed={false}` (Solid 1.x spelled this `<Index>`): the rows have no identity
 * to key on, and a keyed `<For>` would try to diff a list of indistinguishable
 * placeholders.
 */
const Skeleton = (props: { rows?: number }): JSX.Element => (
    <div aria-hidden="true" class="lunora-auth-skeleton">
        <For each={Array.from({ length: props.rows ?? 3 })} keyed={false}>
            {() => <span class="lunora-auth-skeleton__row" />}
        </For>
    </div>
);
    ...
    return (
        <Show when={props.value !== ""}>
            <div class="lunora-auth-strength">
                ...
                {/*
                 * `keyed={false}` (Solid 1.x spelled this `<Index>`): the policy
                 * fixes the rules and their order, so a row's identity is its
                 * position — what changes as the user types is the tick, not the
                 * list. Keyed `<For>` would rebuild every row on each keystroke,
                 * because the requirement objects are rebuilt with it.
                 */}
                <ul aria-live="polite" class="lunora-auth-strength__list">
                    <For each={requirements()} keyed={false}>
                        {(requirement) => (
                            <li class={["lunora-auth-strength__item", { "lunora-auth-strength__item--met": requirement().met }]}>
