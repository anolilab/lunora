...
// Start a hosted checkout for the signed-in user.
...
Webhook route (raw body is required for signature verification):

```ts
routes: {
    "POST /payment/webhook": async (request, env) => payment.handleWebhook(request),
}
```
