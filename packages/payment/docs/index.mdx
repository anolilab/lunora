...
description: Provider-agnostic payments for Lunora — Stripe/Polar/Autumn/Dodo Payments adapters, webhook sync, and a payment/subscription state machine.
---

`@lunora/payment` is one provider-agnostic API over Stripe, Polar, Autumn, and Dodo Payments. Webhook
...
| Option          | Type                 | Notes                                                                                                                    |
| --------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `adapter`       | `PaymentAdapter`     | Built with `createStripeAdapter` / `createPolarAdapter` / `createAutumnAdapter` / `createDodoPaymentsAdapter`. Required. |
...
Every adapter takes its client by injection and verifies inbound webhooks:

```ts
import { createAutumnAdapter, createDodoPaymentsAdapter, createPolarAdapter, createStripeAdapter } from "@lunora/payment";
...
| Option                    | Stripe | Polar | Autumn | Dodo | Notes                                                                                              |
| ------------------------- | :----: | :---: | :----: | :--: | -------------------------------------------------------------------------------------------------- |
| `client`                  |   ✓    |   ✓   |   ✓    |  ✓   | Structural `StripeClientLike` / `PolarClientLike` / `AutumnClientLike` / `DodoPaymentsClientLike`. |
...
Stripe is a PSP (manual authorize/capture available). Polar and Dodo Payments
are Merchants-of-Record — `merchantOfRecord` is `true` and they own tax/invoices,
so manual capture isn't part of their flow. Stripe webhooks use Stripe's
signature scheme; Polar, Autumn, and Dodo Payments use Standard Webhooks
...
throw. It keys a subscription on the `(customerId, productId)` pair, which the
adapter encodes as the composite id `"<customerId>::<productId>"`.
...
`listBalances` straight to Autumn's live API — you do **not** need to configure
...
Autumn-native concepts that have no cross-provider equivalent — **entities**
(per-seat/per-workspace sub-customers), **referrals**, usage-**events** analytics,
the **product catalog**, and a native **checkout** (free trials, prepaid feature
quantities, entity scoping, reward codes) — live in a companion facade,
`createAutumnFeatures`, kept separate so the provider-agnostic surface stays
generic. Share the same injected client between the two. (Credit systems need no
extra method — Autumn models them as features, so their balances flow through
`check` / `listBalances` like any other.)

```ts
import { createAutumnFeatures } from "@lunora/payment";

const autumnFeatures = createAutumnFeatures({ client });
