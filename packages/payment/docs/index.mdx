...
the **plan catalog**, and a native **checkout** (prepaid feature quantities, entity
scoping, reward codes, opting out of a plan's trial) — live in a companion facade,
...
`ctx.db.get` returns the right `bigint` — but it is invisible to comparison:
...
form are skipped rather than rewritten. Repeat for any other table of yours
declaring a `v.bigint()` or `v.bytes()` column — the id is always
