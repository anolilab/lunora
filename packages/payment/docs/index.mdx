...
| `entitlements`  | `EntitlementsConfig` | Plan → features/limits map. Required for a `featureId` check and for `listBalances` on providers without native entitlements; a `priceId` check needs none. |
...
| `refundPayment(input)`                         | `PaymentSession`   | Refund the caller's own captured payment, in full or in part; persists the result. |
| `capturePayment(input)`                        | `PaymentSession`   | Capture the caller's own authorized payment; persists the result.                  |
| `cancelPayment(sessionId, options?)`           | `PaymentSession`   | Cancel the caller's own uncaptured payment; persists the result.                   |
...
<Callout type="warn">
    `ctx.payments.adapter` is the raw provider client, exposed for the surfaces the facade does not wrap (`updateSubscription`, `resumeSubscription`,
    provider-specific extras). **Do not move money through it.** `adapter.refundPayment` / `capturePayment` / `cancelPayment` skip everything the facade methods
    above do first: the caller authorization, the over-refund guard, the local refund ledger (the only thing stopping a retry from issuing a second real refund
    on a provider whose API takes no idempotency key), the derived idempotency key, and the store write. The facade ends in the same provider call.
</Callout>
...
export const recordApiCall = action.action(async ({ ctx }): Promise<{ recorded: boolean }> => {
    // `mode: "add"` (default) increments; `"set"` reconciles the LOCAL period total.
    ...
    // `"set"` throws on a provider that meters usage upstream — see the callout below.
...
<Callout type="warn">
    **`mode: "set"` is local-only, and throws where the provider meters.** Provider meters are additive: they take events, not period totals, so a `"set"` can
    only be forwarded as a delta against the total just read — and a `"set"` that LOWERS usage has no negative delta to send. `set 10 → set 5 → set 8` would
    forward `10` then `3`, billing 13 upstream against a local period total of 8. Rather than over-bill silently, `track` rejects `"set"` with a
    `VALIDATION_ERROR` on any provider whose `capabilities.usageMetering` is `true` (Stripe, Polar, Dodo, Autumn). Use `mode: "add"`, which is exact on both
    sides. `"set"` stays available where usage is enforced locally.
</Callout>
