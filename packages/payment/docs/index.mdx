...
manual capture isn't part of its flow. Autumn ([useautumn.com](https://useautumn.com))
is an entitlement-first billing layer that runs on **your own** Stripe account,
so it is _not_ a Merchant-of-Record, but — like Polar — it abstracts the money
movement, so manual authorize/capture/refund throw. Autumn keys a subscription
on the `(customerId, productId)` pair, which the adapter encodes as the
composite id `"<customerId>::<productId>"`. Stripe webhooks use Stripe's
signature scheme; Polar and Autumn use Standard Webhooks (`webhook-id` /
