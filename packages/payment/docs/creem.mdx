...
`createCreemAdapter` surface and Creem's [official docs](https://docs.creem.io).
...
```ts
import type { CreemClientLike } from "@lunora/payment";
...
```ts
import { createCreemAdapter } from "@lunora/payment";
import { Creem } from "creem";
...
- **No manual capture / payment cancellation.** `capturePayment` and `cancelPayment` throw — Creem captures at checkout and owns the payment lifecycle.
