...
```ts
import type { AutumnClientLike } from "@lunora/payment";
import { createAutumnAdapter } from "@lunora/payment";
...
**product catalog**, and a native **checkout** (free trials, prepaid feature
quantities, entity scoping, reward codes). These live in a companion facade,
...
```ts
import { createAutumnFeatures } from "@lunora/payment";

const autumnFeatures = createAutumnFeatures({ client: client as unknown as AutumnFeaturesClientLike });

await autumnFeatures.entities.create(userId, { featureId: "seats", name: "Workspace A" });
const usage = await autumnFeatures.events.aggregate(userId, { featureId: "api_calls", range: "30d" });
const { url } = await autumnFeatures.checkout(userId, { productId: "pro", freeTrial: true });
```
...
Under the hood the adapter calls Autumn's **attach** with `customer_id` (your
`referenceId`), `product_id`, and the framework-controlled `referenceId` pinned
into `metadata` so caller metadata can never override it. The facade reuses a
reference's stored Autumn customer — Autumn's customer create is idempotent, so
the app-supplied id _is_ the customer id — and attaches an outbound idempotency
key automatically.
...
`quantity` you pass becomes Autumn's `required_balance` (default `1`) — the check
...
Autumn identifies a subscription by the **pair** `(customer_id, product_id)`
rather than a single id. The adapter therefore encodes the Lunora
`Subscription.id` as the composite `"<customerId>::<productId>"` and splits it
...
Lifecycle operations map onto Autumn's attach/cancel model:

- **Cancel** (`cancelSubscription`) — cancels the customer's product, immediately
  or at period end (`atPeriodEnd: true`). A cancel-at-period-end re-reads Autumn's
  real status and only flips the schedule flag, so a `past_due` / `paused` grant is
  never silently promoted to `active`.
- **Update / switch plan** (`updateSubscription`) — a plan change is an `attach` of
  the new product; the adapter then re-reads Autumn's authoritative state.
- **Resume** (`resumeSubscription`) — re-attaches the product to undo a scheduled
  cancellation, then re-reads the real state.
...
<Callout type="warn">
    **"malformed autumn subscription id".** `cancelSubscription` / `getSubscriptionStatus` expect the composite `"<customerId>::<productId>"` id the adapter
    minted (returned by `attach` / `createCheckout` and stored on the subscription row). Pass that id back verbatim — don't reconstruct it from a bare product id.
</Callout>
