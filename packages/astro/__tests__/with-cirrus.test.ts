...
import type { AstroWorkerHandler } from "../src/with-cirrus";
import { withCirrus } from "../src/with-cirrus";

/**
 * Minimal structural fakes mirroring `packages/runtime/__tests__/create-worker.test.ts`.
 * `withCirrus` is thin sugar over `composeWorker`, so the contract under test is
 * the *composition*: `/_cirrus/*` routes into Cirrus (forwarded to the shard),
...
describe("withCirrus — Astro class-B single-worker composition (PLAN4 §3)", () => {
    let shard: ShardSpy;
    ...
    it("routes /_cirrus/rpc into Cirrus rather than the Astro handler", async () => {
        expect.assertions(3);

        const astro = vi.fn<() => Response>(() => new Response("astro ssr"));

        const worker = withCirrus({ fetch: astro }, { shardDO: shard.namespace });
    ...
    it("delegates a non-reserved path to the wrapped Astro SSR handler (object form)", async () => {
        ...
        const worker = withCirrus({ fetch: () => new Response("rendered", { status: 200 }) }, { shardDO: shard.namespace });
    ...
    it("accepts a bare fetch-function Astro handler and delegates to it", async () => {
        ...
        const worker = withCirrus(astro, { shardDO: shard.namespace });
    ...
    it("forwards the realtime path to the shard with the envelope's shardKey", async () => {
        ...
        const worker = withCirrus({ fetch: () => new Response("astro") }, { shardDO: shard.namespace });
    ...
    it("isolates a throwing Astro render as a 500 while /_cirrus/* stays serviceable", async () => {
        expect.assertions(4);

        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => undefined);

        const worker = withCirrus(
            {
                fetch: () => {
