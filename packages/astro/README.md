...
// src/worker.ts — wrap the adapter's worker so Lunora mounts under /_lunora/*.
import astroWorker from "../dist/_worker.js/index.js";
import { withLunora } from "@lunora/astro";

export default {
    fetch: (request: Request, env: Env, ctx: ExecutionContext) => withLunora(astroWorker, { shardDO: env.SHARD }).fetch(request, env, ctx),
};
```
