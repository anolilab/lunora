...
Add the `lunora` integration to `astro.config`, then wrap the Cloudflare adapter's SSR handler in `src/server.ts`.
...
The `astro.config` integration object (`{ name, hooks }`). The only option is `serverEntry` (default `"src/server.ts"`), the module that calls `withLunora` and is the composed worker's `export default`. It is deliberately **not** `src/worker.ts`: `lunora deploy` treats that exact path as a SvelteKit-shaped composed entry and passes it to wrangler positionally, which together with the `@astrojs/cloudflare` redirect's `no_bundle: true` would upload the raw TypeScript source as the worker.
