...
describe("withLunora — Nuxt single-worker composition (PLAN4 §3, M4)", () => {
    let shard: ShardSpy;
    ...
    it("routes /_lunora/rpc into Lunora rather than the Nitro handler", async () => {
        expect.assertions(3);

        const nitro = createNitroHandler(() => new Response("ssr"));

        const worker = withLunora(nitro.handler, { shardDO: shard.namespace });
    ...
    it("isolates a throwing Nitro handler so /_lunora/* stays serviceable", async () => {
        expect.assertions(4);
        ...
        // The SAME worker still services realtime: a /_lunora/rpc request forwards
    ...
    it("forwards cron triggers to Lunora when crons are configured", async () => {
        expect.assertions(2);
        ...
        const worker = withLunora(nitro.handler, {
            crons: { "0 * * * *": cron },
        ...
        // Lunora crons win — Nitro's own scheduled is not invoked.
        expect(nitroScheduled).not.toHaveBeenCalled();
    });

    it("preserves Nitro's scheduled when no Lunora crons are wired", async () => {
        expect.assertions(1);
