...
meta-framework plugs into — *your loaders are live*), plus the independent DX wins that close the
...
- **Done-when:** **the proof** — run the template, open two tabs, mutate in tab A, see tab B's SSR'd-then-live loader data update without a manual refetch. SSR HTML contains the data (view-source check). This validates the *entire* contract.
...
> Hardest tier: these frameworks bundle their own worker, so we *inject* Cirrus rather than own the
...
*Companion plans: `PLAN4.md` (the strategy + seam map), `VOID-TEARDOWN.md` (the reference for void's
class-A/B/C composition, remote-binding-dev protocol, and the gap analysis). Memories:
[[void-cloud-teardown]], [[plan2-gap-status]], [[esm-only-packages]] (new packages must ship ESM-only).*
