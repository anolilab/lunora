...
- `@lunora/platform-rivet` — the Rivet **host**. Implements the contracts over [Rivet Actors](https://rivet.dev/actors) (`rivetkit`): an actor per shard key, with sharded state, durable alarms, runtime cron registration and create-time placement all coming from the platform rather than being emulated, and WebSocket hibernation split between the two (Rivet keeps the connection, the host persists and rebinds the subscription state on it). Its one structural gap is that Rivet's actor SQLite is **async-only** while `ShardSqlExec.exec` is synchronous, so the shard runs on a `better-sqlite3` working copy snapshotted back to Rivet at each commit (`RIVET_CAPABILITIES.localSql` is rated `emulated` for exactly this). **Experimental**, and narrower than the Node host: no `lunora dev --target rivet`, no deploy driver, no `.global()` backend, and the conformance TCK runs against an in-memory actor double rather than a live engine. See `plans/rivet-host-findings.md`.
...
| `@lunora/platform-rivet`      | **Experimental.** Rivet **host** over Rivet Actors (`rivetkit`): native sharded state, alarms, runtime crons; hibernated sockets and local SQL over a snapshotted working copy. No dev/deploy target yet.                 |
