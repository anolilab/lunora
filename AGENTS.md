...
| `@lunora/agent`             | Durable AI agents (opt-in add-on): `defineAgent` (in `lunora/agents.ts`) compiles a replay-safe tool-loop onto Cloudflare Workflows — LLM turns + tool calls as named durable steps (step name = tool idempotency key), idempotent DO SQLite thread tables shipped as the `agent` schema extension, `ctx.agents.<name>.run(threadKey, input)` producers, live thread view via the `agents:agentMessages` subscription, `memory` over a `@lunora/ai/rag` action.                                                                                                                                                                    |
