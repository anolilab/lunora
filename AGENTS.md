...
There is no scoped `@lunora/lunora`, but there **is** an unscoped umbrella package named **`lunora`** (directory `packages/lunora/`) — it re-exports the base packages (`@lunora/server` + subpaths, `@lunora/values`, `@lunora/runtime`, `@lunora/do`) through subpaths (`lunora/server`, `lunora/values`, `lunora/runtime`, `lunora/do`) and ships the `lunora` CLI bin (delegating to `@lunora/cli`), so an app can depend on the single `lunora` package instead of the five base packages. Codegen detects an umbrella dependency (via declared deps) and emits `lunora/*` imports in `_generated/*` instead of `@lunora/*` — opt-in and backward-compatible; granular `@lunora/*` projects are unchanged. The OG-image generator (`scripts/generate-package-og-images.js`) special-cases the unscoped `lunora` name.

### Packages

| Package             | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lunora`            | **Unscoped umbrella.** One install for the base: re-exports `@lunora/server` (+ subpaths), `@lunora/values`, `@lunora/runtime`, `@lunora/do` via subpaths (`lunora/server`, …) and ships the `lunora` CLI bin. Codegen emits `lunora/*` imports when a project depends on it (else `@lunora/*`).                                                                                                                                                                                  |
