...
| `@lunora/platform`             | **Zero-dep** platform contracts: `ShardHost`/`SocketHost`/`ShardDirectory`/`SchedulerHost`, canonical binding `*Like` types, `PlatformCapabilities` matrix. Hosts live in `@lunora/platform-<target>`. |
| `@lunora/platform-conformance` | **Internal.** Behavioral TCK for platform hosts: parameterized Vitest suite + reference in-memory host.                                                                                                |
| `@lunora/shard-engine`         | Host-neutral reactive engine: per-shard state, OCC, CDC, reactive subscriptions, poke protocol. Consumes `@lunora/platform` contracts.                                                                 |
