...
- `@lunora/platform-node` — the Node **host**, still an experimental spike (plan 234): no `lunora dev` wiring, no deploy driver, capabilities mostly `unsupported`/`emulated`. Do not treat it as a shipping target; add it alongside `platform`/`platform-cloudflare` in the API-snapshot guard once it graduates past spike stage.
