...
| `@cirrus/auth`      | Auth built on **better-auth**, D1-backed (`cirrusD1Adapter`, or any pluggable `AuthStore`): email/password + OAuth (`genericOAuth`), session policies (`sessionPresets`), `handleAuthRequest` routing `/api/auth/*`. Curated plugins re-exported from `@cirrus/auth/plugins`: **passkey/WebAuthn**, 2FA, magic-link, email-OTP, admin, organization, JWT, OIDC provider, SIWE, username, anonymous, multi-session.                                                                |
...
| `@cirrus/container` | Cloudflare Containers: `defineContainer` (in `cirrus/containers.ts`) → generated container DO classes + typed `ctx.containers` (`.get`/`.any`/`.pool`); Dockerfile / registry / Railpack `{ build }` image sources; `@cirrus/container/do` (the `CirrusContainer` base) and `@cirrus/container/bridge` (container→Cirrus RPC client) subpaths.                                                                                                                                    |
...
| `@cirrus/seed`      | Snaplet-style deterministic seeding: introspects `defineSchema`, generates realistic fake data (field-name heuristics + validator kinds), resolves FK parents in topological order, and supports per-table counts + per-field overrides. Adapters: `seed(harness, schema, opts)` (`@cirrus/seed/testing`), the `cirrus seed` CLI (→ admin import RPC). Built on an **internal** rebuilt `copycat` over `@faker-js/faker` (input-hashed → stable values); copycat is not exported. |
