...
| `@lunora/hono`       | Hono integration: server-composition only (Hono has no reactive UI layer). `mountLunora(new Hono())` mounts Lunora realtime (`/_lunora/*` RPC + WebSocket + admin) as a Hono middleware and returns the same typed app; `shardDO` defaults to `env.SHARD`. Also `lunora()` (bare middleware) and `withLunora` (the shared `withFrameworkWorker`, Lunora owns the entry). Reactive-loader server helpers at `@lunora/hono/server`. Optional peer `hono`.                                         |
