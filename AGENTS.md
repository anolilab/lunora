...
| `@lunora/bindings`   | Lightweight Cloudflare binding helpers in one install, per-binding subpaths: `@lunora/bindings/kv` (Workers KV → `ctx.kv`), `/images` (Cloudflare Images → `ctx.images`), `/analytics` (Analytics Engine → `ctx.analytics` + Pipelines → `ctx.pipelines`), `/vectors` (Vectorize → `ctx.vectors`), `/r2sql` (R2 SQL → `ctx.r2sql`). Each is a thin, zero-dep `ctx.*` facade; `sideEffects:false` subpath exports keep per-binding tree-shaking. Heavy-peer add-ons stay separate. |
