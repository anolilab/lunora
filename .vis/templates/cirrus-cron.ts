/**
 * `vis generate cirrus-cron` — add a code-first cron job to cirrus/crons.ts.
 *
...
const freshCrons = (jobName: string): string => `import { cronJobs } from "@cirrus/scheduler";

import { internal } from "./_generated/api.js";

/**
 * Code-first cron registry. Each registration runs the referenced internal
 * function on the given schedule. \`@cirrus/codegen\` discovers this default
...
export default createTemplate({
    about: {
        description: "Add a cron job to cirrus/crons.ts (creates the file if missing)",
        name: "cirrus-cron",
    },
    options: {
        name: {
    ...
    produce: ({ builtins, options }) => {
        ...
        const cronsPath = join(builtins.dest_dir, "cirrus", "crons.ts");

        if (!existsSync(cronsPath)) {
            return {
                files: { cirrus: { "crons.ts": freshCrons(raw) } },
                suggestions: [
                    `Created cirrus/crons.ts with job "${raw}".`,
                    `Point the registration at a real internal.<file>.<fn> reference, then run \`cirrus codegen\`.`,
                ],
            };
        }
        ...
        return {
            files: { cirrus: { "crons.ts": result.text } },
            filesMeta: { "cirrus/crons.ts": { force: true } },
            suggestions: [
                `Added cron job "${raw}" to cirrus/crons.ts.`,
                `Edit the schedule/target, then run \`cirrus codegen\` to regenerate the wrangler triggers.`,
            ],
        };
    },
});
