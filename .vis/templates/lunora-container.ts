/**
 * `vis generate lunora-container` — declare a Cloudflare Container in
 * lunora/containers.ts and scaffold its Dockerfile.
 *
 ...
 * One Cloudflare Container instance class. \`lunora dev\`/\`lunora deploy\`
...
const freshContainers = (exportName: string, directory: string): string => `import { defineContainer } from "@lunora/container";

${definitionFor(exportName, directory)}`;

const dockerfile = (exportName: string): string => `# Container image for the \`${exportName}\` definition in lunora/containers.ts.
...
export default createTemplate({
    about: {
        description: "Declare a container in lunora/containers.ts and scaffold its Dockerfile",
    ...
    produce: ({ builtins, options }) => {
        ...
        const containersPath = join(builtins.dest_dir, "lunora", "containers.ts");
        ...
        const entrySuggestion = entry
            ...
            : 'Re-export the generated classes from your worker entry: `export * from "./lunora/_generated/containers"`.';

        const suggestions = [
            `Scaffolded ${directory}/Dockerfile (+ a starter server.mjs).`,
            entrySuggestion,
            "Run `lunora codegen` (or just `lunora dev`) to emit the Container class and reconcile wrangler.jsonc.",
        ];

        if (!existsSync(containersPath)) {
            return {
                files: { ...dockerFiles, ...entryFiles, lunora: { "containers.ts": freshContainers(exportName, directory) } },
                filesMeta: entryMeta,
                suggestions: [`Created lunora/containers.ts with container "${exportName}".`, ...suggestions],
            };
        }
        ...
        return {
            files: { ...dockerFiles, ...entryFiles, lunora: { "containers.ts": `${original}${separator}${definitionFor(exportName, directory)}` } },
            filesMeta: { "lunora/containers.ts": { force: true }, ...entryMeta },
            suggestions: [`Added container "${exportName}" to lunora/containers.ts.`, ...suggestions],
        };
    },
});
