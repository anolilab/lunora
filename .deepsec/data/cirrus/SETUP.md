# Agent setup for `cirrus`

This is a deepsec scanning workspace. Project `cirrus` was just registered
(target: `..`). Setup is incomplete — `data/cirrus/INFO.md`
...
2. **Fill in `data/cirrus/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id cirrus
pnpm deepsec process --project-id cirrus
```
