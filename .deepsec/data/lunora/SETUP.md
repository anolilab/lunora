# Agent setup for `lunora`

This is a deepsec scanning workspace. Project `lunora` was just registered
(target: `..`). Setup is incomplete — `data/lunora/INFO.md`
...
2. **Fill in `data/lunora/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id lunora
pnpm deepsec process --project-id lunora
```
