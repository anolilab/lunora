...
lunora is a pnpm monorepo (`@lunora/*`) for a Convex-style, type-safe
...
- `@lunora/auth` wraps better-auth (cookie sessions, PBKDF2 email/pw,
...
- `apps/` examples, `templates/`, and `.vis/templates/lunora-*.ts` are
  scaffolding/fixtures — not production attack surface.
- The Vite studio plugin and `lunora dev` server are dev-only and
