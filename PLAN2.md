...
**Status update 2026-06-08 (plugin system):** #24 closed — added `installPlugins` + `composePluginMiddleware` (one-shot install), codegen discovery of re-exported component functions, and dogfooded the contract with `@cirrus/ratelimit`'s `ratelimitPlugin`.

**Status update 2026-06-08 (auth):** #28 closed — `@cirrus/auth` now surfaces the full better-auth plugin ecosystem: re-exported plugins + `ctx.authApi` endpoints, plus `authTables(options)` which generates Cirrus `defineTable`s from better-auth's `getAuthTables`, so any configured plugin (org/teams, admin, billing, …) auto-contributes its tables. Remaining open rows: #25 (`add` registry), #27 (TanStack Start / Expo templates).
...
| 24  | Plugin system (`definePlugin`, `defineSchemaExtension`, plugin middleware) | ✅    | ✅     | `packages/server/src/plugin.ts` — `definePlugin`/`defineSchemaExtension`/`defineComponent` + schema `.extend()` (auto-prefix, collision detection, relation/index/trigger preservation), middleware via `.use()` (`ctx.api.<key>`); one-shot `installPlugins(schema, [...])` + `composePluginMiddleware([...])`; codegen discovers re-exported component functions (`discover-functions.ts`); dogfooded by `@cirrus/ratelimit`'s `ratelimitPlugin` |
| 25  | shadcn-style `add` registry (scaffold into user project)                   | ✅    | 🟡     | mechanism shipped — `cirrus registry add/list/view/build` (`packages/cli/src/commands/registry/*`: manifest, catalog, resolve/reconcile/apply, dependency resolution), shadcn/kitcn model, fetches items from `gh:anolilab/cirrus/registry`, 53 tests. Remaining: publishing the actual registry items (content, not mechanism)                                                                                                                    |
...
| 28  | Auth: organizations/teams, admin (ban/impersonate), billing (Polar)        | ✅    | ✅     | `@cirrus/auth` wraps better-auth and exposes the full plugin surface: re-exported plugins (`@cirrus/auth/plugins`), every endpoint on `ctx.authApi` (`withAuthPlugins`), and `authTables(options)` (`packages/auth/src/schema.ts`) which generates Cirrus `defineTable`s from `getAuthTables` — so any configured plugin (organization/teams, admin, billing, passkey, …) auto-contributes its tables to the schema                                |
...
✅ Landed — `packages/server/src/plugin.ts`.

`definePlugin('<key>', …)` + `defineSchemaExtension('<key>', { tables })` (+ relations / aggregate-rank indexes / triggers, all preserved and reference-rewritten through the merge) + `plugin.middleware` injecting `ctx.api.<key>`; apps install via `defineSchema(tables).extend(extension)`. This is how kitcn ships auth/resend/ratelimit as composable units.

Closing the v1 gaps:

- **One-shot install.** `installPlugins(schema, [a, b])` folds every plugin's extension in one call (middleware-only plugins skipped); `composePluginMiddleware([a, b])` runs every plugin middleware under a single `.use(...)`, accumulating each `ctx.api.<key>`. Two calls install N plugins instead of N `.extend()` + N `.use()`.
- **Namespaced-function codegen.** `discover-functions.ts` follows a re-exported component function — `export const check = c.functions.check` or `export const { check } = c.functions` — back to its originating `query/mutation/action({...})` call, so bundled functions emit into the host app's `api` namespace (an unresolvable/published-only reference is skipped, not an error).
- **Dogfood.** `@cirrus/ratelimit` ships `ratelimitPlugin(limiter)` — a real `Plugin` whose middleware exposes the resolved `RateLimiter` under `ctx.api.ratelimit` — built as a plain `Plugin` literal so `@cirrus/server` stays a type-only dependency.

Test-covered in `packages/server/__tests__/plugin.test.ts`, `packages/ratelimit/__tests__/plugin.test.ts`, and `packages/codegen/__tests__/discover-functions.test.ts`.

### 3.6 shadcn-style `add` registry `[matrix #25]`

🟡 Mechanism shipped — `packages/cli/src/commands/registry/*`.

The defining "kitcn" mechanic: `npx kitcn add auth|ratelimit|resend` scaffolds user-owned code (schema, runtime, templates) into the project, with a planner, dependency resolution, dry-run, and schema reconciliation/ownership tracking. Cirrus ships the equivalent — `cirrus registry add/list/view/build` (manifest, catalog, resolve/reconcile/apply, registry-dependency resolution), modeled on shadcn/kitcn, fetching items from `gh:anolilab/cirrus/registry` (alongside `giget` whole-project templates), 53 tests. **What's left is content, not mechanism**: publishing the actual registry items so `cirrus registry add ratelimit` has something to fetch.

### 3.7 Auth feature expansion `[matrix #28]`

✅ Landed — `packages/auth`.

better-auth org/teams, admin (ban/impersonate), billing (Polar) and the rest are all better-auth plugins, and `@cirrus/auth` now surfaces them end to end:

- **Endpoints.** Plugins are re-exported from `@cirrus/auth/plugins`; once added to `createAuth({ plugins: [...] })` their endpoints are callable on `ctx.authApi` (via `withAuthPlugins`) — `createOrganization`, `banUser`, `setRole`, impersonation, etc.
- **Schema generation.** `authTables(options)` (`packages/auth/src/schema.ts`) walks better-auth's own `getAuthTables(options)` — which already merges every configured plugin's `schema` — and emits an equivalent Cirrus `defineTable` per table, mapping `DBFieldAttribute` → `v.*` (`string`/`number`/`boolean`/`date`/array; `references` → `v.id(model)`; `required:false` → `.nullable()`; `unique` → `.unique()`). Spreading `...authTables(options)` into `defineSchema` makes the auth tables first-class Cirrus data; adding a plugin auto-adds its tables with no hand-written definitions.

- **Write path.** `cirrusAuthAdapter(store)` (`packages/auth/src/adapter.ts`, exported as `@cirrus/auth/adapter`) is a better-auth database adapter (via better-auth's `createAdapterFactory`) that routes every read/write through an `AuthStore` — a minimal table-addressed CRUD seam a Cirrus runtime backs with `ctx.db` over the global (D1) auth tables. So `createAuth({ database: cirrusAuthAdapter(store) })` makes better-auth write through Cirrus's data layer instead of its built-in D1/Kysely adapter. Ships with a reference `createMemoryAuthStore` and the pure `matchesWhere` clause evaluator; verified by running real better-auth sign-up/sign-in/credential-check end to end against the adapter (`packages/auth/__tests__/adapter.test.ts`). Single-table CRUD only — better-auth's relational `join` reads are out of scope.
