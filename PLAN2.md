...
**Status update 2026-06-08 (plugin system):** #24 closed — added `installPlugins` + `composePluginMiddleware` (one-shot install), codegen discovery of re-exported component functions, and dogfooded the contract with `@cirrus/ratelimit`'s `ratelimitPlugin`. Remaining open rows: #25 (`add` registry), #27 (TanStack Start / Expo templates), #28 (auth org/teams/billing).
