...
`createXCaller(ctx)` to compose functions server-to-server without HTTP, available in any context (kitcn even mandates it over `ctx.run*` for plugin internals). Cirrus only exposes `ActionCtx.run*`. Depends on 1.1.

### 2.4 RSC / SSR server helpers `[matrix #22]`

`kitcn/rsc`, `auth/nextjs` for server data loading + auth in RSC. Cirrus equivalent = Cloudflare Pages SSR; lower priority but relevant for Next-on-Pages users.
...
Add `analyze` (runtime/bundle analysis), `env`, `info`, `verify`, `view`, `docs`; add TanStack Start + Expo init templates. Incremental, independent of the tiers above.
