...
Remaining nuance (not a parity gap): the auth tables are still **written** through better-auth's own D1 adapter (`ensureMigrated`) — surfacing them as Cirrus tables makes them typed + queryable via `ctx.db`; routing better-auth's writes through Cirrus's DO/D1 ORM is a deeper follow-up.
