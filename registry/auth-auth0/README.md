...
Sign in with [Auth0](https://auth0.com) for Lunora auth. Wires Auth0 as an OIDC provider through [better-auth](https://www.better-auth.com)'s [`genericOAuth`](https://www.better-auth.com/docs/plugins/generic-oauth) plugin (also re-exported by [`@lunora/auth/plugins`](../../packages/auth)), on top of the base [`auth`](../auth) item.
...
2. `lunora/auth/auth0.ts` — the Auth0 OIDC provider plugin (`auth0(env)`). It's **yours** to edit.
...
Add the `auth0` plugin to the `plugins` array in `lunora/auth/index.ts` (scaffolded by the base item):
...
`lunora/auth/auth0.ts` is copied into your repo — change scopes, switch from discovery to explicit `authorizationUrl` / `tokenUrl`, add `mapProfileToUser`, or wire additional providers however you like. `@lunora/auth` provides the wrapper + the re-exported better-auth plugin; this item is the idiomatic Lunora glue.
