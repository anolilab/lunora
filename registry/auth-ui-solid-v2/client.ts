 ...
 *
 * `createAuthClient` is the framework-neutral `better-auth/client`, not
 * `better-auth/solid`. The Solid variant exists to provide a reactive
 * `useSession`, which these screens deliberately do not use — `core/session.ts`
 * is one more external store, adapted by the same bridge as every other
 * controller, so a flow is written once. It also imports `solid-js/store`, a
 * subpath Solid 2 removed, so it would not resolve here at all.
 */
