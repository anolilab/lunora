...
import { createEffect, createSignal, createUniqueId, For, onCleanup, onSettled, Show } from "solid-js";
    ...
    return (
        ...
        <Show when={siteKey()}>
            {(key) => {
                ...
                // Split-phase effect (Solid 2): the compute names the reactive
                // inputs, and the apply returns its own teardown instead of
                // registering `onCleanup` inside the body. Losing the site key or
                // switching provider disposes the widget and drops any single-use
                // token it left behind.
                createEffect(
                    () => {
                        return { key: key(), provider: props.provider };
                    },
                    (input) => {
                        if (host === undefined) {
                            return undefined;
                        ...
                        return renderCaptcha(host, { onError: context.onError, provider: input.provider, siteKey: input.key });
                    },
                );
...
const OneTap = (): JSX.Element => {
    ...
    // `onSettled`, not `createEffect`: prompting again on any later change would
    // nag the user with a prompt they already dismissed. (Solid 1.x spelled this
    // `onMount`.)
    onSettled(() => {
