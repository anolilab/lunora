...
const AuthUIProvider = (props: AuthUIProviderProps): JSX.Element => {
     ...
     */
    const handle = props.discover === false ? undefined : discoverAuthConfig(props.basePath ?? DEFAULT_BASE_PATH);

    /*
     * SEEDED, not written after the fact. Solid 2 rejects a write to reactive
     * state from inside an owned scope, and a component body is one — the 1.x
     * spelling (`createSignal(undefined)` then `setDiscovered(...)` here) throws
     * `REACTIVE_WRITE_IN_OWNED_SCOPE`. Seeding keeps the same property: a cached
     * answer is already in the first render, with no effect round-trip.
     *
     * `ownedWrite` covers the subscription below: the discovery handle may
     * notify synchronously while `subscribe` is still running — i.e. still inside
     * this component's scope — and that write is this component's own state,
     * which is exactly what the option is for.
     */
    const [discovered, setDiscovered] = createSignal<DiscoveredConfig | undefined>(handle?.getState().config, { ownedWrite: true });
