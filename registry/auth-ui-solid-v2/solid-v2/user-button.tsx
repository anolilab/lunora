...
const UserAvatar = (props: UserAvatarProps): JSX.Element => {
    ...
    // (Solid 2 folded `on(source, fn, { defer })` into the split-phase effect.)
    createEffect(
        image,
        () => {
            setFailed(false);
...
const UserButton = (props: UserButtonProps): JSX.Element => {
    ...
    // Split-phase effect (Solid 2): `open()` is the tracked input, and the apply
    // returns the listener teardown rather than registering `onCleanup` inside.
