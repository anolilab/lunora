...
export default createConfig(
    ...
    // ── App-scoped allowances (not blanket rule-off) ──────────────────────────
    // Lunora function modules (lunora/*.ts) and the route modules export *named*
    // symbols by design — codegen references functions by name, and TanStack
    // Router requires the `Route` export in each file under src/routes.
    // Defaulting them would break the by-name references.
    {
    ...
    // TanStack Router shapes these rules read as defects, but the framework
    // requires them:
    //   - `no-use-before-define`: a route module is mutually recursive by design —
    //     `Route` names its component and the component reads `Route.useX()`. The
    //     read happens at render time, never at module evaluation, so one of the
    //     two necessarily appears before the other.
    //   - `sonarjs/deprecation`: React 19 marks the `*EventHandler` aliases
    //     deprecated, but they are still exactly what `onSubmit`/`onChange` are
    //     typed as; the alternative is an untyped inline handler.
    //   - `unicorn/filename-case`: `__root.tsx` and the file-based route names are
