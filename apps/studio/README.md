...
A standalone, deployable build of the Cirrus studio. It's a plain Vite SPA
...
`@cirrus/vite` plugin) serves the studio at `/__cirrus` against your own
...
VITE_CIRRUS_URL=https://my-app.workers.dev pnpm --filter @cirrus/studio-app dev
```
...
```bash
pnpm --filter @cirrus/studio-app build   # → dist/ static assets
```
...
configured (see `@cirrus/studio`'s README).
