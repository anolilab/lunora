...
/**
 * Mark the dev account's address verified, through the admin auth surface.
 *
 * The control plane sets `emailAndPassword.requireEmailVerification`, so a fresh
 * sign-up cannot sign in until its address is confirmed — and in dev the
 * verification mail is captured into the studio Mail tab rather than delivered,
 * which no script can click. This is the same `updateUser` call the studio's own
 * user table makes, under the same `LUNORA_ADMIN_TOKEN` boundary, so it stays a
 * real surface rather than a write behind the app's back.
 *
 * Idempotent: verifying an already-verified user is a no-op, and a failure is
 * not fatal — the sign-in below reports the real problem with a better message
 * than a 4xx from here would.
 */
const verifyDevEmail = async (): Promise<void> => {
    const bearer = `Bearer ${await readAdminToken()}`;
    const headers = { authorization: bearer, "content-type": "application/json", origin: BASE_URL };
    const listed = await fetch(`${BASE_URL}/_lunora/admin/auth/users?searchValue=${encodeURIComponent(DEV_EMAIL)}&searchField=email`, { headers });

    if (!listed.ok) {
        return;
    }

    const { users }: { users?: { email?: string; emailVerified?: boolean; id?: string }[] } = await listed.json();
    const user = users?.find((candidate) => candidate.email === DEV_EMAIL);

    if (user?.id === undefined || user.emailVerified === true) {
        return;
    }

    await fetch(`${BASE_URL}/_lunora/admin/auth/users/update`, {
        body: JSON.stringify({ data: { emailVerified: true }, userId: user.id }),
        headers,
        method: "POST",
    });
};
 ...
 *
 * Sign-up works at all only because `inviteOnly({ allowFirstUser })` is on in
 * dev: registration is invite-only, and the first account on an empty database
 * is the one carve-out. A second dev account needs a sign-up invitation.
 */
