 ...
 *  4. babel(reactCompilerPreset) — React Compiler, after the JSX transform and
 *                       before anything that reads the output. Same wiring as
 *                       `apps/studio`; it is why this app's components carry no
 *                       hand-written `useMemo`/`useCallback` for identity.
 *  5. lunora({ cloudflare: false }) — codegen, wrangler validation, studio
...
export default defineConfig(({ command }) => {
    return {
        plugins: [
            ...
            tanstackStart(),
            react(),
            // `panicThreshold: "none"` matches apps/studio: a component the
