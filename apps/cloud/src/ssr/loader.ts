 ...
 */
// react-doctor-disable-next-line react-doctor/tanstack-start-get-mutation -- the `headers.set` calls below build the OUTBOUND fetch (forwarding the caller's own cookie and origin), not a response header. This handler reads `organizations:list` and mutates nothing, so there is no state for CSRF to reach.
export const loadMyOrganizationIds = createServerFn({ method: "GET" }).handler(async (): Promise<string[]> => {
    // Loaded together: three independent module loads, and awaiting them in
    // series made the first server render pay all three round trips end to end.
...
const preloadOnServer = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data }): Promise<string> => {
        // Loaded together, as in `loadMyOrganizationIds` above.
