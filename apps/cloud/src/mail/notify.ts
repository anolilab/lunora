...
export interface InvitationEmail {
    ...
    /**
     * Sign-up link, when the invitee has no account yet — registration is
     * invite-only, so without one they have nothing to accept the invitation
     * with. Carries its own single-use token (`?invite=`), distinct from the
     * org-invitation token in {@link InvitationEmail.acceptUrl}.
     */
    signUpUrl?: string;
    /** Invitee address. */
    to: string;
...
export const sendInvitationEmail = async (env: Record<string, unknown>, invitation: InvitationEmail): Promise<void> => {
    ...
    // Sign up FIRST when they need to: accepting requires a session, so leading
    // with the accept link sends a brand-new invitee to a screen that bounces
    // them to sign-in, where registration is closed to them.
    const body = invitation.signUpUrl
        ? `You've been invited${org} on Lunora Cloud.\n\nCreate your account:\n${invitation.signUpUrl}\n\nThen accept your invitation:\n${invitation.acceptUrl}`
