...
export const buildOverageReconcileData = async (database: ControlPlaneDatabase, periodStart: number): Promise<OverageReconcileData> => {
    ...
    // react-doctor-disable-next-line react-doctor/server-sequential-independent-await -- both drains run against the SAME D1 connection, so overlapping them adds contention rather than parallelism.
