...
/**
 * Which spans the waterfall draws, and what that choice implies.
 *
 * Two stores answer the same question with different retention: D1 holds the hot
 * window, the archive holds everything past it. Preferring D1 and falling back
 * keeps a just-written trace visible before the archive has it, and
 * `fromArchive` is what lets the card say which one the reader is looking at —
 * an archived trace can be incomplete in ways a live one is not.
 *
 * `loading` is deliberately not "no spans yet": an empty D1 result is a real
 * answer, and only becomes a wait once the archive has also been asked and has
 * not replied. Conflating the two made a genuinely empty trace render as a
 * spinner forever.
 */
const resolveWaterfall = (input: {
    archivedSpans: ReadonlyArray<ObservationSpan> | undefined;
    d1Empty: boolean;
    selectedSpanId: string;
    spans: ReadonlyArray<ObservationSpan> | undefined;
}): { displaySpans: ReadonlyArray<ObservationSpan>; fromArchive: boolean; loading: boolean; selectedSpan?: WaterfallSpan; waterfall: WaterfallSpan[] } => {
    const { archivedSpans, d1Empty, selectedSpanId, spans } = input;
    ...
    return {
        ...
        fromArchive: spans?.length === 0 && (archivedSpans?.length ?? 0) > 0,
        loading: spans === undefined || (d1Empty && archivedSpans === undefined),
        selectedSpan: waterfall.find((span) => span.spanId === selectedSpanId),
        waterfall,
    };
};
...
const OpenTraceCard = ({
    ...
    const { displaySpans, fromArchive, loading, selectedSpan, waterfall } = resolveWaterfall({ archivedSpans, d1Empty, selectedSpanId, spans });
...
/**
 * The trace list: hot rollups first, then archived rows the hot window does not
 * already carry.
 *
 * Deduped by `traceId` with hot winning, because the two stores overlap at the
 * retention boundary and the hot row is the more complete one. Returns the
 * archived slice as well as the merged list — the footer needs to say whether
 * the archive added anything, and each row needs to know which store it came
 * from, both of which would otherwise be recomputed at the call site from the
 * same two inputs.
 */
const mergeTraceRows = <T extends { traceId: string }>(
    hot: ReadonlyArray<T> | undefined,
    archived: ReadonlyArray<T> | undefined,
): { archivedIds: ReadonlySet<string>; archivedRows: T[]; rows: T[] } => {
    const hotIds = new Set((hot ?? []).map((trace) => trace.traceId));
    const archivedRows = (archived ?? []).filter((trace) => !hotIds.has(trace.traceId));

    return { archivedIds: new Set(archivedRows.map((trace) => trace.traceId)), archivedRows, rows: [...(hot ?? []), ...archivedRows] };
};

/**
 * The archive's three states: not fetched yet (offer it), fetched and empty (say
 * so), fetched with rows (the list above already shows them, so nothing here).
 *
 * A component rather than a `let` reassigned through an if/else chain in the
 * section body — the states are mutually exclusive and reading them as one
 * component makes that structural rather than something to trace through.
 */
const ArchiveFooter = ({
    archivedCount,
    loaded,
    ...
    if (!loaded) {
        return (
            <Button className="self-start" disabled={loading} onClick={onLoad} size="sm" variant="outline">
                {loading ? "[Loading…]" : "Load older from archive"}
            </Button>
        );
    }

    if (archivedCount === 0) {
        return <p className={cn(COLUMN_LABEL, "text-muted-foreground m-0")}>No older traces in the archive for this window.</p>;
    }
...
export const TracesSection = ({ focusTraceId, organizationId, preloaded }: SectionProps<ReturnOf<typeof api.projects.listByOrg>>): ReactElement => {
    ...
    const { archivedIds, archivedRows, rows } = mergeTraceRows(traces, olderTraces);
            ...
            {deploymentId ? (
                <Card>
                    ...
                    <CardContent className="flex flex-col gap-4">
                        {rows.length > 0 ? (
                            <Table>
                                <ColumnHeader labels={["Trace", "Root operation", "Spans", "Latency", "Status", "Started"]} />
                                <TableBody>
                                    {rows.map((trace) => (
                                        <TraceRow
                                            archived={archivedIds.has(trace.traceId)}
                                            key={trace.traceId}
                        ...
                        <ArchiveFooter
                            ...
                            loaded={olderTraces !== undefined}
                            loading={loadingOlder}
                            onLoad={loadOlderFromArchive}
                        />
                    </CardContent>
                </Card>
            ) : null}
