...
import { ShareList, StatusBreakdown, VolumeChart } from "./TrafficCharts";
...
/**
 * The three states the sampled snapshot can be in before it has breakdowns to
 * draw: unreadable, still loading, or read and empty.
 *
 * Extracted so `TrafficSection` reads as its layout rather than as a state
 * machine — the three were consecutive conditionals in the middle of the tree,
 * and they are the same concern rendered three ways.
 */
const SnapshotState = ({ error, totalRequests }: { error?: string; totalRequests?: number }): ReactElement | null => {
    if (error !== undefined) {
        return (
            <Card>
        ...
        return (
            <Card>
                <CardContent className="text-muted-foreground py-8 text-center text-sm">
                    No request breakdowns for this window. They come from the metering stream, which fills in once a deployment serves traffic — the live stream
...
/**
 * One latency percentile tile. `ClientOnly` because `traffic.live` can be
 * resolved on the client and not on the server, so rendering the number during
 * SSR fails hydration; `undefined` is "no exact data yet", not zero.
 */
const LatencyStat = ({ label, ms }: { label: string; ms?: number }): ReactElement => (
    <Stat label={label} value={<ClientOnly fallback="—">{ms === undefined ? "—" : formatMs(ms)}</ClientOnly>} />
);
...
export const TrafficSection = ({ organizationId }: TrafficSectionProps): ReactElement => {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <SnapshotState error={error} totalRequests={snapshot?.totalRequests} />
            ...
            <Card>
                <CardContent className="grid gap-6 pt-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                    ...
                    <LatencyStat label="p50 latency" ms={liveHasData ? live.latency.p50 : undefined} />
                    <LatencyStat label="p95 latency" ms={liveHasData ? live.latency.p95 : undefined} />
                    <LatencyStat label="p99 latency" ms={liveHasData ? live.latency.p99 : undefined} />
                </CardContent>
            </Card>
