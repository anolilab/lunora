 ...
 */
/** A set with `value` added if absent and removed if present, leaving the original alone. */
...
/**
 * The `logs.list` arguments for the current filters.
 *
 * A module-level function rather than an inline ternary chain: every filter here
 * has its own "unset" spelling — an empty level set means *all* levels, not
 * none, and a blank search box means no `search` predicate rather than a search
 * for `""` — and those are easy to get backwards while reading a component's
 * render body.
 */
const logQueryArguments = (filters: {
    from: number;
    ...
    return {
        from: filters.from,
        levels: filters.levels.size > 0 ? [...filters.levels] : undefined,
        organizationId: filters.organizationId,
        scriptName: filters.scriptName,
        search: search === "" ? undefined : search,
        to: filters.to,
        traceId: filters.traceFilter,
    };
};
...
export const LogsSection = ({ focusTraceId, organizationId, preloaded }: SectionProps<ReturnOf<typeof api.projects.listByOrg>>): ReactElement => {
    ...
    const logs = useQuery(api.logs.list, scriptName === "" ? "skip" : logQueryArguments({ from, levels, organizationId, scriptName, search, to, traceFilter }));

    const toggleLevel = (level: LogLevel): void => {
        setLevels((previous) => toggled(previous, level));
    };
                    ...
                    <CardContent>
                        ...
                        <pre className="bg-muted/40 max-h-[36rem] overflow-auto p-4 font-mono text-xs leading-relaxed break-words whitespace-pre-wrap">
                            {(logs ?? []).map((entry, index) => (
                                // react-doctor-disable-next-line react-doctor/no-array-index-as-key -- the index is the TIEBREAKER, not the key: log lines share a millisecond. Append-only, read-only, no per-row state, so the reordering hazard cannot arise.
