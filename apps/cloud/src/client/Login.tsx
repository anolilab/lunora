...
interface LoginProps {
    /** Whether the visitor arrived on a sign-up invitation link (`?invite=`), which opens the screen on sign-up. */
    invited?: boolean;
...
export const Login = ({ invited, onSignedIn }: LoginProps = {}): ReactElement => {
    // Opens on sign-up when the visitor arrived on an invitation link — they
    // have no account yet, which is the whole reason the link exists.
    const [mode, setMode] = useState<"signin" | "signup">(invited === true ? "signup" : "signin");
            ...
            <section className="flex items-center border-t px-8 py-12 lg:border-t-0 lg:border-l lg:px-10">
                <ModeContext value={setMode}>
                    <div className="mx-auto w-full max-w-sm">
                        <AuthUIProvider
                            // No cast: `AuthUIProvider` now accepts the client's own type.
                            // `createAuthClient` returns a dynamic-path PROXY — every method
                            // resolves at runtime while the static type carries only the core
                            // surface — which is why `registerAuthClientPlugins` still has to
                            // be TOLD which flows exist rather than the cards introspecting
                            // the client. `AUTH_PLUGINS` is what decides that, matched to the
                            // server's plugin list in `src/server.ts`.
