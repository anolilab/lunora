...
export type RouterEnv = {
     ...
     * way the runtime stashes `__lunoraCtx`.
     *
     * Only for work that must outlive the response — analytics capture, so far.
     * A `fetch` started during a request and not handed to `waitUntil` is
     * cancelled when the response completes, which for fire-and-forget
     * telemetry means it silently never sends.
     */
