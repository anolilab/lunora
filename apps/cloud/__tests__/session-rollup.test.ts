...
describe(foldSessions, () => {
    it("groups turns by sessionId with turn count, summed tokens, and first/last seen", () => {
        const [session] = foldSessions(
            [
                turn({ completionTokens: 10, endedAt: 1000, promptTokens: 5, startedAt: 900, traceId: "ta" }),
                turn({ completionTokens: 20, endedAt: 2000, promptTokens: 7, startedAt: 1800, traceId: "tb" }),
            ],
            10,
        );
    ...
    it("orders sessions newest-active first and honors the limit", () => {
        const sessions = foldSessions(
            [turn({ endedAt: 1000, sessionId: "old" }), turn({ endedAt: 5000, sessionId: "new" }), turn({ endedAt: 3000, sessionId: "mid" })],
            2,
        );
