...
All 154 codes Lunora publishes, generated from the catalog the runtime, CLI, overlay, Studio and client SDK all read.
...
The copy needs DO-backed auth (`.auth({ namespace })`) pinned with `.jurisdiction(…, { pinAuthAndVoice: true })`, and the auth object's `internalSecret` set.
