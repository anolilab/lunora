...
All 151 codes Lunora publishes, generated from the catalog the runtime, CLI, overlay, Studio and client SDK all read.
...
The pinned auth object already has users, so copying into it would merge two user bases. Pass `force: true` to copy anyway: rows whose id or unique key already exists in the pinned object are skipped and reported as skipped.
