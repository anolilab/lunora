...
What is **not** re-exported is the payment kit — `CheckoutButton`,
`CustomerPortalButton` and `useCheckout` render a DOM `<button>` and navigate
with `globalThis.location`, neither of which exists in React Native. Drive a
checkout from a native screen instead: call the action that returns the
provider's `{ url }` (with `useMutation`) and open it with Expo's `Linking` or
`WebBrowser`.
...
`Cookie`, so it is exempt. The HTTP RPC sends `Authorization: Bearer <token>`; the
