...
and the rest run unchanged on a phone. On top of that it adds the two things a
native app needs that a browser gives you for free:
...
// not a valid effect cleanup return — so kick off a promise instead.
useEffect(() => {
    void (async () => {
