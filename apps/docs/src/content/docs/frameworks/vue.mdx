...
> **Preview.** The Vue adapter exposes the same hydrate-then-subscribe handoff as
> React, but it is **preview** maturity — not yet proven end-to-end against a
...
overlapping calls. Optimistic updates are passed **per call** (the Vue adapter
...
> **Nuxt caveat.** Nitro does not expose its fetch handler as an importable
> module, so single-worker composition is **not** achievable for Nuxt. The
> supported Nuxt integration is a two-worker split — the Nuxt/Nitro SSR worker
> plus a standalone Lunora worker owning `/_lunora/*` + `ShardDO`, wired by
