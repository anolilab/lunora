...
related:
    - to: /docs/packages/vue
      note: "the full composable + server reference"
...
Vue has no meta-framework SSR build of its own, so worker composition depends on
...
- **Nuxt**: the [`@lunora/nuxt`](/docs/frameworks/nuxt) module mounts Lunora
  ...
  composition, or you split into two workers instead. See the [Nuxt guide](/docs/frameworks/nuxt).
