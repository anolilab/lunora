...
- **Actions have their own primitive**, named per idiom: `useAction`
  ...
  with `call` in place of `mutate`; Angular's is a plain function, matching its
  `mutate`, because Angular models writes as calls rather than reactive handles.
  All five are narrower than their mutation counterpart: per-call options are
  `{ shardKey }` only, with no `optimistic` / `optimisticUpdate`. An optimistic
  update patches the subscription cache on the assumption a write will land, and
  an action is not a write — it runs in the Worker, may call a third party, and
  has no declared effect on any query.
