...
Because `@astrojs/cloudflare` writes to `dist/_worker.js/` and does **not**
clobber `main`, its wiring needs no `main` redirection.

</Callout>

<Callout type="warn">
    **Nuxt/Nitro: not class B — use `@lunora/nuxt`, and here is why.** Nitro does **not** expose its emitted fetch handler as an importable virtual module —
    there is no `#nitro-cloudflare-handler` or equivalent specifier in any documented Nitro API. `buildFrameworkWorker`/`withFrameworkWorker` composition, which
    needs exactly that importable handler, is therefore not achievable for Nuxt through any supported mechanism. Nuxt gets single-worker composition a different
    way instead: the [`@lunora/nuxt`](/docs/frameworks/nuxt) module mounts Lunora **inside** Nitro as a server route (`addServerHandler` at `/_lunora/**`, a
