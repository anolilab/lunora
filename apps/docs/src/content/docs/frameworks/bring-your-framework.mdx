...
Lunora is not a web framework, and it does not want to be one. It is the
...
Worker**. You keep your framework's routing, rendering, and DX; Lunora adds
...
A Lunora worker is created with `createWorker(...)`, which already accepts an
...
as the `httpRouter` and Lunora dispatches by path:
...
> **Shipped:** a `composeWorker({ httpRouter, ...lunoraOptions })` helper — thin
> sugar over `createWorker` — so templates read cleanly, plus an in-process
> `serverQuery` fast-path so an SSR loader can call Lunora _directly_ inside the
> same worker instead of round-tripping `/_lunora/rpc`. (`serverQuery` keeps the
...
> clobber it). The working wiring, proven by a real `lunora init -t sveltekit` →
...
> Astro composes the same way via `src/worker.ts` + `withLunora`; because
...
end-to-end — its `src/worker.ts` + `wrangler.jsonc` + `lunora deploy` compose
