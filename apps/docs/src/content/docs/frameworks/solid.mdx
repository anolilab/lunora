...
| `createEffect(fn)`         | `createEffect(compute, apply)` — two arguments  |
| `<Suspense>`               | `<Loading>`                                     |
...
One gap: `lunora add auth-ui` has no Solid 2 payload yet. Those screens are
copy-in, user-owned Solid 1.x source, so the CLI refuses on a Solid 2 project
rather than writing code that will not compile. `lunora add auth` (the server
half) is unaffected, as is every other `lunora add` feature.
