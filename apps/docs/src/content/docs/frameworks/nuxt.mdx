...
[`@lunora/vue`](/docs/frameworks/vue) composable resolves its client through
...
```ts title="plugins/lunora.ts"
import { LunoraClient } from "lunorash/client";
import { createLunora } from "@lunora/vue";

export default defineNuxtPlugin((nuxtApp) => {
    // Same-origin: `useRequestURL()` resolves the request origin during SSR and
    // `window.location` in the browser. The client appends `/_lunora/ws` and
    // `/_lunora/rpc` itself, which the module's route serves.
    nuxtApp.vueApp.use(createLunora(new LunoraClient({ url: useRequestURL().origin })));
});
```
