...
for the `.global()` variant, which adds a `migrate generate` step so the table's
declared indexes exist before the import (the table itself is provisioned lazily;
its indexes are not).
