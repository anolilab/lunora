...
| Python, Go, Ruby, Rust, Swift, Java, Kotlin | `lunora sdk generate --lang <id>` |
...
your schema, and does the same for six other languages. See
...
<Callout type="info">
    Optimistic updates and the offline queue are JS-only: the non-JS clients implement the wire protocol, subscriptions and the shape/poke protocol, but not the
    client-side mutation queue.
</Callout>
