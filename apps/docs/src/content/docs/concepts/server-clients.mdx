...
Typed bindings are no longer a follow-up: `lunora sdk generate --lang python`
emits typed argument and result models from your schema, and does the same for six
other languages. See [Non-JS SDKs](/docs/concepts/non-js-sdks) for the full list,
what each one needs installed, and how the generated output is versioned against
your CLI.

<Callout type="info">
    Optimistic updates and the offline queue remain JS-only — the non-JS clients implement the wire protocol, subscriptions and the shape/poke protocol, but not
    the client-side mutation queue.
</Callout>
