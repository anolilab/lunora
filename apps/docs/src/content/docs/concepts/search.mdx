...
Hyperdrive. Same tokenizer, same AND/prefix rules, same ranking. The one thing
that differs is **collation**, which belongs to the engine: MySQL folds case and
accents by default, Postgres compares bytes, and FTS5 strips diacritics — so
`café` and `cafe` match each other on D1 and MySQL but not on Postgres.
