...
Two limits worth knowing. `ß` is left alone — collapsing it to `ss` needs a
case-folding table this deliberately doesn't carry. And folding only strips
_Latin_ diacritics (U+0300–U+036F): text in other scripts is Unicode-normalized
but otherwise untouched, so Japanese voiced sound marks and Korean jamo survive
— `が` stays distinct from `か`, which stripping every combining mark would
have merged.
...
option is ignored and the portable path serves the query. Never a wrong answer,
just not the faster one.
