...
that retries makes progress; the admin op below finishes it in one go. On a
`.global()` table the read is answered from whatever is indexed at the time.
...
reaches it. (A `.global()` table discards the index and rebuilds it, and answers
from the rebuilt part meanwhile.)
...
backfill leaves it that way indefinitely. (On a `.global()` table it answers from
the documents written after the deploy only.) Declared together with the table
there is nothing to walk, so the index is complete from the start and `staged`
costs nothing.
