...
<Callout type="warn">
    This is not an online migration. The table is unavailable from step 1 through step 5, and the dump is inside that window rather than before it. Take the
    window deliberately: a maintenance flag, or a quiet hour. Snapshot first: [Backups](/docs/concepts/backups) covers point-in-time recovery, and a sharding
    migration is exactly the change you want it for.
</Callout>

1. **Stop writes to the table.** Do this _before_ the dump, not after. Export
   walks the table with keyset pagination — successive live reads, not one
   snapshot — so a row inserted behind the cursor is never dumped, and step 6
   then deletes it from the root DO. That is the one way this procedure loses
   data.
...
One extra step, and it is quiet rather than loud. The D1 **table** itself is
provisioned lazily — `ensureMigrated` runs `CREATE TABLE IF NOT EXISTS` before
the first read or write, so an import into a never-written `.global()` table
succeeds. Its declared **indexes** are not: `lunora migrate generate` emits those,
and the deploy applies them. Import before that and a `unique` index does not
exist yet, so duplicates the constraint would have rejected are accepted without
a word.

```bash
# stop writes first — export is keyset pagination over live reads, not a snapshot
lunora export --tables users --out users.ndjson --prod --url <url>
lunora migrate generate add_users_global   # emits the D1 DDL, indexes included
