...
a table the schema calls global (`GLOBAL_TABLE_NOT_EDITABLE`), and the Studio
redirects a global table to a read-only D1 view. So the root copies have to go
**before** the schema edit, while the table is still shard-local and the writer
still owns it.

<Callout type="warn">
    Writes are unavailable from step 1 to step 6. Export is keyset pagination over live reads, not a snapshot, and step 3 deletes the source rows — so a write
    that lands mid-procedure is lost. Snapshot first: [Backups](/docs/concepts/backups) covers point-in-time recovery.
</Callout>
