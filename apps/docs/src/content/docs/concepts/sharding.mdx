...
6. **Reclaim the root DO's copies.** They are still there, and they are what you
   ...
   resume, not during the window.
    ...
    <Callout type="warn">
        **Clear table and Delete matching remove rows permanently**, including on a [`.softDelete()`](/docs/concepts/schema) table — reclaiming the storage is
        the point of this step, and a tombstone reclaims none. The Studio's other two delete affordances, the row editor's **Delete** and the checkbox
        selection's, keep the table's declared soft-delete behaviour.
    </Callout>
