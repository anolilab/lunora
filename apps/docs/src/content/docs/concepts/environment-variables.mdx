...
but is missing keys the example lists — e.g. you just enabled an addon — it
...
the worker picks it up at request time (see [Security](/docs/concepts/security)).

> Lunora never logs secret values, and the `wrangler-validator` plugin warns when
> a binding marked `secret: true` in your schema is missing from your secret
> list — so a forgotten `wrangler secret put` is a build-time warning, not a
> production 500.

## Accessing `env` inside your app

Cloudflare hands the Worker its bindings as the `env` object. In Lunora you wire
configuration from `env` at the **worker entry** — into `createWorker`, into the
generated DO factory thunks, and into adapter setup — rather than reading
...
- [Deployment](/docs/deployment) — the full secrets-and-deploy flow
- [Security](/docs/concepts/security) — env-driven CORS / CSRF configuration
- [Testing](/docs/concepts/testing) — why keeping `env` at the entry keeps handlers testable
- [@lunora/config](/docs/packages/config) — the `.dev.vars` grammar, scaffolder, and wrangler validator
