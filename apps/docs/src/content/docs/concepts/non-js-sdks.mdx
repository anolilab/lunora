...
- **A live query is a `Stream`.** `watchList(args)` (and `client.watch(path, args)`)
  hands back a `Stream` a Flutter `StreamBuilder` consumes directly. Each listener
  opens its own subscription, torn down when it cancels, so disposing the widget
  disposes exactly its own — and two `StreamBuilder`s on one stream cost two server
  subscriptions. The callback-shaped `subscribeList(...)` every other language has
  is there too.
