...
- **Typed models are not available on the JVM.** quicktype's Java and Kotlin
  backends rename fields (a wire `channelId` becomes `channelID`) and emit no
  mapping metadata, so a generated model cannot be projected back onto the wire.
