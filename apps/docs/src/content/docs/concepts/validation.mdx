...
<Callout type="warn">
    ...
    validators from the table shape where you can, or test them against it.
</Callout>
...
- **`v` has refinements.** `.check(predicate, { message, schema })` is the
  general form, and `v.string()` / `v.number()` carry shortcuts over it
  (`.min()`, `.max()`, `.length()`, `.email()`, `.url()`, `.pattern()`,
  `.int()`, `.positive()`). They are enforced
  at parse time and emitted into the JSON Schema.
