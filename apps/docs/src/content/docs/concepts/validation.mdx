...
describe your storage columns and guard the boundary of every function — so
one declaration drives both the database shape and the function's contract,
and the inferred types flow all the way to the generated client.

```ts
import { defineTable, mutation, v } from "lunora/server";
```

Validators come from [`@lunora/values`](/docs/packages/values); `@lunora/server`
re-exports the `v` namespace, so application code imports `v` straight from
`lunora/server`.
...
export const send = mutation
    ...
    .mutation(async ({ ctx, args }) => {
        ...
        return ctx.db.insert("messages", { ...args, userId: ctx.auth.user.id });
    });
```
...
> `.output()` does not apply to streaming/subscription chunks — each chunk is
> yielded as-is. Use it for the single resolved value of a `query` / `mutation`
> / `action`.
...
and output types straight off that `api`, so the **same validators** that parse
on the server type the client call site — pass the wrong `args` shape, or use
the result as the wrong type, and it's a compile error in the browser. There is
no second schema to keep in sync: the validator is the contract on both ends.
