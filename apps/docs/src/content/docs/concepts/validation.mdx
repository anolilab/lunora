...
- **`.safeParse()` returns a different shape.** `{ ok, value }` on success and
  `{ ok: false, error }` on failure — not zod's `{ success, data }`. `.parse()`
  matches zod. Code that reads `.success` compiles and is always `undefined`.
