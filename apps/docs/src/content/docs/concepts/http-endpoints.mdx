...
— an inbound webhook, an OAuth callback, a REST surface for a non-Lunora client,
a file download. That's what `httpAction` and `httpRouter` are for.
...
handler. It's the **raw escape hatch** — you get the underlying `Request` and
return a raw `Response`, so you own status codes, headers, and body. Mount it
with any Hono verb: `app.post(path, httpAction(fn))`, `app.all(path, httpAction(fn))`.

## `HttpActionCtx`

HTTP actions run in the worker (the "action runtime"), separate from the
transactional store. The context is therefore a **narrower view** of `ActionCtx`
— it carries:
...
There is **no direct `ctx.db` / `ctx.scheduler` / `ctx.storage` / `ctx.vectors`**
on an HTTP action — go through `runQuery` / `runMutation` / `runAction` to read
or write your tables. (For streaming a stored R2 object straight from a handler,
...
params, body, and path params plus an optional `.output()` schema — and compiles
down to the same handler type as `httpAction`, so the two are interchangeable
when mounted:

```ts
import { httpRoute, httpRouter, v } from "lunora/server";
import { api } from "@/lunora/_generated/api";
...
**500**. A handler that returns `undefined` produces a **204**.

For Server-Sent Events, swap the terminal `.handler()` for `.stream()` — the
handler is an async generator whose yielded chunks become SSE `data:` frames,
with the client's disconnect wired through to an `AbortSignal`.
...
the router is run outside the runtime — a misconfiguration surfaced loudly rather
than leaving `c.var.lunora` silently `undefined`.
