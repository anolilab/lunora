...
- **Email / password** — PBKDF2-SHA-256 hashing that runs in WebCrypto, no Node
  polyfills.
...
user dashboard — is in the [package reference](/docs/packages/auth).
...
`get-session` endpoint and refetched whenever the token changes, and the value is
