...
The report also reaches the browser's other tabs **on the same origin**. Each
one re-checks the session without reloading, and retires the previous user's
rows the same way. A sign-out made through better-auth's own client in another
same-origin tab is picked up even without the plugin.
