...
- `createLunoraAuthClient` installs `lunoraSessionSync()`, a better-auth client
  plugin that does the same after every auth request that can change the
  session. If you call `createAuthClient` yourself, add it:
...
Every tab of the browser shares one cookie, so the report reaches the other
tabs too: each re-checks the session, without reloading, and retires the
previous user's rows the same way. A sign-out made through better-auth's own
client in another tab is picked up even without the plugin. A session changed
any other way (a custom endpoint) reaches the client when its socket next
reconnects. Call `notifyLunoraSessionChange()` from
