...
destroying them. A consumer below the window is then served out of the archive
...
<Callout>
    Read replicas do not use the archive. A follower below the floor still pays a full bootstrap — the replica pull path checks the floor before it reads, and
    reaching the archive there would put an object-storage round trip on a per-follower poll.
</Callout>
