...
Both tiers cap the snapshot they will build, and for different reasons.
`create --bucket` refuses above **32 MiB**, the limit of the checksum-verified
upload route — bigger would mean an unverified signed PUT, which is not what the
copy you restore from should depend on. The scheduled backup refuses above
**24 MiB**, because it assembles the snapshot inside a Worker isolate. Either
...
Note the key uses the file-name form of the id — dashes, not colons:
