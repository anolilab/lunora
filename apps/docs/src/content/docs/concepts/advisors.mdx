...
<Callout type="warn">
    `constraint_validator` is declared but does not currently run. It needs sampled rows from each table, and the Studio does not gather them — reading a sample
    of every table on each Advisors load is a real cost, and it has not been taken. Nothing else checks these constraints, so treat dangling foreign keys, nulls
    in non-optional columns, and duplicates on a unique index as **unchecked** today rather than as clean.
</Callout>
