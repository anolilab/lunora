...
| `procedure_not_registered`          | warn  | An export whose resolved TYPE is a procedure never reached `_generated/api.ts`: it exists at runtime, and callers see `Property '<name>' does not exist`.                        |
