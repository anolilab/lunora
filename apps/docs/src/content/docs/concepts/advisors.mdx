...
The last four come from codegen rather than `@lunora/advisor`, so they are not in the
programmatic `ALL_LINTS` catalog below — they travel the same channel to the terminal and the
studio.
...
| `procedure_not_registered`          | warn  | An export whose resolved TYPE is a registration — procedure, hook, reactor, mutator, shape, migration, workflow, queue, agent or container — never reached the generated output: produced by a factory or an alias, or (for the single-module kinds) sitting outside `lunora/<kind>.ts`. |
| `procedure_type_check_unavailable`  | warn  | Types don't resolve for `lunora/` where codegen runs, so `procedure_not_registered` cannot report anything.                                                                                                                                                                              |
| `procedure_arguments_unreadable`    | warn  | A registered procedure's `.input()` record could not be read, so `api.ts` types fewer arguments than the runtime enforces.                                                                                                                                                               |
| `procedure_return_type_erased`      | warn  | A registered procedure's return type could neither be named nor reproduced in `_generated/`, so `api.ts` types it `unknown` while the runtime still returns the shape.                                                                                                                   |
