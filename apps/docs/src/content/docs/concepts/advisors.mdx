...
| `constraint_validator` | warn      | Sampled rows that violate a declared constraint: a dangling foreign key, a null in a non-optional column, or a duplicate on a unique index. **Not currently active** — see below. |
...
<Callout type="warn">
    `constraint_validator` is declared but does not currently run, and it is missing **two** inputs, not one. It needs sampled rows from each table, which the
    Studio does not gather (reading a sample of every table on each Advisors load is a real cost, and it has not been taken) — and it needs the schema, which
    the Studio passes as `{ tables: [] }`, so it cannot resolve a table's columns, relations, or unique indexes either. Supplying samples alone would not
    activate it. Nothing else checks these constraints, so treat dangling foreign keys, nulls in non-optional columns, and duplicates on a unique index as
    **unchecked** today rather than as clean — for rows that predate the constraint
    or arrived through a bulk import. A unique index declared in the schema *is*
    enforced by both backends on the normal write path; what nothing checks is
    whether existing rows already violate one.
</Callout>
