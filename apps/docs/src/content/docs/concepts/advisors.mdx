...
| `procedure_not_registered`          | warn  | An export whose resolved TYPE is a registration never reached the generated output: it exists at runtime, but callers see `Property '<name>' does not exist` — and a dropped `onConnect` / `onQueryChange` has no caller at all, so it silently never fires. |
