...
What sets Lunora apart from a live-database-only advisor is that most lints are
...
| `nondeterministic_query_mutation`   | warn  | A `query`/`mutation` calls `Date.now`/`Math.random`/`fetch` — non-deterministic reads break caching and replay. Move side effects into an `action`. |
...
- [Studio](/docs/packages/studio#advisors) — the Security, RLS, and Insights panels.
- [Data masking](/docs/concepts/masking) — what `mask_uncovered_pii_column` guards.
- [Workflows](/docs/packages/workflow) — what the `workflow_*` lints guard.
