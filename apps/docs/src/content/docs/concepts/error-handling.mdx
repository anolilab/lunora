...
`optimisticUpdate` is what names the query a write patches. (The per-call
`optimistic: (current) => next` shortcut targets only the subscription registered
under the mutation's own reference and args — a counter or a document by id, not
a `send` updating a `list`.)
