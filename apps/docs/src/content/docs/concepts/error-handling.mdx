...
Errors in Lunora travel one direction — a handler throws on the server, the
runtime maps it to a transport response, and the client surfaces it as a
rejected promise (or as `error` on a hook). The only decision you make is
**which kind of error you throw**, because that decides what the caller sees.
...
failures throw naturally — the runtime will hide their detail for you.
...
value onto every subscribed query it touches **and records a rollback closure
for each**. If the mutation rejects — a thrown `LunoraError`, a `CONFLICT`, a
network failure — those closures fire and the cache reverts. You don't write any
rollback code; a rejected mutation simply leaves the UI as it was.
