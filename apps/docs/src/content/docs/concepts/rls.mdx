...
- **Reads are AND-merged.** Every `get`, `findFirst`/`findMany`, and the
  ...
  On that fluent chain over a sharded table, a policy made of equality checks
  (`eq`, `ne`, `in`, `notIn`, `isNull`) on string, id, number or boolean
  columns, with a value of the column's own type, goes into the SQL, so
  `take(n)` reads about `n` rows. Any other policy (ranges, `contains`, `NOT`,
  relation predicates, a value whose type differs from the column's), and any
  policy behind `mask()`, is checked row by row: bounded reads then fetch rows
  in growing batches until they have `n` matches. A `.global()` table only
  offers the search chain (`withSearchIndex()`); the same equality policies go
  into its search SQL, except string and id comparisons on MySQL, whose
  collation may fold case. Any other policy there is checked over the whole
  relevance window (at most 1,024 matches). Either way the policy is enforced
  on every returned row. A pushed policy makes a guarded search behave like an
  unguarded one at the cap: `collect()` or iteration over more than 1,024
  visible matches, or `take(n)` with `n` above 1,024, throws `BAD_REQUEST`
  instead of returning a truncated window.
