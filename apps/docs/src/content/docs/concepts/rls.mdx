...
on a to-one relation, `some`/`none`/`every` on a to-many — and the `@lunora/do`
pre-resolver resolves it (a batched semijoin, or a correlated `EXISTS` when the
...
`insert`/`update`/`delete` decision throws `RELATION_PREDICATE_UNSUPPORTED` —
use a flat column check (or a read policy) instead. `rank`/`rankPage` likewise
...
upgrade (unforgeable, carried by the connection), not a per-request claim — so
...
test — no Worker, Durable Object, or live socket. `expectPolicy(policies)` from
`@lunora/server/rls/testing` evaluates them through the **same** primitives the
`rls()` middleware runs at request time, so a green test means the policy
behaves identically in production:

```ts
import { definePolicies, definePolicy } from "lunora/server";
...
`.as(identity)` binds a caller (`userId` / `roles` / `identity`, all optional —
