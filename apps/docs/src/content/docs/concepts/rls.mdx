...
- **Reads are AND-merged.** Every `get`, `findFirst`/`findMany`, and the
  ...
  policy on a `.global()` table or behind `mask()`, is checked row by row: bounded
  reads then fetch rows in growing batches until they have `n` matches. Either
  way the policy is enforced on every returned row.
