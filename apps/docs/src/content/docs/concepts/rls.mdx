...
- **Reads are AND-merged.** Every `get`, `findFirst`/`findMany`, and the
  ...
  On that fluent chain, a policy made of equality checks (`eq`, `ne`, `in`,
  `notIn`, `isNull`) on string, id, number or boolean columns goes into the SQL,
  so `take(n)` reads about `n` rows. Any other policy (ranges, `contains`,
  `NOT`, relation predicates) is checked row by row, and bounded reads fetch
  rows in growing batches until they have `n` matches.
