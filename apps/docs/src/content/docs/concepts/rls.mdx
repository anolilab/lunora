...
- `auth.roles`: the request's role list, read from the `roles` claim on the
  resolved identity (a `string[]`, or the comma-separated string better-auth's
  `admin()` plugin emits). Empty when the claim is absent.
