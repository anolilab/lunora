...
device's notifications. A row owned by someone else is left alone silently, so
the call cannot be used to probe which endpoints exist. Register with the same
`userId` you unregister with; devices registered anonymously all share the one
...
service reports as gone (Web Push 404/410, FCM's `NOT_FOUND` for a dead token):
