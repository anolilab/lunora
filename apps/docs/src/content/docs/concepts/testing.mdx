...
query — exactly the reactivity guarantee your app relies on.
...
> Proving the **full** live path — SSR preload → WebSocket push → second-tab
> update — needs a running, scaffolded app and is a manual procedure, not a unit
...
The harness is plain Vitest, so it runs anywhere `pnpm run test` does — no
Cloudflare account or `wrangler` login needed. Two things to wire up:

- **`lunora codegen --check`** before the test job, so a stale `_generated/` is a
  build failure rather than tests passing against old types (the same gate
...
- [Queries & mutations](/docs/concepts/queries-mutations) — the `query` / `mutation` / `action` functions the harness runs
- [Manual end-to-end verification](/docs/frameworks/verify-live-loaders) — proving the full live-loader path
- [Deployment](/docs/deployment) — the `codegen --check` CI gate
- [@lunora/testing](/docs/packages/testing) — full harness and mail-catcher reference
