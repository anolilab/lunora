...
Lunora gives you several places to look when something misbehaves. They are
listed from the cheapest outward, roughly in the order you should try them.
...
`lunora insights` ranks the live worker's functions three ways, by write-conflict
hot-spots, error hot-spots, and latency outliers:

```bash
lunora insights
lunora insights --format json  # pipe into CI or a script
```
...
see [Observability](/docs/concepts/observability#sinks) and
[Logpush](/docs/deployment#logpush).
