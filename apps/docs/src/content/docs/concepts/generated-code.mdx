...
schema**, so `ctx.db` knows your tables. Author functions importing from the
generated `server` (not the base package) and `ctx.db.query("messages")`,
...
- depend on `lunora` → the generated files import from `lunora/server`,
  `lunora/server/types`, `lunora/client`, etc.
