...
**…or from the Studio.** Pass the same table config to your worker as
`logArchive`, and the Studio **Logs → Archive** feed reads it back through an
admin-gated route (the worker holds the R2 SQL credentials and runs the reader —
