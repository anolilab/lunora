...
in a shared collector — and, because the head-sampling decision is derived from
the trace id, choose its own sampling outcome.
...
one-time hint naming it — a broken waterfall shouldn't be silent. Setting the
option to `false` keeps the behaviour and silences the hint.
...
the trace containing it. In the **Logs** panel, a line emitted inside a dispatch
carries a **Trace** link that opens its waterfall on the shard it came from.
...
spans at all — cheaper still, but you lose the baseline you would compare a
regression against. Reach for `sampling` first; use `onlyErrors` on a sink whose
...
| `http.route`                | always       | the function path — the logical route   |
...
ids or user identifiers — worth knowing before pointing a sink at a third party.
