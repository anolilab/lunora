...
console-style, so existing `console`-shaped calls are unchanged. The rendered
message and the structured fields reach the dev terminal, Workers Logs, and any
sink; the raw positional args of a console-style call reach only the in-process
`onLog` sink you control.
