...
A **container** applies the same bias to its own spans, with one honest limit: it
is a long-running process with no dispatch boundary to re-decide at, so it keeps
the span that errored rather than holding and re-exporting that span's siblings.
Set `alwaysSampleErrors: false` on `createContainerTelemetry` (or
`LUNORA_SAMPLE_ERRORS=0` in the container env) to turn it off there.
