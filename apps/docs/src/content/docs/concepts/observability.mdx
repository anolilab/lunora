...
export const checkout = action({
    handler: async (ctx, args) => {
        ...
        // Nesting is lexical — a `ctx.trace` inside another span's body is
        // parented to it, so the shape of the code is the shape of the waterfall.
        await ctx.trace("fulfil", async () => {
            await ctx.trace("reserve.stock", () => reserve(cart));
            await ctx.trace("email.receipt", () => sendReceipt(charge));
        });
    },
});
```
