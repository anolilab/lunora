...
deploy time with no code change. Exports are fire-and-forget; on a Worker each is
registered with `waitUntil` so it outlives the response.

**Batching is on by default.** Events are buffered and shipped as **one POST per
signal per flush** — whichever of 512 buffered events, a 200 ms window, or the
invocation boundary comes first. A Worker is capped at 50 (free) / 1000 (paid)
subrequests per invocation, so a per-event POST is a correctness problem, not
just an efficiency one. Pass `batch: false` to restore one POST per event, or
`batch: { maxItems, maxDelayMs }` to retune the window.
...
otlpSink({
    ...
    serviceVersion: env.CF_VERSION_METADATA?.id, // service.version — the binding is `{ id, tag, timestamp }`, not a string
...
`service.version` (from `SERVICE_VERSION` / `CF_VERSION_METADATA` — the
`version_metadata` binding's `tag`, else its `id` — / `VERCEL_GIT_COMMIT_SHA` /
`GITHUB_SHA` / `COMMIT_SHA`), `deployment.environment`
...
# resume the next page with the opaque cursor token the previous run printed
lunora logs --durable --table logs --cursor eyJ0cyI6MTczNzQ2MDAwMDAwMH0
```

The reader paginates by keyset on `ts DESC`, not `OFFSET`, so deep pages stay
cheap over Iceberg. The predicate is **inclusive** (`WHERE ts <= cursor`) and the
cursor carries a `seen` set of the boundary millisecond's already-returned rows,
so ties are neither dropped nor duplicated — pass `page.nextCursor` (or the CLI's
printed token) back whole. A bare `ts` is accepted for convenience, but without
`seen` it re-emits every row sharing that millisecond. Filters (`sinceTs`/`untilTs`, `level`,
...
### Attributing a spike to a deploy

Bind Cloudflare's version metadata and the runtime stamps `deploymentId` /
`versionTag` onto every durable request-log row and every emitted Workers-Logs
event:

```jsonc
// wrangler.jsonc — every `lunora init` template ships this
"version_metadata": { "binding": "CF_VERSION_METADATA" },
```

That turns "latency doubled some time this afternoon" into a group-by rather
than a graph-correlation exercise: filter the Requests view or the Workers Logs
Query Builder on the deploy id and the before/after split is exact. It costs
nothing when unused, and nothing reads it but the stamp.

Pair it with `"observability": { "logs": { "invocation_logs": true } }`, which
the templates also ship — the Query Builder groups on the per-invocation summary
line, and without it only explicit `console` events survive, leaving the
group-by nothing to group.
...
re-deciding. That includes a **batched** call (`client.batch(...)`, and the
coalesced replay of queued offline writes): one verdict covers the whole batch
and every sub-request carries it, so a batch is kept or dropped as one thing
rather than each entry deciding for itself.
...
if anything failed and discards them otherwise. The hold is capped at **500
`ctx.trace` spans per trace**, oldest dropped — a loop that traces per item past
that exports only its tail.
...
A **container** applies the same bias to its own spans, and is _told_ the
verdict rather than guessing it: the shard stamps this dispatch's toggle onto
every `ctx.containers.<name>` request beside the `traceparent`, so
`createContainerTelemetry({ request })` picks up both. One honest limit — a
container is a long-running process with no dispatch boundary to re-decide at,
so it keeps the span that errored rather than holding and re-exporting that
span's siblings.
...
Pass `alwaysSampleErrors` explicitly to override what the request carries. A
one-shot container with no request to read falls back to `LUNORA_SAMPLE_ERRORS`
in its env, then to the same default as every other tier.

:::note[`headRate` and the trace id]
`headRate >= 1` keeps everything and `<= 0` drops every non-error trace; in
between, the **last** 8 hex characters of the trace id (its low 32 bits) are
mapped onto `[0, 1)` and compared against the rate — the same rightmost portion
OpenTelemetry's `TraceIdRatioBased` sampler uses, because an inbound id is not
necessarily uniform across all 128 bits (a 64-bit id left-pads with zeros; X-Ray
puts epoch seconds in the leading digits).
...
That keeps the sink model identical to logs and spans. Batching means a hot loop
costs buffered events rather than one POST each, but the measurements themselves
are still one per call — sum locally and record once at the end rather than
...
**Which trace a span lands on** depends on whether the instance has an anchor:
a `traceparent` option, the inbound `request` (its `traceparent` header), or
`LUNORA_TRACEPARENT` in the env. An anchored instance puts every span under that
trace and stamps it on every log, so one request or one job reads as one thing.
An unanchored instance gives each span its own trace and leaves logs unstamped.
So in a long-running server, create the telemetry **per request** from the
inbound request rather than once per process:

```ts
app.post("/transcode", async (request) => {
    const telemetry = createContainerTelemetry({ request }); // one trace per request
    // …
});
```

A module-scope instance is right for a one-shot job started with
`LUNORA_TRACEPARENT`, where the whole process is one unit of work.
...
It takes most of the same [resource attributes](#resource-attributes) as
`otlpSink` (`serviceVersion`, `deploymentEnvironment`, `resourceAttributes`, and
an opt-in `detectResources`), so container and worker telemetry can be grouped by
the same service and version. There is no `serviceNamespace` option here — pass
`resourceAttributes: { "service.namespace": "…" }` to set it:
...
Lunora's own sinks emit **JSON**, gzipped once the body clears 1 KiB (which a
batched flush essentially always does). The cloud ingest accepts that, and also
standard OTLP **protobuf** (`Content-Type: application/x-protobuf`, what most
Collectors default to) on all three endpoints, so any OpenTelemetry SDK or
Collector can ship to it unchanged. A third-party collector or proxy behind the
sink has to decode `content-encoding: gzip`, or set `batch: false` and keep
bodies under the threshold. A capped batch comes back
...
| `content-encoding`                 | `gzip`, on any body of 1 KiB or more                      |
