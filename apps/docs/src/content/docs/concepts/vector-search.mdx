...
Run the `backfillVectors` admin operation once per shard after deploying a new
vector index over a table that already has rows:

```sh
lunora run '__lunora_admin__:backfillVectors' --args '{"maxPages":5}'
```

Each call embeds at most `maxPages` pages of 100 rows (one page when you omit it),
because every row is a remote embed plus a Vectorize upsert and a large table
cannot be embedded in one request. The result reports
`{ done, pages, rows }`. Progress is stored in the shard, so repeat the call until
`done` is `true`. A page that fails is retried by the next call, not skipped.
...
in place, never cleared first. A change to the `embed` function itself (a
different model with the same dimensions) is invisible to that check, so pass
`{"restart":true}` once to re-embed everything.

A backfill never overwrites a newer write. Each page is read inside the shard's
single-writer gate and synced in commit order with live writes. A row patched or
deleted while its page is embedding ends up with the patched vector, or none.
