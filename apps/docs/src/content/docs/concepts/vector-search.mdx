...
export const ShardDO = createShardDO({
    vectors: (env) => {
        const client = fromPostgresJs(postgres(createHyperdrive(env.HYPERDRIVE).connectionString));

        return { "docs-body": createPgVectorIndex({ client, dimensions: 768, metric: "cosine", name: "docs_body" }) };
    },
});
```
