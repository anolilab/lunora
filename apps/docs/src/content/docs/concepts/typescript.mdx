...
arguments and for table columns alike:
...
`args.filters` above is typed from the Zod schema rather than `unknown`. A shared
`const` resolves the same way: a validator hoisted into a named `const` and
reused across several `.output()` calls resolves the same as one written inline,
so a table's public shape can live in one place.
...
see [Types outside the schema](/docs/concepts/data-types#types-outside-the-schema).
...
Generated files are deterministic, so commit them and let CI prove they match the
schema:
