...
are all things you want to happen _later_ — after the mutation commits, on a
delay, or on a recurring schedule. [`@lunora/scheduler`](/docs/packages/scheduler)
covers both shapes: **deferred** jobs through `ctx.scheduler`, and **recurring**
jobs through Cron Triggers, both backed by a `SchedulerDO` you mount once per
app.
...
mutation or an action — including an [internal](/docs/concepts/function-context)
...
- From a **mutation**, scheduling is how you move a side-effect _out_ of the
  transactional read/write scope. A mutation can't call `fetch()` or write to R2;
  it schedules an **action** to do that once the write has committed.
