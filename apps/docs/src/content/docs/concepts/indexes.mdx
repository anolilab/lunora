...
row and filters in memory — fine for a handful of rows, a problem as the table
grows. A `.withIndex()` read seeks straight to the matching range. As a rule,
any read that constrains a column should constrain it through an index, and any
foreign-key column you read by should have an index leading with it.
...
- **`unindexed_foreign_key`** (info) — a foreign-key column with no index leading
  with it, so reads on it full-scan. (Honours SQLite's leftmost-prefix rule;
...
table without scanning rows — `count` (field-less), `sum`/`min`/`max`/`avg` (take
a `field`).
