...
`lunora info --bindings` answers the first without running anything, which is
what a supervisor wants while it is still planning the graph:

```bash
lunora info --bindings                  # human-readable
lunora info --bindings --json           # the manifest a deployer or runner consumes
