...
`lunora dev --emit-bindings <file>` writes both, as the same document
`lunora build --emit-bindings` produces for a deployer, plus a `dev` section:
...
One derivation feeds both commands, so the requirements a deployer provisions
cannot drift from the ones your dev graph proxies. A project with no readable
`wrangler.jsonc` fails the run rather than emitting an empty manifest: "this
Worker needs nothing" is a claim a supervisor would act on.
