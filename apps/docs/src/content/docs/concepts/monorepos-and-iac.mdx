...
until [ "$(lunora dev status --json | jq -r .ready)" = "true" ]; do sleep 0.5; done
pnpm run test:integration
```

`lunora dev status` shows the same thing for a human, as `ready` or `starting`
in its detail line. On the Vite flavors `@lunora/vite` writes the record itself,
once Vite has resolved its URLs — so there the record appearing at all already
means ready.
