...
<Callout type="info">
    **The Durable Object makes the *claim* atomic — that is the part you would otherwise have to build.** A cold key with a hundred concurrent callers is the
    ...
    I/O, so the winner computes in the action and writes back after. That means the lease is what serialises callers, not the write — and a lease needs a
    duration, so pick one longer than the work and shorter than your patience.
