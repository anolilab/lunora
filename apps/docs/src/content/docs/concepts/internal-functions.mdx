...
the same in every way except its **visibility**: it is never exposed on `api`,
the Durable Object's external RPC path rejects it, and it is reachable **only
server-side** via `ctx.runQuery` / `ctx.runMutation` / `ctx.runAction` against
...
```ts
import { internalMutation, internalQuery, v } from "lunora/server";
...
transactional `MutationCtx`, `internalAction` the side-effectful `ActionCtx`.
