...
The `@lunora/mcp` server exposes the same traversal as the read-only
`lunora_find_related` tool, so an agent can follow relationships without your
app writing a query for it. It takes `table` and `id` plus the same `depth`,
`direction`, `edges`, `limit` and `cursor` options, and returns the same nodes.
