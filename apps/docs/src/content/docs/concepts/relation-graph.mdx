...
skips what page N returned. The re-walk is bounded by the same depth and limit
caps as the first page.
...
const docs = defineRag({
    ...
    graphStore: {
        ...
        related: async (sourceIds, { topK }) => {
            const hits = [];

            for (const id of sourceIds) {
                const { nodes } = await ctx.db.related({ table: "tickets", id }, { depth: 2, limit: topK });

                hits.push(...nodes.map((node) => ({ id: `${node.document._id}#0`, score: node.score, text: String(node.document.body) })));
            }
