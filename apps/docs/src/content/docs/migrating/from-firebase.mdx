...
<Callout type="info">
    ...
    script's output reads as-is — but if you write your own dumper against the REST API instead, you will see the string/base64 forms and those
    read too.
</Callout>
...
<Callout type="info">
    ...
    lossy and deliberate — emitting the RFC-3339 string for the rows that have sub-millisecond digits would make the column number for most rows and string for
    a few, which no schema describes. If the sub-millisecond part is load-bearing for you (ordering writes that land inside the same millisecond), carry the
