...
5. **Export Convex data**: `npx convex export --path ./convex-export --include-file-storage`
    ...
    **File storage needs the opt-in.** `lunora import` skips the `_storage`
    ...
    every `{ $storage: id }` reference is rewritten automatically. Plain-string
    columns that hold storage ids (validated with `v.id("_storage")` in Convex)
    are ambiguous, so they are rewritten only through a
    `lunora/import-convex.json` mapping — run with `--scan` to have the CLI
    detect the candidate columns and print the file for you to confirm:

    ```bash
    lunora import ./convex-export --with-storage --scan
    ```

    A `snapshot.zip` from `npx convex export --path ./snapshot.zip` imports the
    same way (`lunora import ./snapshot.zip --with-storage`); and `--verify`
    checks each table's inserted count against its source line count, exiting
