...
5. **Export Convex data**: `npx convex export --path ./convex-export --include-file-storage`
    ...
    **File storage needs the opt-in.** `lunora import` skips the `_storage`
    ...
    <Callout type="info">
        ...
        also verified only after the write, by size plus SHA-256 where the bucket records one, and an object that fails is deleted rather than left behind. If
        you have blobs over 32 MiB and no signed-`PUT` route, copy them across with `wrangler r2 object put` and add their columns to the mapping by hand.
    </Callout>
...
- **Self-hosted studio, not managed**. There's no hosted control plane like
  Convex's. Instead `@lunora/studio` ships a studio that
