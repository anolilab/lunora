...
5. **Export Convex data**: `npx convex export --path ./convex-export` produces a
   JSONL dump per table. Point `lunora import` straight at that directory —
   it reads the export layout, so there is no reshaping step:
