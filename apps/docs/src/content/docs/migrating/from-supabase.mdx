...
6. **Swap auth + storage.** Supabase Auth → [`@lunora/auth`](/docs/packages/auth)
   ...
   `ctx.storage`.
7. **Export & import data.** Dump each table (`pg_dump`/`COPY … TO` or the
   dashboard export), reshape to your new schema (`timestamptz` → numeric
   `createdAt`; `uuid` PKs → your `id` columns), then batch-insert via a Lunora
   mutation or a `lunora run` script. Any `.global()` table needs a
   `lunora migrate generate init` schema step plus a one-off data-import
   migration.
