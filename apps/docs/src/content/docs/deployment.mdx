...
- **Write `target` and `remote` as literals.** `lunora codegen` resolves the
  ...
  `lunora verify` reports `platform_unreadable_target`. A `const` in the same
  file and a shorthand property both work.
