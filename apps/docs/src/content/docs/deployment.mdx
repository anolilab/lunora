...
## Vite-first apps: `lunora/app.ts`

`@lunora/vite` generates the worker entry for a Vite-first app, so there is no
file to chain your own `defineApp()` calls on. Anything mechanical is composed
for you — the shard selector, and `.scheduler(...)` as above. Anything that needs
_your_ code is not: `.auth(...)` takes your better-auth options, `.global(...)`
your D1 writer, `.vectors(...)` your embedder, and `resolveIdentity` is only ever
set by `.auth()`, `.access()` or `.extend()`.

Put those in `lunora/app.ts`, exporting `configureApp`:
...
export const configureApp = (app: AppBuilder<Env>): AppBuilder<Env> =>
    app
        .auth({
            d1: (env) => env.DB,
...
The explicit `: AppBuilder<Env>` return type is worth keeping. The generated
entry is a virtual module that no `tsconfig` covers, so nothing type-checks the
seam itself — annotate the return and a forgotten `return` fails `lunora verify`
instead of the worker failing to boot.

The file is optional, and it must export `configureApp` as a value: anything
else (a type-only export, a default export, a file that does not parse) is
ignored and the entry composes as if it were absent. A hand-written entry
(`src/server.ts`, `src/worker.ts`) needs none of this — chain the calls there.
