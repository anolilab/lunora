...
Non-interactive runs (CI) skip the prompt. You can also manage keys by hand with
`lunora env set NAME VALUE` and `lunora env list`.

To check a `.dev.vars` against its example for missing keys, still-unset
