...
:::note[Vite-first apps: the binding is the opt-in]
...
"virtual:lunora/worker"`) that entry is generated, so there is no file to add the
re-export to — declaring the binding above is what tells `@lunora/vite` to
compose `.scheduler(...)` and re-export the class.

Cron jobs need none of this: they run through the worker's own `scheduled()`
handler, so they work with or without the binding. Only deferred dispatch and the
studio's scheduled-jobs view require it.

Add the `migrations` entry in the same edit — wrangler requires one per Durable
Object class, and migration tags are append-only. `lunora verify` reports both if
either is missing.
:::

## Vite-first apps: `lunora/app.config.ts`
...
`.scheduler(...)` is composed for you because it is mechanical — the namespace
selector and nothing else. Everything that needs _your_ code is not:
...
Put those in `lunora/app.config.ts`, exporting `configureApp`:

```ts
// lunora/app.config.ts
import type { AppBuilder } from "./_generated/app";

export const configureApp = (app: AppBuilder<Env>) =>
    app
        .auth({
            d1: (env) => env.DB,
            options: (env) => ({ secret: env.BETTER_AUTH_SECRET, baseURL: env.BETTER_AUTH_URL }),
        })
        .global({ d1: (env) => env.DB });
```

The generated entry calls it between the shard selector and its own
`.httpRouter(...)` / `.build()`, so the framework wiring stays with the plugin
and your capabilities are chained in the middle. The file is optional: without
it the generated entry is byte-for-byte what it was.

A hand-written entry (`src/server.ts`, `src/worker.ts`) needs none of this —
chain the calls there directly.
