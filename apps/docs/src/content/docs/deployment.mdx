...
- **`lunora prepare`** runs this exact pipeline and stops before step 4 — same
  gates, same verdict, without publishing or building an image. It writes
  (`_generated/`, `wrangler.jsonc`), so use it when you want the reconciled
  config too.
