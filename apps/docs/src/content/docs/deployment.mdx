...
`lunora.config.js`, `.mjs`, `.cjs`, `.mts` and `.cts` are read too, so a
JS-authored project is not forced into TypeScript for one file. Every template's
`tsconfig.json` lists it in `include`, which is what gets it type-checked.
...
- **Keep the type imports type-only.** The `app` hook is bundled into the worker,
  so a runtime `import` here ships with it — and a host-only one breaks the build.
  `import type` is erased, which is why the shape above uses `satisfies` rather
  than a `defineConfig()` call.
- **Annotate the hook's return type.** The generated entry is a virtual module no
  `tsconfig` covers, so nothing type-checks the seam itself; the annotation is
  what turns a forgotten `return` into a `lunora verify` failure instead of a
