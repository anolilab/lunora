...
```bash
pnpm create lunora@latest chat
cd chat
pnpm install
```
...
`lunora deploy` runs codegen, validates `wrangler.jsonc`, applies any pending
...
- [Concepts: sharding](/docs/concepts/sharding) — when to reach for `.shardBy()`
  vs. `.global()`
- [@lunora/auth](/docs/packages/auth) — replace the `must be signed in` stub
  with real sessions
- [@lunora/storage](/docs/packages/storage) — attach images and files to messages
