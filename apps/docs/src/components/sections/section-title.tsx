...
const SectionTitle: FC<{
    ...
    <div className={cn("flex flex-col gap-5", mode === "dark" && "dark", classes?.root)}>
        {prefix && (
            <span className={cn("flex items-center gap-2 font-mono text-sm tracking-wider uppercase", mode === "dark" ? "text-ink-faint" : "text-gray-400")}>
        ...
        {description ? (
            <div
                className={clsx("text-wrap-balance mt-2 text-base/7", mode === "dark" ? "text-ink-muted" : "text-gray-500", {
                    "pl-20": position === "right",
                    "pr-20": position === "left",
                })}
            >
