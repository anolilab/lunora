...
// Self-contained flat config for @cirrus/docs. Each package owns its own
...
export default createConfig(
    ...
    // "docs"/"Docs" is the product/domain name of this app (fumadocs), not an
    // abbreviation to expand; `docs` is also the symbol name emitted by
    // fumadocs-mdx into `.source/server`, so renaming it would break the import.
    // `Props`/`Params` are framework vocabulary: Next.js requires the exact
    // export name `generateStaticParams`, and `PageProps` is the conventional
    // local props type. Extend (not replace) the preset's default replacements.
    {
        rules: {
    ...
    // Next.js uses the automatic JSX runtime (its SWC/Babel transform injects
    // `react/jsx-runtime`), so `import React` is never needed in components.
    // The tsconfig here is `jsx: "preserve"` (Next compiles JSX itself), which
    // is why the preset can't auto-detect the automatic runtime and otherwise
    // flags every JSX element. Scope this to the app's component files only.
    {
        files: ["app/**/*.{ts,tsx}"],
        rules: {
