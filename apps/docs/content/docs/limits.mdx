...
Cirrus's design choices are driven by Cloudflare's per-primitive limits.
...
| Disk                 | **ephemeral**                           | Persist to R2 (`@cirrus/storage`), not local disk       |
| Concurrent / account | **1 500 vCPU / 6 TiB mem / 30 TB disk** | Raisable via support                                    |
| Registry storage     | **50 GB / account**                     | Prune old images with `cirrus containers images delete` |
...
These are pre-emptive — well below the hard ceiling — so you have time to
react.
