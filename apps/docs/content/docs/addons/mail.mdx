...
`@cirrus/mail/inbound` is the inbound counterpart: it turns a Cloudflare [Email
Worker](https://developers.cloudflare.com/email-routing/email-workers/) delivery
into a call to one of your Cirrus functions. The package never imports
...
`dispatchToCirrusFunction` posts an RPC envelope to the root shard stub — the
...
`CIRRUS_ADMIN_TOKEN` to authorize the RPC (read from `env` by default). On a
...
export const onEmail = mutation({
    ...
    handler: async (ctx, { from, subject, text }) => {
...
Cirrus validates the `send_email` binding shape but does not manage routing
