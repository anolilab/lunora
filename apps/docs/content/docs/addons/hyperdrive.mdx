---
title: "@cirrus/hyperdrive"
description: Bring-your-own Postgres/MySQL from an action via Cloudflare Hyperdrive.
---

`@cirrus/hyperdrive` lets an **action** read and write an existing
...
> **Integrate an existing database — don't replace the Cirrus data layer.**
> Hyperdrive points at a database Cirrus has no visibility into, so two
> invariants the rest of Cirrus relies on do **not** hold for it:
>
...
>    `UPDATE` issued over Hyperdrive produces **no** Cirrus change event, so
...
2. Add the binding to `wrangler.jsonc`. Use `localConnectionString` so local
   dev (`cirrus dev`) connects straight to your DB without the edge proxy:
    ...
    Cirrus validates the binding (it errors when `binding` is missing and warns
...
create` can mint, so importing `@cirrus/hyperdrive` surfaces a hint rather
...
export const importLegacyOrders = action({
    args: { orgId: v.string() },
...
Cirrus cannot observe external writes, so a subscription over a `defineSchema`
...
- **No CDC / logical replication.** Cirrus does not ingest your Postgres
