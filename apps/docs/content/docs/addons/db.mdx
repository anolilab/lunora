...
`@cirrus/db` turns your Cirrus queries and mutations into a [TanStack DB](https://tanstack.com/db)
...
It sits on top of `@cirrus/client`'s transport (the same WebSocket subscriptions
and RPC the React hooks use). You keep your schema and functions exactly as they
are; `@cirrus/db` binds them to collections.

## Install

`@cirrus/db` peer-depends on the TanStack packages, so install them alongside it:

```bash
pnpm add @cirrus/db @tanstack/db @tanstack/react-db @tanstack/offline-transactions
```

## Quick start (generated)

You don't have to write the binding by hand. With a schema and functions in
`cirrus/`, generate it from them:

```bash
cirrus codegen                    # ensure _generated/ is up to date
vis generate cirrus-collections   # writes cirrus/collections.ts
```
...
```ts title="cirrus/collections.ts (generated)"
...
import { createCollections } from "../cirrus/collections";

const db = createCollections(client); // `client` from <CirrusProvider>
...
2. **persists the write to a durable outbox** (IndexedDB) and sends it via your
   Cirrus mutation, retrying with backoff until it succeeds — so a write made
...
Cirrus mints the id as usual. See [`ctx.db.insert`](/docs/api/server#client-supplied-ids).

## Manual binding

`defineCollections(client, defs)` is the underlying API. Each entry is:

- `list` — the Cirrus query that lists the rows (the sync source). **Required.**
