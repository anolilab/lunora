---
title: "@cirrus/ratelimit"
description: Token-bucket, fixed-window, and sliding-window rate limiting as procedure middleware.
---

`@cirrus/ratelimit` enforces named rate limits over a pluggable store. You build
...
directly. On rejection the middleware throws a structural `CirrusError` that the
...
```ts
import { RateLimiter, rateLimit, createDbStore } from "@cirrus/ratelimit";
...
> **Failure policy:** the middleware **fails closed by default** — if resolving
> or invoking the limiter throws, it logs via `console.error` and rejects with
> `503`. This is the safer default for security-sensitive limits (auth, account
> creation). Set `failOpen: true` only when degraded availability is preferable
...
| `createDbStore({ db })`   | A Cirrus table via `ctx.db` (the ORM writer on a mutation/action). | Durable per-DO limits inside a procedure, where there is no raw SQL. |

`createSqlStore` creates its table (default `_cirrus_rate_limits`) if missing.
...
```ts
import { defineTable, v } from "@cirrus/server";
...
> Inside a Durable Object the input gate serializes the limiter's
> read-modify-write against storage, so both SQL- and ORM-backed stores are
> atomic against concurrent calls without an explicit transaction. Driving
> `createSqlStore` outside a DO, you own serialization yourself.
...
```ts
import { RateLimiter, ratelimitPlugin } from "@cirrus/ratelimit";
...
- [@cirrus/server](/docs/api/server) — the procedure builder whose `.use()` chain the middleware rides.
