...
Workflows live in `cirrus/workflows.ts` as named exports. Scaffold one with
`vis generate cirrus-workflow --name=orderPipeline`, or write it by hand:
...
| `context.run`    | Call a Cirrus function (`api.*`) by typed reference; pass `{ shardKey }` to target a shard.         |
...
After adding `cirrus/workflows.ts`, re-run codegen (`cirrus dev` does this on
save). It:

1. Emits a `WorkflowEntrypoint` subclass per export into
   `cirrus/_generated/workflows.ts` (`channelWelcome` → `ChannelWelcomeWorkflow`).
...
```ts
// src/server/index.ts
export { ChannelWelcomeWorkflow } from "../../cirrus/_generated/workflows";
```
