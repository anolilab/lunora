---
title: "@cirrus/testing"
description: In-memory harness for queries, mutations, and actions, plus E2E mail-catcher helpers.
---

`@cirrus/testing` is the toolkit for testing a Cirrus app. It ships two
surfaces: an **in-memory function harness** (`cirrusTest`) that runs your
...
helpers** re-exported from [`@cirrus/mail/testing`](/docs/addons/mail) for
driving end-to-end email flows.

```sh
pnpm add -D @cirrus/testing
```

## In-memory harness

`cirrusTest(schema)` runs the migrations against a fresh in-memory SQLite
...
Each `cirrusTest(...)` opens one in-memory database backed by a native handle.
...
`cirrusTest(schema)` returns a `TestHarness`:
...
```ts
import { internalMutation } from "@cirrus/server";
...
const sendViaInternal = mutation({
    args: { author: v.string(), body: v.string() },
    handler: (ctx, args) => ctx.runMutation(internalSend, args),
});
...
const scheduleSomething = mutation({
    args: {},
    handler: (ctx) => ctx.scheduler.runAfter(1000, "noop:fn", {}),
});
...
In `cirrus dev`, [`@cirrus/mail`](/docs/addons/mail) captures every outbound
...
deterministically. They are re-exported from `@cirrus/testing`:
...
gates introspection behind `CIRRUS_ADMIN_TOKEN`), the recipient `to`, and an
...
test("password reset email carries a working link", async () => {
    ...
    const adminToken = process.env.CIRRUS_ADMIN_TOKEN!;
...
| -------------- | ------------ | --------------------------------------------------- |
| `adminToken`   | —            | Admin bearer token (`CIRRUS_ADMIN_TOKEN`).          |
...
- [@cirrus/mail](/docs/addons/mail) — captures the outbound email the helpers read
- [@cirrus/auth](/docs/addons/auth) — the verification / reset / magic-link flows you test end-to-end
