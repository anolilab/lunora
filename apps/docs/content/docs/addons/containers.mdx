...
const job = await cirrus.run(api.jobs.next, { queue: "transcode" }); // typed args + result
```
...
functions run as — without that check, anyone who reaches `/_cirrus/rpc` runs as
...
Generate the token once, set it with `wrangler secret put CIRRUS_CONTAINER_TOKEN`
...
if it can't reach the shard, the `cirrus dev` terminal above remains the source
of truth.
