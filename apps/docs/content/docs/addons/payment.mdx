---
title: "@cirrus/payment"
description: Provider-agnostic payments for Cirrus — Stripe/Polar adapters, webhook sync, and a payment/subscription state machine.
---

`@cirrus/payment` is one provider-agnostic API over Stripe and Polar. Webhook
...
`ctx.payments` is wired by codegen onto `ActionCtx` whenever a `cirrus/` source
imports `@cirrus/payment` or reads `ctx.payments`. The adapter — which carries
...
```ts
import { createPolarAdapter, createStripeAdapter } from "@cirrus/payment";
...
- signature into the shard via `ctx.runAction`, where `ctx.payments` and its
  store exist:
...
> Webhooks are eventually-but-not-guaranteed: an endpoint down past the
> provider's retry window can drop an event for good. `reconcile` re-fetches the
> provider's current truth for given payment/subscription ids and overwrites the
> store when it has drifted — pair it with a `@cirrus/scheduler` job that sweeps
> non-terminal rows.

## Data it stores

Codegen discovers tables by parsing your `cirrus/schema.ts` AST, so it **cannot**
resolve a cross-package `defineSchema({ ...paymentTables })` spread. Declare the
tables you use **inline** in your own `cirrus/schema.ts`, mirroring the columns
in `@cirrus/payment`'s exported `paymentTables` (the canonical column reference
...
The [Studio](/docs/addons/studio) Payments panel (under **Logs**) shows synced
customers, subscriptions, and webhook events for the app — see
[Logs](/docs/addons/studio#logs). The panel only appears once codegen detects
payments in use.

## See also

- [@cirrus/server](/docs/api/server) — `ctx.payments`, mirror tables into your schema
- [@cirrus/scheduler](/docs/addons/scheduler) — drive the reconciliation sweep
- [Studio](/docs/addons/studio) — the Payments panel
