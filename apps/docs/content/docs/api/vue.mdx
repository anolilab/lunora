---
title: "@cirrus/vue"
description: Vue 3 composables built on @cirrus/client — live queries, optimistic mutations, and an SSR hydration handoff.
---

`@cirrus/vue` is the Vue 3 adapter for Cirrus. It's a thin, idiomatic layer over
the framework-neutral [`@cirrus/client`](/docs/api/client) — which owns the
...
| Symbol                 | Kind          | Role                                                                                         |
| ---------------------- | ------------- | -------------------------------------------------------------------------------------------- |
| `createCirrus`         | plugin        | `app.use(createCirrus(client))` — provide the shared client to the whole app.                |
| `provideCirrus`        | composable    | `provide` the client to a subtree from inside a parent `setup()`.                            |
| `useCirrus`            | composable    | Read the `CirrusClient` from the nearest provider. Throws if none is mounted.                |
| `CIRRUS_INJECTION_KEY` | injection key | The `InjectionKey` the provider uses — for advanced manual `inject()`.                       |
...
## Provider — `createCirrus` / `provideCirrus` / `useCirrus`

Mount the single app-wide `CirrusClient` so every composable can resolve it. Use
the plugin form at the app root, or `provideCirrus` inside a parent component's
`setup()` to scope a client to a subtree:

```ts
import { createCirrus, provideCirrus, useCirrus } from "@cirrus/vue";
...
`useCirrus()` throws a clear error when called outside a
`createCirrus`/`provideCirrus` scope, so a missing provider fails loudly instead
...
import { useQuery } from "@cirrus/vue";
import { ref } from "vue";
...
```vue
<script setup lang="ts">
import { useMutation } from "@cirrus/vue";
...
Cirrus subscription cache and rolls them back on failure. Any `useQuery` /
...
query on the server with `preloadQuery` (from `@cirrus/vue/server`), pass the
...
```vue
<script setup lang="ts">
import { hydratePreloaded } from "@cirrus/vue";
import type { Preloaded } from "@cirrus/vue";
...
fetch handler as an importable module. Re-exports `withCirrus`
(`withFrameworkWorker` from [`@cirrus/runtime`](/docs/api/runtime)), which wraps a
framework handler as Cirrus's fallback `httpRouter` and mounts only the reserved
`/_cirrus/*` plane plus `ShardDO`.

> **Nuxt caveat.** Nitro does not expose its emitted fetch handler as an
> importable virtual module, so single-worker `withCirrus` composition is **not**
> achievable for Nuxt. The supported Nuxt integration is a two-worker split: the
> Nuxt/Nitro SSR worker plus a separate standalone Cirrus worker owning
> `/_cirrus/*` + `ShardDO`, wired by `NUXT_PUBLIC_CIRRUS_URL`. `withCirrus`
