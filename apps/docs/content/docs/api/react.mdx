---
title: "@cirrus/react"
description: React 18+/19 hooks built on @cirrus/client, with React Server Component data loading.
---
...
the Next.js App Router lives in [`@cirrus/react/server`](#server-components-nextjs-app-router).
...
Every hook in `@cirrus/react` calls `useState`/`useEffect` and owns a live
...
**`@cirrus/react/server`**. It opens no socket and touches no browser globals,
...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const client = createServerClient({ url: process.env.CIRRUS_URL! });
...
import { usePreloadedQuery } from "@cirrus/react";
import type { Preloaded } from "@cirrus/react/server";
...
Mount `CirrusProvider` once in a Client Component near the root. It creates (or
...
export function Providers({ children }: { children: React.ReactNode }) {
    const [client] = useState(() => new CirrusClient({ url: process.env.NEXT_PUBLIC_CIRRUS_URL! }));
    return <CirrusProvider client={client}>{children}</CirrusProvider>;
}
```

### `@cirrus/react/server` reference

- **`createServerClient({ url, token?, fetch? })`** — a request-scoped,
  HTTP-only `CirrusClient`. Build one **per request** so a user's `token`
