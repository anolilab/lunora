...
`@cirrus/vite` is a Vite plugin factory. A single `cirrus()` call wires up
...
| ------------------ | --------------------- | ------------------------------------------------------------- |
| `schemaDir`        | `"cirrus"`            | Directory containing `schema.ts` + function files             |
| `generatedDir`     | `"cirrus/_generated"` | Where to emit `api.ts`, `server.ts`, `dataModel.ts`           |
