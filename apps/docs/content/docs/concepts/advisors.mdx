...
What sets Cirrus apart from a live-database-only advisor is that most lints are
...
every `cirrus dev` save and on deploy, and the Studio computes the runtime set
when you open the Advisors panels. The catalog is also available
programmatically:

```ts
import { runAdvisor, STATIC_LINTS, RUNTIME_LINTS, ALL_LINTS } from "@cirrus/advisor";
