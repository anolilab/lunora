...
A `*` wildcard combined with `allowCredentials: true` is rejected — browsers
refuse the combination and it defeats the allowlist. The worker throws at
construction, and the [`@lunora/config`](#config-time-validation) validator
catches the same mistake at build time when it is driven through wrangler `vars`.
Preflight (`OPTIONS`) is answered automatically for allowlisted origins.
...
| `nondeterministic_query_mutation`        | warn  | Move `Date.now`/`Math.random`/`fetch` out of the query/mutation into an `action`.     |
