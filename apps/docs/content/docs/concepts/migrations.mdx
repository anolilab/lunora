...
Cirrus has two distinct migration surfaces, because it has two distinct
storage backends:

- **Sharded / root tables** live in each Durable Object's own SQLite. There is
  no schema-migration step — editing [`cirrus/schema.ts`](/docs/concepts/schema)
...
- **Global tables** (`.global()`) live in D1. A relational store needs explicit
  `CREATE TABLE` / `ADD COLUMN` SQL, so `cirrus migrate generate` diffs your
...
> There is no migrations DSL for sharded-table _schema_. If all your tables are
> sharded, the only migration surface you'll touch is `defineMigration` for data
> backfills.

## Schema migrations (global tables)

`cirrus migrate generate` parses `cirrus/schema.ts`, filters to `.global()`
tables, and diffs the result against `cirrus/migrations/.snapshot.json`. When
the diff is non-empty it writes a timestamped SQL file and updates the snapshot.

```bash
cirrus migrate generate                    # name defaults to "auto"
cirrus migrate generate add_email_index    # name the migration
```

This emits `cirrus/migrations/<timestamp>_<slug>.sql` and rewrites
`cirrus/migrations/.snapshot.json`. Commit both — they are deterministic, and
...
The generated SQL is applied through `@cirrus/d1`'s `MigrationRunner`, which
...
`defineMigration` from `@cirrus/server` anywhere under `cirrus/` — codegen
discovers it through the type checker and keys a registry on its `id`.

```ts
// cirrus/migrations.ts
...
> `id` and `table` must be **static string literals**. Codegen lifts them at
> build time to key the registry and resolve the target table, and rejects
> duplicate ids across the project.

### Scaffolding

`cirrus migrate create <name>` writes a `defineMigration` stub into
`cirrus/migrations.ts`, appending to the file (and adding the import) when it
already exists. It refuses to clobber a migration with the same id or export
name.

```bash
cirrus migrate create backfill_read_by --table messages
```
...
`cirrus migrate up | down | status <id>` drives the cross-shard orchestrator. It
resolves the migration's table locally, then POSTs an admin RPC to the worker's
`/_cirrus/migrate` endpoint, which fans the run out to every live shard of that
table and rolls the per-shard outcomes up.

```bash
cirrus migrate up backfill-read-by             # run forward across shards
cirrus migrate up backfill-read-by --dry-run   # preview counts, rewrite nothing
cirrus migrate down backfill-read-by           # apply the down transform
cirrus migrate status backfill-read-by         # per-shard run-state
```
...
| `--token`      | Admin bearer token. Falls back to `CIRRUS_ADMIN_TOKEN`.                                        |
...
Every run is admin-gated: pass `--token` or set `CIRRUS_ADMIN_TOKEN`. Against
...
in a reserved `__cirrus_migrations` table, persisting after every batch. Two
properties follow:
...
[Studio › Database](/docs/addons/studio#database)) inspects and drives data
...
issues no credentials of its own — it relies on the worker's `CIRRUS_ADMIN_TOKEN`
gate like the CLI.

## Resetting local state

`cirrus reset` is not a migration command, but it's the companion when a local
schema change leaves stale dev data. It clears the local Miniflare state
directory (`.wrangler/state`), and with `--all` also removes `.cirrus-cache`.

```bash
cirrus reset          # clear .wrangler/state (prompts to confirm)
cirrus reset --all    # also remove .cirrus-cache
cirrus reset --yes    # skip the prompt (required when stdin is not a TTY)
```
...
- [CLI](/docs/api/cli) — full `cirrus migrate`, `cirrus deploy`, and `cirrus reset` reference.
- [Studio › Database](/docs/addons/studio#database) — the Migrations panel and the rest of the data console.
