...
> The advisor [`constraint_validator`](/docs/concepts/advisors) lint is a
> _complementary_ check: it samples existing rows and flags ones that already
> violate a declared constraint (data inserted before the constraint existed —
> a migration, a raw import, a past bug). Write-time enforcement guards new
> writes; the advisor catches the backlog.
...
> `.unique()` (the column constraint) is distinct from the `.unique()` **query**
> helper, which asserts a read matched at most one row and throws a
...
> `.default(value)` / `.$defaultFn(fn)` make a column _optional on insert_ (the
> write layer fills it) but it is still `NOT NULL` once stored — "optional on
> insert" and "nullable" are independent. Use `.nullable()` only when `NULL` is
> a value you actually want to read back.
...
> **Cross-backend caveat.** When a cascade crosses storage backends (a sharded
> Durable Object holder of a `.global()` D1 parent, or vice-versa) the cascade
> is _not transactional_: the global side fires before the local delete commits.
> If the local delete then fails, holders may be gone while the parent remains.
> Add your own reconciliation (a periodic job or a queue retry) where this
> matters.
...
- [Schema](/docs/concepts/schema) — declaring tables, validators, indexes and relations
- [Advisors](/docs/concepts/advisors) — the `constraint_validator` lint that samples existing rows for violations
- [Row-level security](/docs/concepts/rls) — server-side authorization over which rows a procedure may read or write
