 ...
 *
 * This is vinext's Pages Router worker, verbatim from
 * `vinext init --platform=cloudflare`. It drives the canonical 9-step Next.js
 * request pipeline through vinext's build-time virtual modules. Lunora wraps it
 * (see `worker/index.ts`) rather than replacing it, so realtime `/_lunora/*`
 * traffic and Next.js pages share one Worker.
 *
 * Regenerate with `vinext init --platform=cloudflare` if vinext updates its
 * server API; keep `worker/index.ts` as the wrapper.
 */
            ...
            const deps: PagesPipelineDeps = {
                ...
                renderPage:
                    typeof renderPage === "function"
                        ? (req, resolvedUrl, options, stagedHeaders) => renderPage(req, resolvedUrl, null, ctx, stagedHeaders, options)
                        : null,
                ...
                serveFilesystemRoute: async (requestPathname, _stagedHeaders, phase) => {
                    return fetchWorkerFilesystemRoute(request, requestPathname, phase, (assetRequest) => env.ASSETS.fetch(assetRequest));
                },
            };
