...
/**
 * Branded static welcome page. No Lunora wiring here — the realtime demo lives at
 * `/demo`. The whole design is scoped under `.lunora-welcome` so its global class
 * names cannot collide with app styles (see ./welcome.css).
 */
function WelcomePage() {
    // SSR renders the Night theme so the server HTML is deterministic; we reconcile
    // to the visitor's `prefers-color-scheme` after mount to avoid a hydration
    // mismatch. The signal drives `data-theme` on the `.lunora-welcome` root.
    ...
    return (
        <div class="lunora-welcome" data-theme={theme()}>
            ...
            <div class="lw-wrap">
                ...
                <div class="lw-foot">Running on Lunora · TanStack Start + Solid</div>
            </div>
        </div>
    );
}
