...
import { LUNORA_FUNCTIONS } from "./_generated/functions.js";
 ...
 * the class is bound as `SHARD` in `wrangler.lunora.jsonc`. Pass `scheduler` /
 * `storage` / `d1` thunks here once you add `@lunora/scheduler`,
 * `@lunora/storage`, or `.global()` tables.
 */
...
/**
 * Standalone Lunora worker for the two-worker Nuxt split.
 *
 * This worker owns `/_lunora/*` (RPC + WebSocket realtime) and the `ShardDO`
 * class. The Nuxt/Nitro SSR worker is a separate deployment (`wrangler.jsonc`);
 * the browser client connects its WebSocket here via `runtimeConfig.public.lunoraUrl`
 * (set NUXT_PUBLIC_LUNORA_URL to this worker's URL in production).
 *
 * Why two workers? Nitro does not expose its emitted fetch handler as an
 * importable virtual module, so composing `/_lunora/*` into the Nitro output is
