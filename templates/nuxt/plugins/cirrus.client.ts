...
/**
 * Client-only Nuxt plugin (`.client.ts`): builds the browser `CirrusClient` and
 * provides it to the whole app via `@cirrus/vue`'s `createCirrus` plugin. Every
...
export default defineNuxtPlugin((nuxtApp) => {
    ...
    // Empty `cirrusUrl` → same origin as the page (the common single-worker
    // deploy). Point it at a remote worker for split deploys.
    const url = config.public.cirrusUrl || window.location.origin;

    const client = new CirrusClient({ url });

    nuxtApp.vueApp.use(createCirrus(client));
});
