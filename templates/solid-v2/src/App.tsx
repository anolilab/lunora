...
/**
 * Branded static welcome page.
 *
 * Written in Solid 2 idioms on purpose — this is the first Solid code a new
 * project reads, and the 1.x reflexes it replaces are the ones people (and code
 * generators) reach for by habit:
 *
 *  - `onSettled` for mount-time setup, not `onMount` (removed in 2.0). It also
 *    takes the teardown as a return value instead of a nested `onCleanup`.
 *  - `class={{ … }}` object form for conditional classes, not `classList`
 *    (removed) and not a hand-built string.
 *  - Writes belong in event handlers or `onSettled`; writing a signal from the
 *    component body throws in 2.0's dev mode.
 *  - Reactive values are read inside JSX, never at the top of the body.
 *
 * The whole design is scoped under `.lunora-welcome` so its global class names
 * cannot collide with app styles (see ./welcome.css).
 */
export default function App() {
    // Start on Night so the first paint is deterministic, then reconcile to the
    // visitor's `prefers-color-scheme`. `onSettled` runs once the tree has
    // settled, which is where a write like this is legal in Solid 2.
    ...
    return (
        <div class="lunora-welcome" data-theme={theme()}>
            ...
            <div class="lw-wrap">
                ...
                <div class="lw-foot">Running on Lunora · Solid 2</div>
            </div>
        </div>
    );
}
