 ...
 *
 * The branded Lunora welcome page — a static, dependency-free landing view.
 * The realtime demo lives at `/demo` (`src/app/pages/demo.page.ts`).
 *
 * The markup + CSS are reproduced faithfully from the shared design reference.
 * All CSS is scoped under the root `.lunora-welcome` class, so it cannot collide
 * with app styles even though `encapsulation: ViewEncapsulation.None` applies it
 * globally (Angular's emulated encapsulation would otherwise rewrite the
 * descendant selectors and CSS custom properties this design relies on).
 *
 * Theme: defaults to `"dark"` (Night) for SSR, then reconciles to the user's
 * `prefers-color-scheme` on mount in the browser. The toggle button flips the
 * `theme` signal, which drives `data-theme` on the root element.
 */
@Component({
    ...
    styles: [
        `
            ...
            .lunora-welcome {
                ...
                /* NIGHT (default) */
                --bg: #0e0e11;
                --surface: hsl(240 12% 8% / 0.72);
                --surface-2: hsl(240 11% 11% / 0.82);
                --line: hsl(0 0% 100% / 0.08);
                --line-2: hsl(0 0% 100% / 0.14);
                --t-display: hsl(228 30% 97%);
                --t-primary: hsl(228 26% 90%);
                --t-secondary: hsl(228 12% 62%);
                --t-faint: hsl(228 10% 44%);
                --logo: hsl(228 30% 97%);
                --accent: var(--violet);
                --shot-bg: hsl(240 12% 6%);
                --glow-1: hsl(256 80% 52% / 0.3);
                --glow-2: hsl(196 84% 52% / 0.13);
                --arc: hsl(256 60% 70% / 0.11);
            ...
            .lunora-welcome[data-theme="light"] {
                ...
                --surface-2: hsl(0 0% 100% / 0.95);
                --line: hsl(228 16% 88%);
                --line-2: hsl(228 14% 80%);
                --t-display: hsl(240 14% 10%);
                --t-primary: hsl(240 12% 18%);
                --t-secondary: hsl(235 9% 42%);
                --t-faint: hsl(235 8% 58%);
                --logo: hsl(240 16% 9%);
                --accent: hsl(256 58% 56%);
                --shot-bg: hsl(228 26% 99%);
                --glow-1: hsl(256 80% 60% / 0.14);
                --glow-2: hsl(196 84% 58% / 0.08);
                --arc: hsl(256 40% 55% / 0.12);
            }
            .lunora-welcome *,
            .lunora-welcome *::before,
            .lunora-welcome *::after {
            ...
            .lunora-welcome button {
                font-family: inherit;
                cursor: pointer;
            }
            .lunora-welcome ::selection {
                background: hsl(256 72% 68% / 0.3);
            }
            .lunora-welcome code {
            ...
            .lunora-welcome .lw-bg .glow {
                ...
                border-radius: 50%;
                background: radial-gradient(circle at 50% 50%, var(--glow-1), var(--glow-2) 40%, transparent 66%);
            }
            .lunora-welcome .lw-bg .arc {
                position: absolute;
                left: 50%;
                border-radius: 50%;
            ...
            .lunora-welcome .lw-bg .arc.a1 {
                top: -340px;
                width: 980px;
                height: 980px;
            }
            .lunora-welcome .lw-bg .arc.a2 {
                top: -240px;
                width: 720px;
                height: 720px;
                opacity: 0.7;
            ...
            .lunora-welcome .lw-toggle:hover {
                color: var(--t-display);
                border-color: var(--accent);
            }
            .lunora-welcome .lw-toggle svg {
                width: 13px;
                height: 13px;
            }

            .lunora-welcome .lw-wrap {
                position: relative;
                z-index: 2;
                width: 100%;
                max-width: 1080px;
                margin: 0 auto;
                padding: clamp(44px, 8vh, 92px) clamp(20px, 5vw, 48px);
                display: flex;
                flex-direction: column;
                min-height: 100vh;
            }
            .lunora-welcome .brand {
                display: flex;
                align-items: center;
            ...
            .lunora-welcome .brand svg {
                width: 54px;
                height: auto;
                display: block;
            }
            .lunora-welcome .brand .word {
                font-size: 34px;
                font-weight: 600;
            ...
            .lunora-welcome .card:hover {
                border-color: var(--line-2);
                background: var(--surface-2);
            }
            .lunora-welcome .card:hover .arrow {
                color: var(--accent);
                transform: translateX(3px);
            }
            .lunora-welcome .arrow {
                color: var(--t-faint);
                transition:
                    color 0.2s,
                    transform 0.2s;
            }
            .lunora-welcome .arrow svg {
                width: 20px;
                height: 20px;
                display: block;
            }
            ...
            .lunora-welcome .shot .search {
                flex: 1;
                height: 22px;
                border: 1px solid var(--line);
                border-radius: 4px;
            }
            .lunora-welcome .shot .ver {
                font-family: var(--mono);
                font-size: 8px;
                letter-spacing: 0.08em;
                color: var(--t-faint);
            }
            .lunora-welcome .shot .body {
                display: grid;
                grid-template-columns: 92px 1fr;
                min-height: 224px;
            }
            .lunora-welcome .shot .nav {
            ...
            .lunora-welcome .shot .nav i,
            .lunora-welcome .shot .doc i {
                height: 5px;
                border-radius: 2px;
                background: var(--line);
            }
            .lunora-welcome .shot .doc {
                padding: 16px 18px;
                display: flex;
                flex-direction: column;
                gap: 9px;
            }
            .lunora-welcome .shot .doc .h {
                height: 8px;
                width: 46%;
                border-radius: 2px;
            ...
            .lunora-welcome .shot .doc .accent {
                height: 5px;
                width: 26%;
                border-radius: 2px;
                background: var(--accent);
            }
            .lunora-welcome .feature .info {
                margin-top: auto;
                padding-top: clamp(18px, 2.4vh, 28px);
            }
            .lunora-welcome .feature .ic {
                width: 40px;
                height: 40px;
                margin-bottom: 15px;
            }
            .lunora-welcome .feature .ic svg {
                width: 19px;
                height: 19px;
            }
            .lunora-welcome .feature h2 {
                margin: 0 0 10px;
                font-size: 19px;
                font-weight: 600;
            ...
            .lunora-welcome .feature .row {
                display: flex;
                align-items: flex-end;
                gap: 16px;
            }
            .lunora-welcome .feature p {
                margin: 0;
                color: var(--t-secondary);
                font-size: 14px;
                max-width: 50ch;
            }
            .lunora-welcome .feature .row .arrow {
                margin-left: auto;
            }

            /* right stack — smaller cards, spread to align bottoms with the feature */
            .lunora-welcome .stack {
                display: flex;
                flex-direction: column;
                gap: 16px;
                height: 100%;
            ...
            .lunora-welcome .mini .mc {
                flex: 1;
            }
            .lunora-welcome .mini .ic {
                width: 32px;
                height: 32px;
                margin-bottom: 10px;
            }
            .lunora-welcome .mini .ic svg {
                width: 16px;
                height: 16px;
            }
            .lunora-welcome .mini h3 {
                margin: 0 0 5px;
                font-size: 16px;
                font-weight: 600;
    ...
    template: `
        <div class="lunora-welcome" [attr.data-theme]="theme()">
            <div class="lw-bg">
            ...
            <button class="lw-toggle" type="button" aria-label="Toggle color theme" (click)="toggleTheme()">
                @if (theme() === "dark") {
            ...
            <div class="lw-wrap">
                ...
                <div class="grid">
                    <a class="card feature" href="https://lunora.sh/docs">
                        <div class="shot" aria-hidden="true">
                            ...
                            <div class="body">
                                <div class="nav">
                                    <i style="width:80%"></i><i style="width:60%"></i><i style="width:72%"></i><i style="width:50%"></i><i style="width:66%"></i
                                    ><i style="width:44%"></i><i style="width:58%"></i>
                                </div>
                                <div class="doc">
                                    <span class="h"></span><i style="width:92%"></i><i style="width:88%"></i><span class="accent"></span
                                    ><i style="width:80%"></i><i style="width:90%"></i><i style="width:72%"></i><i style="width:84%"></i
                                    ><i style="width:78%"></i>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="mc">
                                <span class="ic"
                                    ><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7">
                                        ...
                                        <path d="M3 7l9 5 9-5M12 12v10" /></svg
                                ></span>
                ...
                <div class="lw-foot">Running on Lunora · AnalogJS</div>
            </div>
        </div>
    `,
})
export default class WelcomePage implements OnInit {
    /** Active theme — drives `data-theme` on the root. SSR default is Night ("dark"). */
    protected readonly theme = signal<"dark" | "light">("dark");

    private readonly platformId = inject(PLATFORM_ID);

    /** Reconcile to the user's `prefers-color-scheme` once mounted in the browser. */
    public ngOnInit(): void {
        if (isPlatformBrowser(this.platformId) && globalThis.matchMedia?.("(prefers-color-scheme: light)").matches) {
