...
| **A batch slot whose `error` key holds a NON-OBJECT reports the durable write COMMITTED.** Every port matches the slot's envelope with a type-checked pattern (`isinstance` / a comma-ok assertion / `as?` / `is Map` / `.filter(is_object)`), and a string, `null`, an array or a number fails that test and falls straight through to the commit branch: `{"id":0,"body":{"error":"bad gateway"}}` settles the write committed with a `null` result and un-persists it. | **Behind `protocol/README.md` §4.3, in all eight.** The spec retries a slot with no readable verdict — it carries no HTTP status of its own, so there is nothing to classify it by, which leaves it where a slot the server never returned already is — and the reference client does. The whole-batch branch a few lines above each of these loops already re-queues the IDENTICAL shape, so the two disagree on one value. Nothing in `conformance-cases.json` reaches it; `responseTransportError` covers the single-call path only. |
