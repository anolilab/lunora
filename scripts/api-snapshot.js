...
const COVERED = [
    ...TIER_1.map((dir) => ({ dir, tier: "core" })),
    ...TIER_2.map((dir) => ({ dir, tier: "stable-adapter" })),
    ...TIER_3.map((dir) => ({ dir, tier: "experimental" })),
];
...
const renderPackage = (program, checker, covered) => {
    ...
    // The experimental tier is snapshotted to make drift visible, not to promise
    // anything about it — saying "SemVer applies" there would be false, and would
    // be read as a commitment the tier has not made.
    const stabilityNote =
        covered.tier === "experimental"
