...
const LUNORA_ADVISORIES: AdvisoryFinding[] = [
    ...
    {
        "cacheKey": "procedure_without_structured_event:billing:recordApiCall",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public action `recordApiCall` (billing) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "recordApiCall",
    ...
    {
        "cacheKey": "procedure_without_structured_event:billing:apiCallsRemaining",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public action `apiCallsRemaining` (billing) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "apiCallsRemaining",
