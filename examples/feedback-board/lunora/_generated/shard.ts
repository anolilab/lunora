...
const LUNORA_ADVISORIES: AdvisoryFinding[] = [
    {
        "cacheKey": "nondeterministic_query_mutation:summaries:64:Date.now",
        "categories": [
            "SCHEMA"
        ],
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:feedback:create",
        "categories": [
            "SECURITY"
        ],
        ...
        "detail": "Public mutation `create` (feedback) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "create",
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:feedback:setStatus",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public mutation `setStatus` (feedback) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:feedback:toggleVote",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public mutation `toggleVote` (feedback) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "toggleVote",
            "file": "feedback",
            "kind": "mutation",
            "sensitive": false
        },
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:feedback:addComment",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public mutation `addComment` (feedback) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:feedback:remove",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public mutation `remove` (feedback) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "remove",
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:summaries:generate",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public action `generate` (summaries) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "generate",
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:summaries:remove",
        "categories": [
            "SECURITY"
        ],
        "description": "A public `mutation`/`action` has no `rateLimit` middleware. Publicly-callable writes are flood and brute-force targets — an attacker can exhaust writes, mail quota, or credits, or guess credentials on auth-shaped endpoints.",
        "detail": "Public mutation `remove` (summaries) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "remove",
    ...
    {
        "cacheKey": "procedure_without_structured_event:feedback:create",
        "categories": [
            "SCHEMA"
        ],
        ...
        "detail": "Public mutation `create` (feedback) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "create",
    ...
    {
        "cacheKey": "procedure_without_structured_event:feedback:setStatus",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `setStatus` (feedback) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "setStatus",
    ...
    {
        "cacheKey": "procedure_without_structured_event:feedback:toggleVote",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `toggleVote` (feedback) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "toggleVote",
    ...
    {
        "cacheKey": "procedure_without_structured_event:feedback:addComment",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `addComment` (feedback) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "addComment",
    ...
    {
        "cacheKey": "procedure_without_structured_event:feedback:remove",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `remove` (feedback) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "remove",
    ...
    {
        "cacheKey": "procedure_without_structured_event:summaries:generate",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public action `generate` (summaries) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "generate",
    ...
    {
        "cacheKey": "procedure_without_structured_event:summaries:remove",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `remove` (summaries) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "remove",
    ...
    {
        "cacheKey": "ai_unbounded_generation_public:summaries:generate",
        "categories": [
            "SECURITY"
        ],
        ...
        "detail": "`generate` (summaries) is public and runs an AI generation with no `maxOutputTokens` bound — an anonymous caller can drive unbounded, billable completions in a loop. Add a `maxOutputTokens` cap and rate-limit the entry point.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "generate",
    ...
    {
        "cacheKey": "ai_run_without_logging:summaries:generate",
        "categories": [
            "PERFORMANCE"
        ],
        ...
        "detail": "action `generate` (summaries) runs an AI generation with no structured event, so its cost and output cannot be attributed later.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "generate",
...
const LUNORA_SCHEMA_SNAPSHOT: { hash: string; json: string } = { hash: "88040490acd3066e", json: "{\n  \"migrationIds\": [],\n  \"tables\": {\n    \"comments\": {\n      \"fields\": {\n        \"feedbackId\": {\n          \"kind\": \"id\",\n          \"optional\": false\n        },\n        \"authorName\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"authorEmail\": {\n          \"kind\": \"string\",\n          \"optional\": true\n        },\n        \"content\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"isOfficial\": {\n          \"kind\": \"boolean\",\n          \"optional\": false\n        }\n      },\n      \"indexes\": {\n        \"by_feedback\": {\n          \"fields\": [\n            \"feedbackId\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"feedback\": {\n      \"fields\": {\n        \"title\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"description\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"status\": {\n          \"kind\": \"union\",\n          \"optional\": false\n        },\n        \"authorName\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"authorEmail\": {\n          \"kind\": \"string\",\n          \"optional\": true\n        },\n        \"upvoteCount\": {\n          \"kind\": \"number\",\n          \"optional\": false\n        },\n        \"commentCount\": {\n          \"kind\": \"number\",\n          \"optional\": false\n        },\n        \"tags\": {\n          \"kind\": \"array\",\n          \"optional\": true\n        }\n      },\n      \"indexes\": {\n        \"by_upvotes\": {\n          \"fields\": [\n            \"upvoteCount\"\n          ],\n          \"unique\": false\n        },\n        \"by_status\": {\n          \"fields\": [\n            \"status\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"summaries\": {\n      \"fields\": {\n        \"title\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"summary\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        },\n        \"feedbackIds\": {\n          \"kind\": \"array\",\n          \"optional\": false\n        },\n        \"generatedAt\": {\n          \"kind\": \"number\",\n          \"optional\": false\n        }\n      },\n      \"indexes\": {\n        \"by_generated\": {\n          \"fields\": [\n            \"generatedAt\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"votes\": {\n      \"fields\": {\n        \"feedbackId\": {\n          \"kind\": \"id\",\n          \"optional\": false\n        },\n        \"voterEmail\": {\n          \"kind\": \"string\",\n          \"optional\": false\n        }\n      },\n      \"indexes\": {\n        \"by_voter\": {\n          \"fields\": [\n            \"voterEmail\"\n          ],\n          \"unique\": false\n        },\n        \"by_feedback_and_voter\": {\n          \"fields\": [\n            \"feedbackId\",\n            \"voterEmail\"\n          ],\n          \"unique\": true\n        }\n      },\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    }\n  },\n  \"version\": 1\n}\n" };
