...
const LUNORA_TABLE_INDEXES = JSON.parse("{\"posts\":[{\"fields\":[\"publishedAt\"],\"name\":\"by_published\",\"type\":\"index\"},{\"fields\":[\"body\"],\"name\":\"posts_search\",\"type\":\"vector\"}],\"drafts\":[{\"fields\":[\"updatedAt\"],\"name\":\"by_updated\",\"type\":\"index\"},{\"fields\":[\"authorId\",\"updatedAt\"],\"name\":\"by_author_updated\",\"type\":\"index\"}],\"ratelimit_buckets\":[{\"fields\":[\"key\"],\"name\":\"by_key\",\"type\":\"index\"}]}") as Record<string, Array<{ fields: string[]; name: string; type: "geo" | "index" | "rank" | "search" | "vector"; unique?: boolean }>>;

/** Columns per table (typed, with PK/FK markers) for the studio's schema diagram, served via `__lunora_admin__:describeTable`. */
const LUNORA_TABLE_COLUMNS = JSON.parse("{\"posts\":[{\"name\":\"_id\",\"optional\":false,\"pk\":true,\"type\":\"id\"},{\"name\":\"_creationTime\",\"optional\":false,\"type\":\"number\"},{\"name\":\"authorId\",\"optional\":false,\"type\":\"string\"},{\"name\":\"title\",\"optional\":false,\"type\":\"string\"},{\"name\":\"body\",\"optional\":false,\"type\":\"string\"},{\"name\":\"imageKey\",\"optional\":true,\"type\":\"string\"},{\"name\":\"publishedAt\",\"optional\":false,\"type\":\"number\"}],\"drafts\":[{\"name\":\"_id\",\"optional\":false,\"pk\":true,\"type\":\"id\"},{\"name\":\"_creationTime\",\"optional\":false,\"type\":\"number\"},{\"name\":\"authorId\",\"optional\":false,\"type\":\"string\"},{\"name\":\"title\",\"optional\":false,\"type\":\"string\"},{\"name\":\"body\",\"optional\":false,\"type\":\"string\"},{\"name\":\"updatedAt\",\"optional\":false,\"type\":\"number\"}],\"ratelimit_buckets\":[{\"name\":\"_id\",\"optional\":false,\"pk\":true,\"type\":\"id\"},{\"name\":\"_creationTime\",\"optional\":false,\"type\":\"number\"},{\"name\":\"key\",\"optional\":false,\"type\":\"string\"},{\"name\":\"value\",\"optional\":false,\"type\":\"number\"},{\"name\":\"ts\",\"optional\":false,\"type\":\"number\"},{\"name\":\"prev\",\"optional\":true,\"type\":\"number\"}]}") as Record<
    string,
    Array<{
        bucket?: string;
        enumValues?: string[];
...
const LUNORA_ADVISORIES = JSON.parse("[{\"cacheKey\":\"nondeterministic_query_mutation:cleanup:14:Date.now\",\"categories\":[\"SCHEMA\"],\"description\":\"A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). A `query` may be re-run by a live subscription, so non-determinism there can flicker between evaluations (WARN). An ordinary `mutation` handler does not replay on this runtime — it runs at most once per logical write — so this is informational there (INFO) unless the mutation is itself invoked from a workflow step or queue consumer that can replay.\",\"detail\":\"`Date.now(…)` in purgeStaleDrafts (cleanup:14) runs inside a mutation handler. Ordinary mutations don't replay on this runtime (idempotency dedup returns a cached result rather than re-running the handler, and an OCC conflict throws to the caller instead of retrying internally), so this is informational — no action needed unless `purgeStaleDrafts` is invoked from a workflow step or queue consumer that can itself replay.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"callee\":\"Date.now\",\"exportName\":\"purgeStaleDrafts\",\"file\":\"cleanup\",\"kind\":\"mutation\",\"line\":14},\"name\":\"nondeterministic_query_mutation\",\"remediation\":\"For a `query`: move the non-deterministic call into an `action(...)` (which runs once and may use ambient APIs), then pass the computed value into the mutation as an argument, or accept that the value may differ across re-evaluations. For an ordinary `mutation`: no action needed — the handler runs at most once per logical write on this runtime. If the mutation is dispatched from inside a workflow step or queue consumer, treat it like an action value instead, since the surrounding step/consumer can replay.\",\"title\":\"Non-deterministic call in query/mutation handler\"},{\"cacheKey\":\"nondeterministic_query_mutation:drafts:78:Date.now\",\"categories\":[\"SCHEMA\"],\"description\":\"A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). A `query` may be re-run by a live subscription, so non-determinism there can flicker between evaluations (WARN). An ordinary `mutation` handler does not replay on this runtime — it runs at most once per logical write — so this is informational there (INFO) unless the mutation is itself invoked from a workflow step or queue consumer that can replay.\",\"detail\":\"`Date.now(…)` in save (drafts:78) runs inside a mutation handler. Ordinary mutations don't replay on this runtime (idempotency dedup returns a cached result rather than re-running the handler, and an OCC conflict throws to the caller instead of retrying internally), so this is informational — no action needed unless `save` is invoked from a workflow step or queue consumer that can itself replay.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"callee\":\"Date.now\",\"exportName\":\"save\",\"file\":\"drafts\",\"kind\":\"mutation\",\"line\":78},\"name\":\"nondeterministic_query_mutation\",\"remediation\":\"For a `query`: move the non-deterministic call into an `action(...)` (which runs once and may use ambient APIs), then pass the computed value into the mutation as an argument, or accept that the value may differ across re-evaluations. For an ordinary `mutation`: no action needed — the handler runs at most once per logical write on this runtime. If the mutation is dispatched from inside a workflow step or queue consumer, treat it like an action value instead, since the surrounding step/consumer can replay.\",\"title\":\"Non-deterministic call in query/mutation handler\"},{\"cacheKey\":\"nondeterministic_query_mutation:drafts:83:Date.now\",\"categories\":[\"SCHEMA\"],\"description\":\"A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). A `query` may be re-run by a live subscription, so non-determinism there can flicker between evaluations (WARN). An ordinary `mutation` handler does not replay on this runtime — it runs at most once per logical write — so this is informational there (INFO) unless the mutation is itself invoked from a workflow step or queue consumer that can replay.\",\"detail\":\"`Date.now(…)` in save (drafts:83) runs inside a mutation handler. Ordinary mutations don't replay on this runtime (idempotency dedup returns a cached result rather than re-running the handler, and an OCC conflict throws to the caller instead of retrying internally), so this is informational — no action needed unless `save` is invoked from a workflow step or queue consumer that can itself replay.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"callee\":\"Date.now\",\"exportName\":\"save\",\"file\":\"drafts\",\"kind\":\"mutation\",\"line\":83},\"name\":\"nondeterministic_query_mutation\",\"remediation\":\"For a `query`: move the non-deterministic call into an `action(...)` (which runs once and may use ambient APIs), then pass the computed value into the mutation as an argument, or accept that the value may differ across re-evaluations. For an ordinary `mutation`: no action needed — the handler runs at most once per logical write on this runtime. If the mutation is dispatched from inside a workflow step or queue consumer, treat it like an action value instead, since the surrounding step/consumer can replay.\",\"title\":\"Non-deterministic call in query/mutation handler\"},{\"cacheKey\":\"nondeterministic_query_mutation:posts:122:Date.now\",\"categories\":[\"SCHEMA\"],\"description\":\"A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). A `query` may be re-run by a live subscription, so non-determinism there can flicker between evaluations (WARN). An ordinary `mutation` handler does not replay on this runtime — it runs at most once per logical write — so this is informational there (INFO) unless the mutation is itself invoked from a workflow step or queue consumer that can replay.\",\"detail\":\"`Date.now(…)` in publish (posts:122) runs inside a mutation handler. Ordinary mutations don't replay on this runtime (idempotency dedup returns a cached result rather than re-running the handler, and an OCC conflict throws to the caller instead of retrying internally), so this is informational — no action needed unless `publish` is invoked from a workflow step or queue consumer that can itself replay.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"callee\":\"Date.now\",\"exportName\":\"publish\",\"file\":\"posts\",\"kind\":\"mutation\",\"line\":122},\"name\":\"nondeterministic_query_mutation\",\"remediation\":\"For a `query`: move the non-deterministic call into an `action(...)` (which runs once and may use ambient APIs), then pass the computed value into the mutation as an argument, or accept that the value may differ across re-evaluations. For an ordinary `mutation`: no action needed — the handler runs at most once per logical write on this runtime. If the mutation is dispatched from inside a workflow step or queue consumer, treat it like an action value instead, since the surrounding step/consumer can replay.\",\"title\":\"Non-deterministic call in query/mutation handler\"},{\"cacheKey\":\"procedure_without_structured_event:drafts:save\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public mutation `save` (drafts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"save\",\"file\":\"drafts\",\"kind\":\"mutation\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"},{\"cacheKey\":\"procedure_without_structured_event:posts:requestImageUpload\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public action `requestImageUpload` (posts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"requestImageUpload\",\"file\":\"posts\",\"kind\":\"action\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"},{\"cacheKey\":\"procedure_without_structured_event:posts:publish\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public mutation `publish` (posts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"publish\",\"file\":\"posts\",\"kind\":\"mutation\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"}]") as AdvisoryFinding[];
...
const LUNORA_ADVISOR_PROCEDURES = JSON.parse("[{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"purgeStaleDrafts\",\"file\":\"cleanup\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"internal\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"listMine\",\"file\":\"drafts\",\"hasEmailArg\":false,\"kind\":\"query\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"save\",\"file\":\"drafts\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"list\",\"file\":\"posts\",\"hasEmailArg\":false,\"kind\":\"query\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"get\",\"file\":\"posts\",\"hasEmailArg\":false,\"kind\":\"query\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"search\",\"file\":\"posts\",\"hasEmailArg\":false,\"kind\":\"query\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":true,\"reachesOutbound\":true,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"requestImageUpload\",\"file\":\"posts\",\"hasEmailArg\":false,\"kind\":\"action\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"publish\",\"file\":\"posts\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"public\"}]") as AdvisorProcedure[];
...
const LUNORA_STUDIO_FEATURES = JSON.parse("{\"analytics\":true,\"auth\":true,\"containers\":false,\"flags\":false,\"kv\":true,\"mail\":false,\"notifications\":false,\"payments\":false,\"queues\":false,\"scheduler\":true,\"storage\":true,\"vectors\":true,\"workflows\":false,\"platform\":{\"features\":{\"shardedState\":\"native\",\"globalTables\":\"native\",\"websocketHibernation\":\"native\",\"durableStreams\":\"emulated\",\"commitOrderedTables\":\"native\",\"localSql\":\"native\",\"serverReactors\":\"emulated\",\"memoryTables\":\"emulated\",\"shardAlarms\":\"native\",\"shardPlacement\":\"native\",\"shardReadReplicas\":\"emulated\",\"crossShardFanout\":\"emulated\",\"queues\":\"native\",\"relationGraph\":\"emulated\",\"workflows\":\"native\",\"scheduler\":\"emulated\",\"cronTriggers\":\"native\",\"authJurisdictionMove\":\"native\",\"agents\":\"emulated\",\"objectStorage\":\"native\",\"objectStorageBackups\":\"emulated\",\"objectStorageCdcArchive\":\"emulated\",\"keyValueStore\":\"native\",\"vectorStore\":\"native\",\"ai\":\"native\",\"browser\":\"native\",\"images\":\"native\",\"containers\":\"native\",\"analytics\":\"native\",\"edgeRequestMetadata\":\"native\",\"hostTraceFusion\":\"native\",\"logArchive\":\"native\",\"pipelines\":\"native\",\"pointInTimeRecovery\":\"native\",\"mail\":\"emulated\",\"secrets\":\"native\",\"hyperdrive\":\"native\",\"httpCache\":\"native\",\"identityProxy\":\"native\"},\"id\":\"cloudflare\",\"name\":\"Cloudflare\"}}") as StudioFeaturesResult;
...
const LUNORA_SCHEMA_SNAPSHOT: { hash: string; json: string } = { hash: "5372ca59110a5db3", json: "{\n  \"migrationIds\": [],\n  \"tables\": {\n    \"drafts\": {\n      \"commitOrdered\": false,\n      \"fields\": {\n        \"authorId\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"body\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"title\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"updatedAt\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        }\n      },\n      \"indexes\": {\n        \"by_author_updated\": {\n          \"fields\": [\n            \"authorId\",\n            \"updatedAt\"\n          ],\n          \"unique\": false\n        },\n        \"by_updated\": {\n          \"fields\": [\n            \"updatedAt\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"memory\": false,\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"posts\": {\n      \"commitOrdered\": false,\n      \"fields\": {\n        \"authorId\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"body\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"imageKey\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": true,\n          \"unique\": false\n        },\n        \"publishedAt\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"title\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        }\n      },\n      \"indexes\": {\n        \"by_published\": {\n          \"fields\": [\n            \"publishedAt\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"memory\": false,\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"ratelimit_buckets\": {\n      \"commitOrdered\": false,\n      \"fields\": {\n        \"key\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"prev\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": true,\n          \"unique\": false\n        },\n        \"ts\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"value\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        }\n      },\n      \"indexes\": {\n        \"by_key\": {\n          \"fields\": [\n            \"key\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"memory\": false,\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    }\n  },\n  \"version\": 1\n}\n" };
