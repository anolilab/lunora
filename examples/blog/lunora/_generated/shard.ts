...
const LUNORA_ADVISORIES: AdvisoryFinding[] = [
    ...
    },
    {
        ...
        "detail": "Public mutation `save` (drafts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "save",
    ...
    {
        "cacheKey": "procedure_without_structured_event:posts:requestImageUpload",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public action `requestImageUpload` (posts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
    ...
    {
        "cacheKey": "procedure_without_structured_event:posts:publish",
        "categories": [
            "SCHEMA"
        ],
        "description": "A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.",
        "detail": "Public mutation `publish` (posts) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.",
        "facing": "INTERNAL",
        "level": "INFO",
        "metadata": {
            "exportName": "publish",
