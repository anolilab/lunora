...
const LUNORA_ADVISORIES: AdvisoryFinding[] = [
    ...
    {
        "cacheKey": "nondeterministic_query_mutation:posts:95:Date.now",
        "categories": [
            "SCHEMA"
        ],
        "description": "A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). These handlers may be re-run on OCC retry / subscription re-evaluation, so they must be deterministic — time, randomness, and network belong in an `action`.",
        "detail": "`Date.now(…)` in publish (posts:95) runs inside a mutation handler — query/mutation handlers must be deterministic. Compute it in an `action` and pass the value into the mutation as an argument.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "callee": "Date.now",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `text` of public procedure `search` (posts:33) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "text",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `contentType` of public procedure `requestImageUpload` (posts:61) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `title` of public procedure `publish` (posts:84) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "title",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `body` of public procedure `publish` (posts:84) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "body",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `imageKey` of public procedure `publish` (posts:84) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "imageKey",
