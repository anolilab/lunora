...
/**
 * Email + password only — this example is about game state, not about auth. See
 * `examples/auth-playground` for OAuth, organizations, admin, and 2FA.
 *
 * Only the options are exported: `defineApp().auth({ … })` in
 * `src/server/index.ts` owns building the instance, running the migration
 * sweep, and dispatching `/api/auth/*`.
 *
