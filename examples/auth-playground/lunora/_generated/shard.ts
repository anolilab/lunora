...
const LUNORA_ADVISORIES: AdvisoryFinding[] = [
    {
        "cacheKey": "nondeterministic_query_mutation:documents:66:Date.now",
        "categories": [
            "SCHEMA"
        ],
        ...
        "detail": "`Date.now(…)` in create (documents:66) runs inside a mutation handler — query/mutation handlers must be deterministic. Compute it in an `action` and pass the value into the mutation as an argument.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "callee": "Date.now",
    ...
    {
        "cacheKey": "public_mutation_without_ratelimit:documents:create",
        "categories": [
            "SECURITY"
        ],
        ...
        "detail": "Public mutation `create` (documents) has no rate limit. Add `.use(rateLimit(...))` or `.use(protectPublic({ rateLimit }))`.",
        "facing": "EXTERNAL",
        "level": "WARN",
        "metadata": {
            "exportName": "create",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `organizationId` of public procedure `create` (documents:52) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "organizationId",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `title` of public procedure `create` (documents:52) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "title",
    ...
    },
    {
        ...
        "description": "A public `v.string()` argument has no maximum-length bound. An unbounded string lets a client submit arbitrarily large input — abusing storage and CPU on every request that processes it.",
        "detail": "Arg `body` of public procedure `create` (documents:52) is an unbounded `v.string()`. Add a max-length bound to cap payload size.",
        "facing": "EXTERNAL",
        "level": "INFO",
        "metadata": {
            "argument": "body",
