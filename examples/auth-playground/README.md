# @lunora-example/auth-playground

End-to-end demo of the better-auth plugins Lunora re-exports under
`@lunora/auth/plugins` — `organization` and `admin`. Sign up, create an
...
```bash
pnpm install
pnpm --filter @lunora-example/auth-playground dev
```
...
1. Create a D1 database: `wrangler d1 create lunora-auth-playground`.
2. Drop the returned `database_id` into `wrangler.jsonc`.
3. Apply better-auth's schema:

    ```bash
    pnpm tsx -e "import('@lunora/auth').then(async ({ compileMigrationsSql, createAuth }) => { const sql = await compileMigrationsSql({ database: undefined, secret: 'placeholder' }); console.log(sql); })" \
      > schema.sql
    wrangler d1 execute lunora-auth-playground --file schema.sql
    ```
...
src/
  server/index.ts         worker entry — routes /api/auth/* + Lunora RPC
  client/auth-client.ts   browser SDK with the matching plugin clients
  client/main.tsx         bootstraps <LunoraProvider> + <App>
...
`lunora/auth.ts`. The same migration story applies — `ensureMigrated` will
