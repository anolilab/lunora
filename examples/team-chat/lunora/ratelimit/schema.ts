...
export const limits = {
    ...
    // A heartbeat runs every 15s per open tab, so this has to admit several
    // tabs per user without ever admitting a flood.
    presence: { kind: "token bucket", period: 60_000, rate: 60 },
    // Each upload mints a signed URL and costs storage; keep it deliberate.
