...
const LUNORA_TABLE_INDEXES = JSON.parse("{\"todos\":[{\"fields\":[\"createdAt\"],\"name\":\"by_creation\",\"type\":\"index\"}],\"ratelimit_buckets\":[{\"fields\":[\"key\"],\"name\":\"by_key\",\"type\":\"index\"}]}") as Record<string, Array<{ fields: string[]; name: string; type: "geo" | "index" | "rank" | "search" | "vector"; unique?: boolean }>>;

/** Columns per table (typed, with PK/FK markers) for the studio's schema diagram, served via `__lunora_admin__:describeTable`. */
const LUNORA_TABLE_COLUMNS = JSON.parse("{\"todos\":[{\"name\":\"_id\",\"optional\":false,\"pk\":true,\"type\":\"id\"},{\"name\":\"_creationTime\",\"optional\":false,\"type\":\"number\"},{\"name\":\"text\",\"optional\":false,\"type\":\"string\"},{\"name\":\"done\",\"optional\":false,\"type\":\"boolean\"},{\"name\":\"createdAt\",\"optional\":false,\"type\":\"number\"}],\"ratelimit_buckets\":[{\"name\":\"_id\",\"optional\":false,\"pk\":true,\"type\":\"id\"},{\"name\":\"_creationTime\",\"optional\":false,\"type\":\"number\"},{\"name\":\"key\",\"optional\":false,\"type\":\"string\"},{\"name\":\"value\",\"optional\":false,\"type\":\"number\"},{\"name\":\"ts\",\"optional\":false,\"type\":\"number\"},{\"name\":\"prev\",\"optional\":true,\"type\":\"number\"}]}") as Record<
    string,
    Array<{
        bucket?: string;
        enumValues?: string[];
...
const LUNORA_ADVISORIES = JSON.parse("[{\"cacheKey\":\"nondeterministic_query_mutation:todos:41:Date.now\",\"categories\":[\"SCHEMA\"],\"description\":\"A `query`/`mutation` handler calls a non-deterministic API (`Date.now`, `Math.random`, `crypto.randomUUID`, `crypto.getRandomValues`, or `fetch`). A `query` may be re-run by a live subscription, so non-determinism there can flicker between evaluations (WARN). An ordinary `mutation` handler does not replay on this runtime — it runs at most once per logical write — so this is informational there (INFO) unless the mutation is itself invoked from a workflow step or queue consumer that can replay.\",\"detail\":\"`Date.now(…)` in add (todos:41) runs inside a mutation handler. Ordinary mutations don't replay on this runtime (idempotency dedup returns a cached result rather than re-running the handler, and an OCC conflict throws to the caller instead of retrying internally), so this is informational — no action needed unless `add` is invoked from a workflow step or queue consumer that can itself replay.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"callee\":\"Date.now\",\"exportName\":\"add\",\"file\":\"todos\",\"kind\":\"mutation\",\"line\":41},\"name\":\"nondeterministic_query_mutation\",\"remediation\":\"For a `query`: move the non-deterministic call into an `action(...)` (which runs once and may use ambient APIs), then pass the computed value into the mutation as an argument, or accept that the value may differ across re-evaluations. For an ordinary `mutation`: no action needed — the handler runs at most once per logical write on this runtime. If the mutation is dispatched from inside a workflow step or queue consumer, treat it like an action value instead, since the surrounding step/consumer can replay.\",\"title\":\"Non-deterministic call in query/mutation handler\"},{\"cacheKey\":\"procedure_without_structured_event:todos:add\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public mutation `add` (todos) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"add\",\"file\":\"todos\",\"kind\":\"mutation\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"},{\"cacheKey\":\"procedure_without_structured_event:todos:toggle\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public mutation `toggle` (todos) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"toggle\",\"file\":\"todos\",\"kind\":\"mutation\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"},{\"cacheKey\":\"procedure_without_structured_event:todos:remove\",\"categories\":[\"SCHEMA\"],\"description\":\"A public `mutation`/`action` emits no structured event. When it fails you get a stack trace with no request context — no ids, no tenant, no outcome — so the failure is visible but not searchable.\",\"detail\":\"Public mutation `remove` (todos) emits no structured event. Add a `ctx.log` line or a `ctx.span` so a failure carries its request context.\",\"facing\":\"INTERNAL\",\"level\":\"INFO\",\"metadata\":{\"exportName\":\"remove\",\"file\":\"todos\",\"kind\":\"mutation\"},\"name\":\"procedure_without_structured_event\",\"remediation\":\"Emit one event on the primary path: `ctx.log.info(\\\"<verb>\\\", { … })`, or wrap the handler in `ctx.span(\\\"<name>\\\", …)` to attach timing too.\",\"title\":\"Public write emits no structured event\"}]") as AdvisoryFinding[];
...
const LUNORA_ADVISOR_PROCEDURES = JSON.parse("[{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":false,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"list\",\"file\":\"todos\",\"hasEmailArg\":false,\"kind\":\"query\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"add\",\"file\":\"todos\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"toggle\",\"file\":\"todos\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"public\"},{\"callsMail\":false,\"emitsEvent\":false,\"fanOut\":false,\"handlesErrors\":false,\"reachesOutbound\":false,\"runsAiGeneration\":false,\"throwsBareError\":false,\"unboundedAiGeneration\":false,\"usesInsertManyUnsafe\":false,\"writesUserTable\":false,\"exempt\":false,\"exemptReason\":\"\",\"usesCaptcha\":false,\"usesEmailGate\":false,\"usesMask\":false,\"usesRateLimit\":true,\"usesRls\":false,\"analyzableBody\":true,\"exportName\":\"remove\",\"file\":\"todos\",\"hasEmailArg\":false,\"kind\":\"mutation\",\"visibility\":\"public\"}]") as AdvisorProcedure[];
...
const LUNORA_SCHEMA_SNAPSHOT: { hash: string; json: string } = { hash: "1bc59f2f2212cac9", json: "{\n  \"migrationIds\": [],\n  \"tables\": {\n    \"ratelimit_buckets\": {\n      \"commitOrdered\": false,\n      \"fields\": {\n        \"key\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"prev\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": true,\n          \"unique\": false\n        },\n        \"ts\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"value\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        }\n      },\n      \"indexes\": {\n        \"by_key\": {\n          \"fields\": [\n            \"key\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"memory\": false,\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    },\n    \"todos\": {\n      \"commitOrdered\": false,\n      \"fields\": {\n        \"createdAt\": {\n          \"kind\": \"number\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"done\": {\n          \"kind\": \"boolean\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        },\n        \"text\": {\n          \"kind\": \"string\",\n          \"nullable\": false,\n          \"optional\": false,\n          \"unique\": false\n        }\n      },\n      \"indexes\": {\n        \"by_creation\": {\n          \"fields\": [\n            \"createdAt\"\n          ],\n          \"unique\": false\n        }\n      },\n      \"memory\": false,\n      \"relations\": {},\n      \"shardMode\": \"root\"\n    }\n  },\n  \"version\": 1\n}\n" };
